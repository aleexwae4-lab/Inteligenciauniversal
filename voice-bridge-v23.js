(()=>{
  ...
  function install(){
    ...
    if(native){native.setAttribute('aria-pressed','true');native.addEventListener('click',()=>{queueMicrotask(async()=>{const v=engine();if(!v)return;await v.unlock?.();if(v.engine==='browser'&&document.documentElement.dataset.voiceState==='playing'){v.pause?.();sync({enabled:true,state:'paused'});return}if(document.documentElement.dataset.voiceState==='paused'){v.resume?.();sync({enabled:true,state:'playing'});return}v.setEnabled?.(true);const last=window.__waeLastAnswer;if(last){await v.speak?.(last,{force:true});return}sync({enabled:true,state:'ready'})})})}
    const form=document.getElementById('mobileSafeComposer'),send=document.getElementById('mobileSafeSend');
    if(form&&send&&!document.getElementById('mobileSafeVoice')){const b=document.createElement('button');b.type='button';b.id='mobileSafeVoice';b.setAttribute('aria-label','Voz automática');b.textContent='🔊';b.style.cssText='flex:0 0 42px;width:42px;height:42px;border:1px solid rgba(255,255,255,.12);border-radius:11px;background:#11161a;color:#79e8a4;font:700 18px/1 system-ui;pointer-events:auto;touch-action:manipulation';b.addEventListener('click',async()=>{if(native){native.click();return}const v=engine();if(!v)return;await v.unlock?.();if(document.documentElement.dataset.voiceState==='playing'){v.pause?.();sync({enabled:true,state:'paused'});return}if(document.documentElement.dataset.voiceState==='paused'){v.resume?.();sync({enabled:true,state:'playing'});return}await v.setEnabled?.(true);sync({enabled:true,state:'ready'})});form.insertBefore(b,send)}
