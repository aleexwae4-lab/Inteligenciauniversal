(()=>{
  ...
  async function renderChat(body,signal){
    ...
    const budget=payload.web_enabled||payload.mode==='research'?32000:20000;
  ...
  function cleanFailure(body,error){
    const payload=error?.payload&&typeof error.payload==='object'?error.payload:{
      ...
      message:'Universal Core canceló esta ejecución antes de dejar la conversación bloqueada. Reintenta el turno; las ejecuciones anteriores ya fueron liberadas.',
      recoverable:true,
  ...
  async function executeEdgeTurn(body,signal){
    ...
    if(hit){
      if(hit.ok)return body.stream===true?sseResponse(hit.data,{'x-wae-dedupe':'recent'}):jsonResponse(hit.data,200,{'x-wae-dedupe':'recent'});
      return jsonResponse(hit.data,hit.status,{'x-wae-dedupe':'recent-failure'});
    }
    ...
    if(inflight.has(key)){
      ...
      return jsonResponse(result.data,result.status,{'x-wae-dedupe':'inflight-failure'});
    }
    ...
    const active=linkSignal(signal,body.web_enabled===true||body.mode==='research'?33000:21000);
    activeTurn={key,controller:active.controller};

    const task=(async()=>{
      try{
        const data=await renderChat(body,active.signal);
        const result={ok:true,data,status:200};
        recent.set(key,{...result,at:Date.now()});
        return result;
      }catch(error){
        const failure=cleanFailure(body,error);
        const result={ok:false,data:failure.payload,status:failure.status};
        recent.set(key,{...result,at:Date.now()});
        return result;
      }finally{
        active.cleanup();
  ...
  window.fetch=async(input,init={})=>{
    ...
    if(direct.matches){
      ...
      if(hit){
        return hit.ok?jsonResponse(hit.data,200,{'x-wae-dedupe':'direct-replay'}):jsonResponse(hit.data,hit.status,{'x-wae-dedupe':'direct-failure'});
      }
      if(inflight.has(key)){
        const result=await inflight.get(key);
        return result.ok?jsonResponse(result.data,200,{'x-wae-dedupe':'direct-inflight'}):jsonResponse(result.data,result.status,{'x-wae-dedupe':'direct-inflight-failure'});
      }
    }
