...
test('v76 remains directly callable while live compatibility advances through v81 and preserves v77 plus v63 control',async()=>{
  ...
  const v81Wrapper=await readFile(new URL('../api/capacity-chat-v81.js',import.meta.url),'utf8');
  const controlPlane=await readFile(new URL('../lib/gpu-control-plane-v81.js',import.meta.url),'utf8');
  assert.match(alias,/capacity-chat-v81\.js/);
  ...
  assert.match(v81Wrapper,/capacityChatV63/);assert.match(v81Wrapper,/generateWithGpuControlPlane/);assert.match(v81Wrapper,/sensitiveRequest/);assert.match(v81Wrapper,/requiresGroundedData/);assert.match(v81Wrapper,/originAllowed/);assert.match(v81Wrapper,/allowRequest/);assert.match(v81Wrapper,/WAE_GPU_ALLOW_ATTACHMENTS/);assert.match(v81Wrapper,/WAE_GPU_ALLOW_UNGROUNDED_RESEARCH/);
  assert.match(controlPlane,/gpu-fabric-v77\.js/);assert.match(controlPlane,/wae-gpu-scheduler\/v81/);assert.match(controlPlane,/wae_gpu_metrics_record_v79/);
});
