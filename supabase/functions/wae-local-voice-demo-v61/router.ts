...
const sensitiveRx=/\b(curp|rfc|nss|pasaporte|credencial|ine|domicilio|direcci[oó]n personal|tel[eé]fono|correo personal|email personal|expediente|carpeta de investigaci[oó]n|historia cl[ií]nica|paciente|nombre completo|contrase[nñ]a|password|api[_ -]?key|token|secreto|secret|confidencial|privado|datos personales|personal data|medical record|case file)\b/i;
...
export function classifyTask(q:string,mode='general',attachments:any[]=[]){
  q=String(q||'').trim();const files=a(attachments).length>0,sensitiveData=files||sensitiveRx.test(q);let category='analysis';
  ...
  return{category,path,risk:category==='high_risk'?'high':path==='DEEP'?'medium':'low',complexity:path==='FAST'?'low':path==='STANDARD'?'medium':'high',sensitiveData};
}
