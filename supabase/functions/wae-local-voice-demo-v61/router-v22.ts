...
const FAST_LATENCY_BUDGET_MS=3000;
const STREAM_TTFT_BUDGET_MS=2500;
...
function health(m:any){return String(m.effective_health||m.registry_health||'unknown').toLowerCase()}
function isContinuity(m:any){return String(m.provider||'').toLowerCase()==='universal_core'}
function productionStreamEligible(m:any){return m.streaming_claimed===true&&m.streaming_verified===true&&m.circuit_state==='CLOSED'&&health(m)==='healthy'&&Number.isFinite(Number(m.ewma_ttft_ms))&&Number(m.ewma_ttft_ms)<=STREAM_TTFT_BUDGET_MS}
function fastLaneEligible(m:any){return !isContinuity(m)&&m.circuit_state==='CLOSED'&&health(m)==='healthy'&&Number.isFinite(Number(m.ewma_latency_ms))&&Number(m.ewma_latency_ms)<=FAST_LATENCY_BUDGET_MS&&Number(m.reliability??m.reliability_score??60)>=50}

// Probe eligibility remains permissive so the canary can discover newly working streaming transports.
export function streamEligible(m:any){if(!m.streaming_claimed)return false;if(m.streaming_verified)return true;const probes=Number(m.streaming_probe_count||0),lastFail=Date.parse(String(m.last_stream_failure_at||''));return probes===0||!Number.isFinite(lastFail)||Date.now()-lastFail>6*3600000}
function capabilityMatch(m:any,req:any){if(req.vision&&!m.vision_capable)return 0;if(req.tools&&!m.tools_capable)return 0;if(req.streaming&&!productionStreamEligible(m))return 0;if(req.context_window&&Number(m.context_window||0)<Number(req.context_window))return 0;let z=100;if(req.reasoning==='high'&&!m.reasoning_capable)z-=25;if(req.structured_output&&!m.structured_output_capable)z-=8;if(req.streaming&&m.streaming_verified)z+=8;return Math.max(0,Math.min(108,z))}
function scoreModel(m:any,task:any,req:any,specific:any){const cap=capabilityMatch(m,req);if(cap<=0)return{...m,eligible:false,score:-999};const quality=n(specific?.quality_score??m.eval_score??m.quality_score??m.reputation_score,70),reliability=n(m.reliability_score??specific?.reliability_score??m.reputation_reliability_score,60),w=weights(task.path);let score=quality*w.q+reliability*w.r+latScore(m.ewma_latency_ms)*w.l+ttftScore(m.ewma_ttft_ms)*w.t+costScore(m)*w.c+cap*w.m;const h=health(m);if(h==='degraded')score-=12;if(h==='unknown')score-=5;if(m.circuit_state==='HALF_OPEN')score-=25;score-=failurePenalty(m);const minimum=task.path==='FAST'?65:task.path==='DEEP'?78:70,sufficient=quality>=minimum&&reliability>=45;if(!sufficient)score-=20;return{...m,eligible:true,quality,reliability,sufficient,failure_debt:failureDebt(m),score:Number(score.toFixed(3))}}
...
export async function registry(db:any,task:any,q:string){const cap=evalCapability(task,q);const[{data:models},{data:rep}]=await Promise.all([db.from('iu_adaptive_model_registry_v1').select('*').eq('enabled',true).in('access_tier',['FREE','TRIAL','LOCAL']),db.from('wae_provider_capability_reputation_v1').select('provider,model,quality_score,reliability_score,reputation_score,confidence_score,samples').eq('capability',cap)]);const map=new Map((rep||[]).map((x:any)=>[`${x.provider}::${x.model}`,x]));return(models||[]).filter(canInvoke).map((m:any)=>({...m,specific:map.get(`${m.provider}::${m.model_name}`)||null}))}
...
export function rank(reg:any[],task:any,req:any,variant='candidate'){
  ...
  if(req.streaming)list=list.filter(productionStreamEligible);
  ...
  const continuity=ranked.filter(isContinuity);
  const normal=ranked.filter((x:any)=>!isContinuity(x));

  // FAST paths get an explicit production latency budget. Quality/reliability still gate eligibility.
  if(task.path==='FAST'){
    const fast=normal.filter((x:any)=>x.sufficient&&fastLaneEligible(x));
    const fastSet=new Set(fast);
    const closed=normal.filter((x:any)=>x.circuit_state==='CLOSED'&&!fastSet.has(x));
    const halfOpen=normal.filter((x:any)=>x.circuit_state==='HALF_OPEN');
    const known=new Set([...fast,...closed,...halfOpen]);
    const remaining=normal.filter((x:any)=>!known.has(x));
    return[...fast,...closed,...halfOpen,...remaining,...continuity];
  }

  const preferred=normal.filter((x:any)=>x.sufficient&&x.circuit_state==='CLOSED');
  const preferredSet=new Set(preferred);
  const closedFallbacks=normal.filter((x:any)=>x.circuit_state==='CLOSED'&&!preferredSet.has(x));
  const halfOpenFallbacks=normal.filter((x:any)=>x.circuit_state==='HALF_OPEN');
  const known=new Set([...preferred,...closedFallbacks,...halfOpenFallbacks]);
  const remaining=normal.filter((x:any)=>!known.has(x));
  return[...preferred,...closedFallbacks,...halfOpenFallbacks,...remaining,...continuity];
}
...
export async function markSuccess(db:any,m:any,latency:number,ttft:any,streamed=false){const oldL=Number(m.ewma_latency_ms),ewmaL=Number.isFinite(oldL)?Math.round(oldL*.8+latency*.2):latency;const ledger:any={provider:m.provider,model:m.model_name,consecutive_failures:0,consecutive_successes:Math.min(32767,Number(m.consecutive_successes||0)+1),ewma_latency_ms:ewmaL,open_until:null,last_success_at:nowIso(),last_error_class:'',score_snapshot:Math.min(100,Math.max(60,Number(m.reliability_score||80)+2)),health_snapshot:'healthy',updated_at:nowIso(),expires_at:new Date(Date.now()+86400000).toISOString()};if(streamed&&Number.isFinite(Number(ttft))){const oldT=Number(m.ewma_ttft_ms);ledger.ewma_ttft_ms=Number.isFinite(oldT)?Math.round(oldT*.8+Number(ttft)*.2):Number(ttft)}const ops:any[]=[db.from('wae_ai_models').update({consecutive_failures:0,last_success_at:nowIso(),health_status:'healthy',circuit_open_until:null}).eq('provider',m.provider).eq('model_name',m.model_name).is('organization_id',null),db.from('wae_provider_reliability_ledger_v1').upsert(ledger,{onConflict:'provider,model'})];if(streamed){const now=nowIso();ops.push(db.from('iu_model_transport_capabilities_v1').upsert({provider:m.provider,model_name:m.model_name,streaming_verified:true,probe_count:Number(m.streaming_probe_count||0)+1,streaming_success_count:Number(m.streaming_success_count||0)+1,streaming_failure_count:Number(m.streaming_failure_count||0),ewma_ttft_ms:ledger.ewma_ttft_ms??Number(ttft),last_probe_at:now,last_stream_success_at:now,last_failure_reason:null,metadata:{source:'adaptive-router-v22',delta_verified:true},updated_at:now},{onConflict:'provider,model_name'}))}await Promise.allSettled(ops)}
