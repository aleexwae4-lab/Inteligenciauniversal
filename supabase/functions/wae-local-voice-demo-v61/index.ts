...
async function webSearch(db:any,q:string){const{data,error}=await db.rpc('wae_live_web_search_internal_v2',{p_query:q,p_limit:6});if(error)throw error;const p=o(data),rows=a(p.sources);return rows.map((x:any,i:number)=>({key:`W${i+1}`,title:s(x?.title,500)||s(x?.host,250)||'Fuente web',url:s(x?.url,1800),host:s(x?.host,250),snippet:s(x?.snippet,1800),published_at:s(x?.published_at,100)||null})).filter((x:any)=>/^https?:\/\//i.test(x.url)).slice(0,6)}
...
async function prepareChat(db:any,sid:string,body:any,requestReceived:number){
  ...
  const mode=['general','research','code','analysis','design','executive'].includes(s(body.mode,20))?s(body.mode,20):'general',task=classifyTask(q,mode,body.attachments),reqs=requirements(task,mode,body.web_enabled===true,body.attachments,body.stream===true),routerStarted=Date.now();
  ...
  const mem=memr.data||[],system=policy(mode)+memoryContext(mem)+webContext(sources)+fileContext(body.attachments)+`\n\nCurrent UTC date: ${new Date().toISOString().slice(0,10)}.`,msgs=[{role:'system',content:system},...[...(hr.data||[])].reverse().map((x:any)=>({role:x.role,content:s(x.content,9000)}))],reg=await registry(db,task,q),routerVariant=variant(body,sid),ranked=rank(reg,task,reqs,routerVariant);
  return{sid,requestReceived,requestId,q,mode,task,reqs,cid,userId,sources,webError,mem,msgs,ranked,routerVariant,routerStarted}
}

async function finalize(db:any,ctx:any,generated:any,degraded:boolean,failures:any[],providerTtft:any){
  const providerRequestAt=generated.provider_request_at||Date.now(),latency=Date.now()-ctx.requestReceived,routerOverhead=Math.max(0,providerRequestAt-ctx.requestReceived),assistantId=crypto.randomUUID(),cost=estimateCost(generated.modelRow,generated.input_tokens,generated.output_tokens),hasTtft=providerTtft!==null&&providerTtft!==undefined&&Number.isFinite(Number(providerTtft)),response=responseObject({text:generated.text,sources:ctx.sources,provider:generated.provider,model:generated.model,latency,memoryCount:ctx.mem.length,messageId:assistantId,conversationId:ctx.cid,requestId:ctx.requestId,webUsed:ctx.sources.length>0,degraded,task:ctx.task,routerVariant:ctx.routerVariant,providerTtft:hasTtft?Number(providerTtft):null,routerOverhead,cost});
...
async function executeModels(db:any,ctx:any,stream:boolean,signal:AbortSignal|null,onDelta?:(d:string,t:number)=>void){
  ...
  let degraded=false;
  if(!generated&&ctx.sources.length&&!stream){degraded=true;generated={text:`Recuperé evidencia web, pero el modelo de síntesis no respondió correctamente. Conservo las fuentes verificadas para que no se pierda el resultado.\n\n${ctx.sources.map((x:any)=>`- **${x.title}** — ${x.snippet}`).join('\n')}`,provider:'web_recovery',model:'evidence-only',input_tokens:null,output_tokens:null,modelRow:{cost_profile:{}},provider_request_at:Date.now(),ttft_ms:null,streamed:false};providerTtft=null}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(action==='health'||action==='capabilities'){
      ...
      return jsonResponse(200,{success:true,version:VERSION,response_schema:SCHEMA,router:ROUTER,canary_pct:CANARY_PCT,model_count:reg.length,verified_streaming_models:verified.length,memory:true,web_search:true,feedback_learning:true,training_examples:true,output_guard:true,structured_responses:true,natural_voice:true,observability:true,adaptive_routing:true,streaming_mode:'verified_only',ttft:'real_first_delta_only',circuit_breaker:true,cost_engine:true,eval_driven:true,response_style:'premium-rich-voice-safe',tools:['memory.recall','web.search','feedback.learn','files.context','response.structure'],modes:['general','research','code','analysis','design','executive']},origin)
    }
