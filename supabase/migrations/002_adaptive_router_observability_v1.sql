alter table public.iu_request_traces add column if not exists router_overhead_ms integer;
alter table public.iu_request_traces add column if not exists provider_ttft_ms integer;
alter table public.iu_request_traces add column if not exists client_ttft_ms integer;
alter table public.iu_request_traces add column if not exists routing_path text;
alter table public.iu_request_traces add column if not exists task_category text;
alter table public.iu_request_traces add column if not exists router_variant text;
alter table public.wae_provider_reliability_ledger_v1 add column if not exists ewma_ttft_ms integer;

create index if not exists iu_request_traces_router_created_idx on public.iu_request_traces(router_variant, created_at desc);
create index if not exists iu_request_traces_provider_model_created_idx on public.iu_request_traces(provider, model_name, created_at desc);
...
select m.id,m.provider,m.model_name,m.enabled,m.access_tier,m.priority,m.health_status registry_health,
  case
    ...
    when coalesce(m.consecutive_failures,0)>0
      and coalesce(m.last_failure_at,'epoch'::timestamptz)>coalesce(m.last_success_at,'epoch'::timestamptz)
  ...
  r.ewma_latency_ms,r.ewma_ttft_ms,r.score_snapshot reliability_score,r.health_snapshot reliability_health,
  r.open_until reliability_open_until,r.updated_at reliability_updated_at,
  rep.quality_score,rep.reputation_reliability_score,rep.reputation_score,rep.reputation_confidence,
  rep.reputation_samples,rep.reputation_refreshed_at,ev.eval_score,ev.eval_samples,ev.last_eval_at,
