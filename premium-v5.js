(()=>{
  ...
  const MOBILE_BRAIN_RELEASE='mobile-brain/v110-full-universal-core';
  ...
  function loadUniversalBrain(){
    if(window.__WAE_MOBILE_BRAIN_V110__||document.querySelector('script[data-wae-mobile-brain="v110"]'))return;
    const script=document.createElement('script');script.src='./mobile-brain-v110.js?v=110';script.async=false;script.dataset.waeMobileBrain='v110';
    script.addEventListener('load',()=>{document.documentElement.dataset.waeMobileBrain='v110';window.dispatchEvent(new CustomEvent('wae:mobile-brain-loader-ready',{detail:{version:MOBILE_BRAIN_RELEASE}}))},{once:true});
    document.head.appendChild(script);
  }
  ...
  function init(){document.documentElement.dataset.waePremiumRich='v43';loadGptExperience();loadProductModules();loadFeedbackHistory();observe();loadUniversalBrain();setTimeout(upgrade,250);setTimeout(upgrade,900)}
  document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init,{once:true}):init();
  window.__waePremiumRich={version:'v43',visibleResponse:GPT_EXPERIENCE_RELEASE,productModules:PRODUCT_MODULES_RELEASE,feedbackHistory:FEEDBACK_HISTORY_RELEASE,mobileBrain:MOBILE_BRAIN_RELEASE,refresh:upgrade};
})();
