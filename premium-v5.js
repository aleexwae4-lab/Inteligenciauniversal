(()=>{
  ...
  const ADAPTIVE_WORKSPACE_RELEASE='adaptive-workspace/v107';
  ...
  function loadAdaptiveWorkspace(){
    if(!document.querySelector('link[data-wae-adaptive-workspace="v107"]')){
      const css=document.createElement('link');css.rel='stylesheet';css.href='./adaptive-workspace-v107.css?v=107';css.dataset.waeAdaptiveWorkspace='v107';document.head.appendChild(css);
    }
    if(window.__WAE_ADAPTIVE_WORKSPACE_V107__||document.querySelector('script[data-wae-adaptive-workspace="v107"]'))return;
    const script=document.createElement('script');script.src='./adaptive-workspace-v107.js?v=107';script.async=false;script.dataset.waeAdaptiveWorkspace='v107';
    script.addEventListener('load',()=>{document.documentElement.dataset.waeAdaptiveWorkspace='v107';window.dispatchEvent(new CustomEvent('wae:adaptive-workspace-loader-ready',{detail:{version:ADAPTIVE_WORKSPACE_RELEASE}}))},{once:true});
    document.head.appendChild(script);
  }
  ...
  function init(){document.documentElement.dataset.waePremiumRich='v43';loadGptExperience();loadProductModules();loadFeedbackHistory();loadAdaptiveWorkspace();observe();setTimeout(upgrade,250);setTimeout(upgrade,900)}
  document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init,{once:true}):init();
  window.__waePremiumRich={version:'v43',visibleResponse:GPT_EXPERIENCE_RELEASE,productModules:PRODUCT_MODULES_RELEASE,feedbackHistory:FEEDBACK_HISTORY_RELEASE,adaptiveWorkspace:ADAPTIVE_WORKSPACE_RELEASE,refresh:upgrade};
})();
