...
async function persistTrustedCertification({req,certification,entries,targetId,referenceId,commitSha}){
  ...
  try{
    const response=await fetch(`${base}/rest/v1/rpc/wae_record_trusted_supremacy_v54`,{
      method:'POST',headers:{'Content-Type':'application/json',apikey:key,'X-Client-Info':'wae-benchmark-arena-v62'},
...
export default async function handler(req,res){
  ...
  if(action==='evaluate'){
    ...
    return res.status(200).json({success:true,evaluation:evaluateBenchmarkCandidate({caseId:text(body.caseId||'ad-hoc',120),prompt,mode,answer,sources:body.sources,latencyMs:body.latencyMs,costUsd:body.costUsd,assertions,metadata:{source:'api/evals-v62'}})});
  }
