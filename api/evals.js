...
async function persistTrustedCertification({req,certification,entries,targetId,referenceId,commitSha}){
  ...
  try{
    const response=await fetch(`${base}/rest/v1/rpc/wae_record_trusted_supremacy_v54`,{method:'POST',headers:{'Content-Type':'application/json',apikey:key,'X-Client-Info':'wae-benchmark-arena-v62'},body:JSON.stringify({p_worker_token:token,p_certification:certification,p_entries:safeEntries,p_target_id:targetId,p_reference_id:referenceId,p_commit_sha:text(commitSha,80)||null}),signal:AbortSignal.timeout(5000)});
...
async function persistAdversarialCertification({req,certification,entries,targetId,referenceId,commitSha}){
  ...
  const base=String(process.env.SUPABASE_URL||'').replace(/\/$/,'');const key=String(process.env.SUPABASE_PUBLISHABLE_KEY||'');if(!base||!key)return{ok:false,status:503,error:'supabase_configuration_missing'};
  ...
  try{
    const response=await fetch(`${base}/rest/v1/rpc/wae_record_trusted_evidence_v72`,{method:'POST',headers:{'Content-Type':'application/json',apikey:key,'X-Client-Info':'wae-adversarial-evidence-v72'},body:JSON.stringify({p_worker_token:token,p_certification:certification,p_entries:attestations,p_target_id:targetId,p_reference_id:referenceId,p_commit_sha:text(commitSha,80)||null}),signal:AbortSignal.timeout(8000)});
    const payload=await response.json().catch(()=>null);if(!response.ok)return{ok:false,status:[400,401,403].includes(response.status)?403:502,error:'trusted_evidence_record_rejected',detail:text(payload?.message||payload?.error||'',180)};return{ok:true,status:200,record:payload};
  }catch{return{ok:false,status:502,error:'trusted_evidence_record_unavailable'}}
}
