(()=>{
  ...
  function policy(payload={}){
    ...
    const current=CURRENT_RX.test(q),research=String(payload.mode||'').toLowerCase()==='research'||payload.web_enabled===true||RESEARCH_RX.test(q),highRisk=HIGH_RISK_RX.test(q);
    return{gate:current||research||highRisk,current,research,highRisk};
  }
  ...
  window.fetch=async(input,init={})=>{
    ...
    window.dispatchEvent(new CustomEvent('wae:truth-speed-gate',{detail:{version:VERSION,current:decision.current,research:decision.research,highRisk:decision.highRisk}}));

    // Mobile attempts verified SSE first. Sensitive requests intentionally skip that
