...
function importProject(e){const f=e.target.files?.[0];e.target.value='';if(!f)return;if(f.size>2000000)return notify('El archivo excede 2 MB');f.text().then(text=>{const data=JSON.parse(text);if(data.format!=='wae-factory/v1')throw Error('Formato no admitido');const p=cleanProject(data.project);p.id=id();projects.unshift(p);projects=projects.slice(0,25);current=p;selected=p.files[0].name;persist();render();preview();notify('Proyecto importado')}).catch(()=>notify('No se pudo importar: revisa el formato'))}
...
$('#wfProjects').addEventListener('change',e=>{save();current=projects.find(p=>p.id===e.target.value)||current;selected=current.files[0].name;persist();render();preview()});
$('#wfNewProject').addEventListener('click',()=>{const name=prompt('Nombre del producto digital');if(!name?.trim())return;save();current=createProject(name.trim().slice(0,80));projects.unshift(current);projects=projects.slice(0,25);selected='index.html';persist();render();preview()});
