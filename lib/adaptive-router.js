const rx={
  ...
  structured:/\b(json|csv|tabla|estructur|extrae|clasifica|columnas?|filas?|dataset|datos|schema|esquema)\b/i,
  maps:/\b(mapa|maps|ubicaci[oó]n|direcci[oó]n|cerca de|cerca|negocio[s]?|comercio[s]?|banco[s]?|oxxo|hotel(?:es)?|restaurante[s]?|cine[s]?|farmacia[s]?|gasolinera[s]?)\b/i,
  automation:/\b(automatiza|automatizaci[oó]n|workflow|flujo|cron|programa(?:do)?|recordatorio|tarea recurrente|agente|ejecuta|orquesta)\b/i,
  product:/\b(producto|product[oó]gica|roadmap|feature|funcionalidad|ux|ui|experiencia de usuario|mvp|saas)\b/i,
  creative:/\b(crea|dise[nñ]a|copy|historia|guion|slogan|marca|branding|creativo)\b/i,
  government:/\b(gobierno|gubernamental|municipio|municipal|estado|secretar[ií]a|dependencia|servidor p[uú]blico|ciudadano|pol[ií]tica p[uú]blica|servicio p[uú]blico|transparencia|protecci[oó]n civil)\b/i,
...
export function classifyTask(message,{mode='general',attachments=[]}={}){
  ...
  else if(rx.maps.test(q))category='local_intelligence';
  else if(rx.automation.test(q))category='automation';
  else if(rx.product.test(q))category='product_engineering';
  else if(rx.structured.test(q))category='structured_data';
  else if(rx.creative.test(q)||mode==='design')category='creative';
  else if(rx.government.test(q))category='government';
  ...
  const path=['simple_chat','factual'].includes(category)?'FAST':['high_risk','web_research','reasoning','government','local_intelligence','automation','product_engineering'].includes(category)?'DEEP':'STANDARD';
...
export function capabilityRequirements(task,{mode='general',webEnabled=false,attachments=[],stream=false}={}){
  ...
  return {reasoning:task.path==='DEEP'?'high':task.path==='STANDARD'?'medium':'low',web:webEnabled||mode==='research'||task.category==='web_research',tools:['tool_execution','local_intelligence','automation','product_engineering'].includes(task.category),vision:task.category==='multimodal',files:files||task.category==='document_analysis',rag:files||task.category==='document_analysis',structuredOutput:['structured_data','coding','government','enterprise','analysis','high_risk','local_intelligence','automation','product_engineering'].includes(task.category),contextWindow:files||task.path==='DEEP'?32768:8192,latencyPriority:task.path==='FAST'?'high':task.path==='STANDARD'?'medium':'low',risk:task.risk,streaming:stream===true};
}
