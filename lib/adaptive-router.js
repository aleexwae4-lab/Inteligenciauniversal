...
export function classifyTask({message='', mode='general', attachments=[]}={}) {
  ...
  else if (mode==='research' || researchRx.test(q)) category='web_research';
...
export function capabilityRequirements(task,{mode='general',webEnabled=false,attachments=[]}={}) {
  ...
  return {
    ...
    structured_output: ['structured_data','coding','enterprise','analysis','high_risk'].includes(category),
...
export function scoreModel(model, task, req) {
  ...
  const cost=costScore(model.input_per_million ?? model.cost_input_per_million ?? null);
