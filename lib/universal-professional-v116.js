...
export function projectEconomics(input){
  ...
  const {unitPrice,unitVariableCost,fixedCost,volume,initialInvestment,currency='MXN',period='mes'}=input;
  ...
  if(typeof currency!=='string'||!/^[A-Z]{3}$/.test(currency)||typeof period!=='string'||period.length>35||!period.trim())return null;
