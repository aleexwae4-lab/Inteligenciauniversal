...
export const ANSWER_COMPOSITION_GUIDANCE=[
  ...
  'Para análisis y misiones complejas, prioriza: conclusión ejecutiva, evidencia o cálculo, implicaciones, límites y acciones siguientes. No devuelvas un catálogo de capacidades cuando el usuario pidió trabajo.',
  'Si el usuario solicita una capacidad, demuestra primero el trabajo que puede ejecutarse con los datos disponibles; pide únicamente el insumo mínimo que bloquee la ejecución.',
  'No repitas la misma conclusión en resumen, cuerpo y cierre.'
].join(' ');
...
function normalizeProse(block=''){
  ...
  for(let raw of lines){
    ...
    for(let partIndex=0;partIndex<expanded.length;partIndex++){
      let part=expanded[partIndex];
      const continuedFromBreak=partIndex>0;
      const t=part.trim();
      if(!t){
        blankRun++;
      ...
      if(numbered){
        ...
        orderedCounter=0;
      }else if(!bullet&&!continuedFromBreak){
        orderedCounter=0;
      }
      out.push(part);
    }
