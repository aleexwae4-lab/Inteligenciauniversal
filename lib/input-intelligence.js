...
function replaceSpanishWhat(text,corrections){
  return text.replace(/\b(un|una|el|los|unos|unas)\s+what\b/gi,(match,article)=>{
    corrections.push({from:'what',to:'watt'});
    return `${article} watt`;
  });
}
...
export function normalizeUserIntent(value=''){
  ...
  if(electricalScore>=2){
    ...
    text=replaceSpanishWhat(text,corrections);
    text=replaceTracked(text,/\b(?:wat|guat)\b/gi,'watt',corrections);
