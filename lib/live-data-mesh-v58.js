...
function normalizeSearxng(item={},retrievedAt){
  const url=text(item.url,1800);
  if(!/^https?:\/\//i.test(url))return null;
  return{
    title:text(item.title||'Fuente web',500),url,host:hostOf(url),snippet:text(item.snippet||item.content||'',1800),
    published_at:item.published_at?parseSeenDate(item.published_at):null,retrieved_at:retrievedAt,
    source:'searxng',source_class:'live_metasearch',freshness_tier:item.published_at?freshness(parseSeenDate(item.published_at),retrievedAt).tier:'retrieved_live',
    freshness_age_ms:item.published_at?freshness(parseSeenDate(item.published_at),retrievedAt).ageMs:null,
    score:Number.isFinite(Number(item.score))?Number(item.score):null,
    engine:text(item.engine,120)||null
  };
}
...
async function searchSearxng(query,{fetchImpl=fetch,maxResults=6,timespan='7d'}={}){
  const timeRange=timespan==='24h'?'day':timespan==='7d'?'week':timespan==='30d'?'month':undefined;
  const result=await executeOpenSourceTool('oss.searxng.search',{query,limit:maxResults,...(timeRange?{time_range:timeRange}:{})},{fetchImpl});
  if(!result?.success)throw new Error(result?.error||'searxng_search_failed');
  return arr(result?.result?.results);
}

function mergeSources(groups,retrievedAt){
  const dedup=new Map();
  for(const item of groups.flat().filter(Boolean)){
    const normalized=['gdelt-doc','tavily','searxng'].includes(item.source)?item:null;
...
export async function retrieveLiveData({message='',mode='general',webEnabled=false,force=false,maxResults=6,fetchImpl=fetch,webSearch=executeWebSearch}={}){
  ...
  if(process.env.SEARXNG_BASE_URL)tasks.push({id:'searxng',promise:searchSearxng(query,{fetchImpl,maxResults,timespan})});
  ...
  settled.forEach((result,index)=>{
    ...
    if(result.status==='fulfilled'){
      ...
      groups.push(arr(result.value).map(item=>id==='gdelt-doc'?normalizeGdelt(item,retrievedAt):id==='searxng'?normalizeSearxng(item,retrievedAt):normalizeTavily(item,retrievedAt)).filter(Boolean));
    }else{
