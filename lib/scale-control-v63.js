...
let snapshotCache={at:0,capability:'',value:null};

function text(value,max=240){return String(value??'').slice(0,max)}
function configured(){return Boolean(process.env.SUPABASE_URL&&process.env.SUPABASE_PUBLISHABLE_KEY&&process.env.WAE_RUNTIME_BRIDGE_TOKEN)}
...
async function callBridge(action,payload={},timeoutMs=BRIDGE_TIMEOUT_MS){
  ...
  try{
    const response=await fetch(`${base}/rest/v1/rpc/wae_runtime_control_bridge_v63`,{
      method:'POST',
      headers:{
        'Content-Type':'application/json',
        apikey:String(process.env.SUPABASE_PUBLISHABLE_KEY),
        'X-Client-Info':'wae-universal-core-v63'
      },
      body:JSON.stringify({p_action:action,p_payload:payload,p_token:String(process.env.WAE_RUNTIME_BRIDGE_TOKEN)}),
      signal:AbortSignal.timeout(timeoutMs)
    });
    const data=await response.json().catch(()=>null);
    if(!response.ok)return{ok:false,error:'runtime_bridge_rejected',status:response.status};
    return data&&typeof data==='object'?data:{ok:false,error:'runtime_bridge_invalid_payload'};
  }catch(error){return{ok:false,error:'runtime_bridge_unavailable',detail:text(error?.message,120)}}
}
...
export async function distributedAdmission({principal='anonymous',session='',tenant='',body={}}={}){
  ...
  const result=await callBridge('admit',{
    principalHash,sessionHash,tenantHash,
    perMinute:PER_MINUTE,
    globalConcurrent:GLOBAL_CONCURRENT,
    leaseSeconds:LEASE_SECONDS
  });
  if(result?.ok===true)return{applied:true,allowed:result.allowed===true,principalHash,sessionHash,tenantHash,...result};
  const failClosed=String(process.env.WAE_DISTRIBUTED_ADMISSION_FAIL_CLOSED||'0')==='1';
  return{applied:true,allowed:!failClosed,degraded:true,mode:'local_fallback',principalHash,sessionHash,tenantHash,reason:result?.error||'bridge_unavailable'};
}
...
export async function loadPersistentRouterSnapshot({capability='general_reasoning',force=false}={}){
  ...
  const now=Date.now();
  if(!force&&snapshotCache.value&&snapshotCache.capability===normalized&&now-snapshotCache.at<SNAPSHOT_TTL_MS)return snapshotCache.value;
  const result=await callBridge('snapshot',{capability:normalized,limit:40});
  if(result?.ok===true){snapshotCache={at:now,capability:normalized,value:result};return result}
  return snapshotCache.value||{ok:false,runtime:[],capabilityReputation:[],error:result?.error||'snapshot_unavailable'};
}

export async function persistProviderObservation(observation={}){
  if(!configured())return{ok:false,unconfigured:true};
  const safe={
    provider:text(observation.provider,100),model:text(observation.model,240),
    success:observation.success===true,errorClass:text(observation.errorClass,40),
    latencyMs:Number.isFinite(Number(observation.latencyMs))?Number(observation.latencyMs):undefined,
    ttftMs:Number.isFinite(Number(observation.ttftMs))?Number(observation.ttftMs):undefined,
    quality:Number.isFinite(Number(observation.quality))?Number(observation.quality):undefined,
    evidence:Number.isFinite(Number(observation.evidence))?Number(observation.evidence):undefined,
    instruction:Number.isFinite(Number(observation.instruction))?Number(observation.instruction):undefined,
    qualityGrade:text(observation.qualityGrade,20),
    costMicrounits:Number.isFinite(Number(observation.costMicrounits))?Number(observation.costMicrounits):undefined
  };
  if(!safe.provider)return{ok:false,error:'provider_required'};
  const result=await callBridge('observe',safe,OBSERVE_TIMEOUT_MS);
  if(result?.ok===true)snapshotCache.at=0;
  return result;
}
...
export function scaleControlCapabilities(){
  ...
  return{
    version:SCALE_CONTROL_VERSION,
    softwareSubscriberTarget:SCALE_TARGET_SUBSCRIBERS,
    distributedAdmission:true,
    admissionShards:SCALE_ADMISSION_SHARDS,
    distributedConcurrencyTarget:GLOBAL_CONCURRENT,
    localConcurrencyPerInstance:localLimit,
    estimatedInstancesForConfiguredPeak:Math.ceil(GLOBAL_CONCURRENT/localLimit),
    persistentProviderReputation:true,
    hashedPrincipalsOnly:true,
    storesPromptContent:false,
    storesResponseContent:false,
    horizontalScaleReady:true,
    currentInfrastructureLoadCertified:false,
    certificationPolicy:'20k subscriber readiness is software architecture; production peak capacity requires multi-instance infrastructure and load certification'
  };
}

export function scaleControlSnapshot(){return{...scaleControlCapabilities(),bridgeConfigured:configured(),snapshotTtlMs:SNAPSHOT_TTL_MS,leaseSeconds:LEASE_SECONDS,distributedRatePerMinute:PER_MINUTE}}

export function __resetScaleControlForTests(){snapshotCache={at:0,capability:'',value:null}}
