...
export const TOOL_FABRIC_VERSION='universal-tool-fabric/v67-web-intelligence-v2';
...
const configured={
  ...
  web_intelligence:()=>bool(process.env.TAVILY_API_KEY||process.env.BRAVE_SEARCH_API_KEY||(process.env.GOOGLE_CUSTOM_SEARCH_API_KEY&&process.env.GOOGLE_CUSTOM_SEARCH_ENGINE_ID)),
  github:()=>bool(process.env.GITHUB_TOKEN),
  supabase:()=>bool(process.env.SUPABASE_URL&&process.env.SUPABASE_SERVICE_ROLE_KEY),
  ia_gratis:()=>bool(process.env.IA_GRATIS_API_TOKEN),
  local:()=>true,
  disabled:()=>false,
};
...
export const TOOL_DEFINITIONS=Object.freeze([
  {
    id:'web.search',version:'2.1.0',provider:'web_intelligence',capabilities:['web_search','deep_research'],actions:['search'],
    ...
    tags:['web','research','multi-provider','direct-retrieval','structured-data','pdf','evidence-ranked','read'],
  },
  {
  ...
  {
    id:'ia_gratis.search',version:'1.0.0',provider:'ia_gratis',capabilities:['web_search'],actions:['ai_search'],
    riskLevel:'low',sideEffect:'read',approval:'none',tenantScope:'session',timeoutMs:22000,maxInputChars:12000,tokenCost:50,externalProcessing:true,
    input:{type:'object',required:['query'],properties:{query:{type:'string',maxLength:12000}}},
    output:{type:'object',properties:{reply:{type:'string'},data:{type:'object'}}},
    tags:['ia-gratis','web','search','external','read'],
  },
  {
    id:'ia_gratis.summarize',version:'1.0.0',provider:'ia_gratis',capabilities:['conversation_reasoning'],actions:['summarize'],
    riskLevel:'low',sideEffect:'none',approval:'none',tenantScope:'session',timeoutMs:22000,maxInputChars:21000,tokenCost:20,externalProcessing:true,
    input:{type:'object',required:['text'],properties:{text:{type:'string',maxLength:20000},style:{type:'string',maxLength:80}}},
    output:{type:'object',properties:{reply:{type:'string'},data:{type:'object'}}},
    tags:['ia-gratis','text','summary','external','compute'],
  },
  {
    id:'ia_gratis.translate',version:'1.0.0',provider:'ia_gratis',capabilities:['conversation_reasoning'],actions:['translate'],
    riskLevel:'low',sideEffect:'none',approval:'none',tenantScope:'session',timeoutMs:22000,maxInputChars:21000,tokenCost:20,externalProcessing:true,
    input:{type:'object',required:['text'],properties:{text:{type:'string',maxLength:20000},target:{type:'string',maxLength:40}}},
    output:{type:'object',properties:{reply:{type:'string'},data:{type:'object'}}},
    tags:['ia-gratis','text','translation','external','compute'],
  },
  {
    id:'ia_gratis.humanize',version:'1.0.0',provider:'ia_gratis',capabilities:['conversation_reasoning'],actions:['humanize'],
    riskLevel:'low',sideEffect:'none',approval:'none',tenantScope:'session',timeoutMs:22000,maxInputChars:20500,tokenCost:20,externalProcessing:true,
    input:{type:'object',required:['text'],properties:{text:{type:'string',maxLength:20000}}},
    output:{type:'object',properties:{reply:{type:'string'},data:{type:'object'}}},
    tags:['ia-gratis','text','rewrite','external','compute'],
  },
  {
    id:'ia_gratis.detect',version:'1.0.0',provider:'ia_gratis',capabilities:['conversation_reasoning'],actions:['detect_ai_text'],
    riskLevel:'low',sideEffect:'none',approval:'none',tenantScope:'session',timeoutMs:22000,maxInputChars:20500,tokenCost:20,externalProcessing:true,
    input:{type:'object',required:['text'],properties:{text:{type:'string',maxLength:20000}}},
    output:{type:'object',properties:{reply:{type:'string'},data:{type:'object'}}},
    tags:['ia-gratis','text','detection','external','compute'],
  },
  {
...
function publicDefinition(tool){
  ...
  return{
    ...
    tokenCost:Number(tool.tokenCost||0)||null,externalProcessing:tool.externalProcessing===true,
    configured:isConfigured,enabled:isConfigured&&tool.provider!=='disabled',
    contractHash:sha256(JSON.stringify({id:tool.id,version:tool.version,input:tool.input,output:tool.output,actions:tool.actions,sideEffect:tool.sideEffect,approval:tool.approval,tokenCost:Number(tool.tokenCost||0),externalProcessing:tool.externalProcessing===true})),
  };
}
...
export function toolFabricSnapshot(){
  ...
  return{
    ...
    policy:{failClosed:true,defaultDeny:true,mutationsEnabled:false,clientApprovalTrusted:false,toolOutputIsUntrusted:true,rawSecretsExposed:false,externalProcessingDeclared:true,perCallBudgetEnforced:true},
...
export function validateToolInput(toolId,payload={}){
  ...
  for(const [key,rule] of Object.entries(tool.input?.properties||{})){
    ...
    if(rule.type==='object'&&(typeof input[key]!=='object'||Array.isArray(input[key])))return{ok:false,error:'tool_input_type',field:key,expected:'object'};
