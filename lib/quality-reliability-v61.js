...
export function auditQualityReliability(payload={},context={}){
  ...
  const critical=String(ai.gate||'PASS')==='HOLD'||instruction.hardFailure;
  const score=clamp(instruction.score*.32+evidence*.32+relevance*.22+clarity*.14);
  const blockers=[];
  if(String(ai.gate||'')==='HOLD')blockers.push('answer_intelligence_hold');
  if(String(ai.gate||'')==='REPAIR')blockers.push('citation_repair_required');
  ...
  return{
    ...
    blockers,
    promotion_eligible:!critical&&score>=85&&['PASS','UNVERIFIED'].includes(String(ai.gate||'PASS')),
    claim_policy:{benchmark_required_for_competitor_superiority:true,universal_superiority_claim:false}
  };
}
