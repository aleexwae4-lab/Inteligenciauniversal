...
const STOP=new Set('para como porque esta este estos estas esto eso esa ese las los una uno unos unas que quien cual donde cuando desde hasta sobre entre pero mas muy del por con sin una cada puede puedo puedes tiene tienes tener soy eres ser fue son han hay aqui allí ahora luego despues antes user usuario asistente assistant respuesta mensaje quieres quiero'.split(/\s+/));

const cleanText=(value,max=12000)=>String(value??'').replace(/\u0000/g,'').trim().slice(0,max);
const fold=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
const keywords=value=>[...new Set((fold(value).match(/[a-z0-9]{4,}/g)||[]).filter(w=>!STOP.has(w)))].slice(0,28);
...
export function contextualCoherenceReportV103({message='',history=[],answer=''}={}){
  const rows=normalizeConversationHistoryV103(history,12);
  const contextual=contextualFollowupV103(message,rows);
  const lastAssistant=[...rows].reverse().find(x=>x.role==='assistant')?.text||'';
  if(!contextual||!lastAssistant)return{version:CONTEXT_INTEGRITY_V103,required:false,pass:true,overlap:1,anchors:[]};
  const anchors=keywords(lastAssistant);
  if(!anchors.length)return{version:CONTEXT_INTEGRITY_V103,required:true,pass:true,overlap:1,anchors:[]};
  const answerWords=new Set(keywords(answer));
  const matched=anchors.filter(w=>answerWords.has(w));
  const overlap=matched.length/Math.min(6,anchors.length);
  const pass=matched.length>=1||overlap>=.2;
  return{
    version:CONTEXT_INTEGRITY_V103,
    required:true,
    pass,
    ...
    reason:pass?null:'contextual_thread_drift'
  };
}
