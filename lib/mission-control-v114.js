...
export function planNativeMission(payload={},message='',history=[],attachments=[]) {
  ...
  return {version:MISSION_CONTROL_VERSION,plan,mode,toolIds,requiresLive,preserveContext:history.length>0,hasAttachments:attachments.length>0};
}
...
export async function executeNativeReadTools(mission,message,{timeoutMs=6500}={}) {
  ...
  const task=runTools({agent:{tools:[]},message,requestedTools:mission.toolIds});
  let timer;
  try{
...
export function attachMissionMetadata(result,mission,toolResults=[]) {
  ...
  const summary={version:MISSION_CONTROL_VERSION,plan:publicUniversalIntelligencePlan(mission.plan),
