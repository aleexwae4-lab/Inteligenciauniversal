...
export async function streamOpenAI({url,headers,model,input,signal,onProviderEvent}){
  ...
  await readSSE(url,{signal,method:'POST',headers,body:JSON.stringify({model,input,stream:true})},({data})=>{
    if(data?.type==='response.output_text.delta'&&typeof data.delta==='string'){
      firstToken(state,{provider:'openai',model},onProviderEvent);parts.push(data.delta);chunks++;
