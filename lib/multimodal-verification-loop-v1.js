...
function verificationTerms(s=''){
  return [...new Set(clean(s,900).toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').split(/[^a-z0-9]+/).filter(x=>x.length>=5))].slice(0,18);
}
function polarity(s=''){
  const x=' '+clean(s,900).toLowerCase()+' ';
  return /\\b(no|not|sin|nunca|never|false|falso|incorrecto|incorrect)\\b/.test(x)?'negative':'positive';
}

export function evaluateVerificationEvidence(report){
  if(!report)return report;
  const tasks=(report.tasks||[]).map(task=>{
    const sources=task?.result?.sources||[];
    if(!sources.length||!['evidence_found'].includes(task.status)) return task;
    const q=verificationTerms(task.statement);
    const scored=sources.map(source=>{
      const hay=verificationTerms((source.title||'')+' '+(source.snippet||''));
      const overlap=q.filter(x=>hay.includes(x));
      return {...source,termOverlap:overlap.length,matchedTerms:overlap.slice(0,8)};
    }).sort((a,b)=>(b.termOverlap||0)-(a.termOverlap||0));
    const best=scored[0]?.termOverlap||0;
    let assessment='insufficient';
    if(best>=4) assessment='corroborating_signal';
    else if(best>=2) assessment='weak_signal';
    const polarityConflict=scored.some(x=>x.termOverlap>=3&&polarity(x.snippet)!==polarity(task.statement));
    if(polarityConflict) assessment='contradictory_signal';
    return {...task,status:assessment,result:{...task.result,evaluation:{assessment,bestTermOverlap:best,sourceCount:scored.length,sources:scored.slice(0,4),method:'bounded_term_and_polarity_review',requiresHumanOrModelReview:true}}};
  });
  return {...report,tasks,summary:{...(report.summary||{}),corroboratingSignals:tasks.filter(x=>x.status==='corroborating_signal').length,contradictorySignals:tasks.filter(x=>x.status==='contradictory_signal').length,weakSignals:tasks.filter(x=>x.status==='weak_signal').length,insufficient:tasks.filter(x=>x.status==='insufficient').length},policy:{...(report.policy||{}),evidenceEvaluationActive:true,signalsAreNotTruth:true,noAutomaticResolution:true}};
}
