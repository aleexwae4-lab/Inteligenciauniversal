...
export const STATELESS_INTELLIGENCE_V84='stateless-intelligence/v84';
...
function systemFor(mode='general'){
  const profile=mode==='code'?'Para código, entrega una solución coherente y ejecutable, con supuestos, pruebas y fallos previsibles.':mode==='analysis'?'Para análisis, abre con la conclusión y después evidencia, supuestos, riesgos, trade-offs y acciones concretas.':mode==='design'?'Para diseño, resuelve como producto de producción: flujo, estados, jerarquía, accesibilidad y criterios de calidad.':'Adapta la profundidad a la dificultad y contesta primero lo que el usuario preguntó.';
...
export async function runStatelessNativeRecovery({body={}}={}){
  ...
  const controller=new AbortController(),timeoutMs=Math.max(6000,Math.min(24000,Number(process.env.WAE_V84_STATELESS_TIMEOUT_MS||16000))),timer=setTimeout(()=>controller.abort(),timeoutMs),started=Date.now();
  try{
    ...
    const latencyMs=Date.now()-started,responseObject=buildAssistantResponse({content:text,sources:[],provider:'universal_core',model:'universal-core-native-v84',latencyMs,memoryCount:0,requestId:null,messageId:null,conversationId:null,webUsed:false,degraded:false});
    return {success:true,reply:text,response:responseObject,speech_text:responseObject.speechText,components:responseObject.components,actions:responseObject.actions,web_sources:[],provider:'universal_core',model:'universal-core-native-v84',latency_ms:latencyMs,response_schema:'assistant-response/v1',degraded:false,quality,recovery:{active:true,path:STATELESS_INTELLIGENCE_V84,persistence:'bypassed_for_this_response',memory_used:false}};
