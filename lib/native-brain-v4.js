...
async function stableReferenceFallback(message=''){
  ...
  try{
    ...
    return {text:`**${title}**\n\n${extract}`,provider:'wikimedia_reference',model:'stable-reference-v1',path:'stable-reference-race',degraded:true,sources:[{title:`Wikipedia — ${title}`,url:`https://es.wikipedia.org/wiki/${encodeURIComponent(String(title).replace(/ /g,'_'))}`,source:'Wikipedia',retrievedAt:new Date().toISOString()}],failures:[]};
...
async function stableKnowledgeRace({payload,message,history,userKey,intent}){
  const body={...payload,message,mode:intent.mode};
  const system=systemFor({intent,memory:'',attachments:''});
  const knowledge=candidateOrReject(
    bounded(async()=>{
      ...
      return meaningful(text)?{text,provider:result?.provider||'universal_core',model:result?.model||'knowledge-fabric',path:'knowledge-fabric-race',degraded:result?.degraded===true,sources,failures:[]}:null;
    },3400,'NATIVE_STABLE_KNOWLEDGE'),
    'knowledge'
  );
  const reference=candidateOrReject(stableReferenceFallback(message),'reference');
  const generation=candidateOrReject(
    bounded(async()=>{
      const result=await generateWithFallback({provider:'auto',system,message,history});
      const text=clean(result?.text);
      return meaningful(text)?{text,provider:result?.provider||'native-inference',model:result?.model||'unknown',path:'inference-fabric-auto-race',degraded:result?.degraded===true,sources:Array.isArray(result?.webSources)?result.webSources:[],failures:Array.isArray(result?.failures)?result.failures:[]}:null;
    },5200,'NATIVE_STABLE_GENERATION'),
    'generation'
  );
  try{return await Promise.any([knowledge,reference,generation])}catch{return null}
}

export function nativeBrainStatus(){
  const providers=providerRegistry();
  return {version:NATIVE_BRAIN_VERSION,ready:true,architecture:'native-resilient-knowledge-race-orchestrator',primaryInference:'auto',providers:providers.map(p=>({id:p.id,configured:p.configured,model:p.model})),emergencyProviders:emergencyProviderRegistry().map(p=>({id:p.id,model:p.model})),memory:memoryStatus(),lanes:['local-kernel','stable-knowledge-race','memory-context','knowledge-fabric','inference-fabric-auto','stable-reference','emergency-inference','verified-rescue'],universalRouting:true,responseSchema:'assistant-response/v1'};
}
...
export async function nativeBrainReply(payload={}){
  ...
  // Stable factual questions race independent knowledge lanes immediately. They must never
  // wait serially for one remote provider before another route is even attempted.
  if(intent.factual&&!intent.requiresEvidence&&!attachments.length){
    const raced=await stableKnowledgeRace({payload,message,history,userKey,intent});
    if(raced){
      const out=envelope({payload,history,text:raced.text,provider:raced.provider,model:raced.model,path:raced.path,started,degraded:raced.degraded===true,sources:raced.sources||[],failures:raced.failures||[],intent,memoryCount:0});
      persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
    }
  }

  const memory=await bounded(()=>recallMemory(userKey,message,8,sessionId,conversationId),2200,'NATIVE_MEMORY').catch(()=>[]);
  const memoryContext=formatMemoryContext(memory);

  const shouldKnowledge=intent.requiresEvidence||shouldUseKnowledgeAnswer({...payload,message,mode:intent.mode});
  if(shouldKnowledge){
    try{
      const researched=await bounded(()=>runKnowledgeAnswer({body:{...payload,message,mode:intent.mode},userKey}),5200,'NATIVE_KNOWLEDGE');
  ...
  try{
    ...
    const deadline=['analysis','code','design','executive','research'].includes(intent.mode)?12000:6500;
    ...
    if(meaningful(generated?.text)&&(!intent.requiresEvidence||generatedSources.length>0)){
      const out=envelope({payload,history,text:clean(generated.text),provider:generated.provider||'native-inference',model:generated.model||'unknown',path:'inference-fabric-auto',started,degraded:generated.degraded===true,sources:generatedSources,failures:Array.isArray(generated.failures)?generated.failures:[],intent,memoryCount:memory.length});
      persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
    }
    primaryFailure=Object.assign(new Error(intent.requiresEvidence?'auto_primary_missing_evidence':'auto_primary_empty'),{failures:generated?.failures||[]});
  }catch(error){primaryFailure=error}

  if(intent.factual&&!intent.requiresEvidence&&!attachments.length){
    const reference=await stableReferenceFallback(message);
    if(reference){
      const out=envelope({payload,history,text:reference.text,provider:reference.provider,model:reference.model,path:'stable-reference',started,degraded:true,sources:reference.sources,failures:primaryFailure?.failures||[],intent,memoryCount:memory.length});
      persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
    }
  }

  try{
    const emergency=await bounded(()=>emergencyGenerate({body:{...payload,message,history},userKey,failure:primaryFailure}),4200,'NATIVE_EMERGENCY');
    ...
    if(meaningful(text)&&(!intent.requiresEvidence||emergencySources.length>0)){
      const out=envelope({payload,history,text,provider:emergency.provider||'emergency',model:emergency.model||'emergency',path:'emergency-inference',started,degraded:true,sources:emergencySources,failures:primaryFailure?.failures||[],intent,memoryCount:memory.length});
      persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
    }
  }catch{}

  try{
    const rescued=await bounded(()=>rescueMission({payload:{...payload,message,mode:intent.mode},userKey,error:primaryFailure||{code:'NATIVE_V4_RESCUE'},allowResearch:intent.requiresEvidence||intent.factual}),4800,'NATIVE_RESCUE');
    ...
    if(meaningful(text)&&(!intent.requiresEvidence||rescuedSources.length>0)){
      const out=envelope({payload,history,text,provider:rescued.provider||'universal_core',model:rescued.model||'rescue',path:'verified-rescue',started,degraded:true,sources:rescuedSources,failures:primaryFailure?.failures||[],intent,memoryCount:memory.length});
      persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
    }
  }catch{}
