...
function wireV11(js){
  ...
  const patches=[
    "const logicEventTargetV10=logicEventTarget;",
    "logicEventTarget=function(rule){if(['npc_defeated'].includes(rule.event.type))return rule.event.npcId||'';if(['item_collected','inventory_changed','item_used'].includes(rule.event.type))return rule.event.itemId||'';return logicEventTargetV10(rule)};",
    ...
    "const applyLogicRuleV10=applyLogicRule;",
    "applyLogicRule=function(){applyLogicRuleV10();const rule=logicRule();if(!rule)return;const target=editorEl('logicEventTarget').value.trim();if(rule.event.type==='npc_defeated')rule.event.npcId=target.slice(0,60);if(['item_collected','inventory_changed','item_used'].includes(rule.event.type))rule.event.itemId=target.slice(0,60);renderLogicBuilder()};"
  ].join('');
  js=js.replace(
