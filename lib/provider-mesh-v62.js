...
function latencyScore(ms){if(!finite(ms))return55;return clamp(100-(Math.max(0,Number(ms))/120))}
function ttftScore(ms){if(!finite(ms))return55;return clamp(100-(Math.max(0,Number(ms))/30))}
function costScore(microunits){if(!finite(microunits))return60;return clamp(100-(Math.max(0,Number(microunits))/150))}
...
function scoreProvider(provider,index,task,now){
  ...
  return{
    ...
    ewmaCostMicrounits:finite(s.ewmaCostMicrounits)?Math.round(s.ewmaCostMicrounits):null,attempts:s.attempts,qualitySamples:s.qualitySamples,
    confidence:Number(confidenceScore(s).toFixed(1)),consecutiveFailures:s.consecutiveFailures,lastErrorClass:s.lastErrorClass,registryIndex:index
  };
}
...
export function selectProviderRoute({message='',mode='general',attachments=[],requestedProvider='auto',registry=providerRegistry(),now=Date.now()}={}){
  ...
  return{
    ...
    candidates:candidates.slice(0,5).map(({id,score,circuitOpen,reliability,quality,evidence,instruction,ewmaLatencyMs,ewmaTtftMs,ewmaCostMicrounits,attempts,qualitySamples,confidence,consecutiveFailures,lastErrorClass})=>({id,score,circuitOpen,reliability,quality,evidence,instruction,ewmaLatencyMs,ewmaTtftMs,ewmaCostMicrounits,attempts,qualitySamples,confidence,consecutiveFailures,lastErrorClass})),
...
function resultTelemetry(result={}){
  const meta=result?.response?.metadata||{},quality=result?.quality_reliability||meta?.qualityReliability||{};
  return{
    latencyMs:result?.latencyMs??result?.latency_ms??meta?.latencyMs??null,
    ttftMs:result?.ttft_ms??result?.ttftMs??meta?.providerTtft??meta?.ttftMs??null,
    costMicrounits:result?.cost_microunits??meta?.costMicrounits??null,
    quality
  };
}
...
export function providerMeshSnapshot({registry=providerRegistry(),now=Date.now()}={}){
  return{
    contract:PERFORMANCE_ROUTER_CONTRACT,version:PERFORMANCE_ROUTER_VERSION,learning:'operational_runtime_quality_feedback',memoryScope:'process_local',persistent:false,baseModelWeightsChanged:false,
    scoringDimensions:['quality','instruction','evidence','reliability','latency','ttft','cost','prior'],
    providers:configuredRegistry(registry).map((provider,index)=>{const row=scoreProvider(provider,index,{path:'STANDARD'},now);return{...row,circuitState:row.circuitOpen?'OPEN':'CLOSED'}})
  };
}

export function performanceRouterCapabilities(){
  return{version:PERFORMANCE_ROUTER_VERSION,contract:PERFORMANCE_ROUTER_CONTRACT,adaptive:true,qualityAware:true,evidenceAware:true,ttftAware:true,costAware:true,circuitBreaker:true,learningScope:'process_local',persistentLearning:false,baseModelTraining:false};
}
