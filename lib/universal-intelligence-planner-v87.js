...
const SCIENCE_RX=/\b(cientific|ciencia|medic|salud|clinic|farmacol|biolog|genetic|fisic|quimic|matematic|estadistic|paper|papers|estudio|estudios|evidencia|meta analisis|metaanalisis|revision sistematica|pubmed|openalex|crossref|arxiv|doi|teorema|physics|chemistry|biology|medicine|scientific|academic)\b/i;
const ACADEMIC_RX=/\b(investiga|investigacion|academ|bibliografia|fuentes|evidencia|paper|papers|articulo|publicacion|literatura cientifica|estado del arte|review|research)\b/i;
const REASONING_RX=/\b(razona|razonamiento|analiza|analisis|explica por que|demuestra|deduce|infiere|compara|contrasta|evalua|diagnostica|causa|trade off|tradeoff|escenario|hipotesis|arquitectura|estrategia|decision|optimiza|planifica|resuelve)\b/i;
const EXEC_ACTION_RX=/\b(analiza|audita|compara|estrategia|plan|planifica|decide|decision|disena|optimiza|escala|monetiza|diagnostica|arquitectura|evalua|recomienda|construye|implementa)\b/i;
const EXPLICIT_MULTIAGENT_RX=/\b(multiagente|multiagentes|multi agent|multiagent|comite|consejo|war room|mesa de guerra|especialistas|agentes especializados)\b/i;
const CODE_RX=/\b(codigo|programa|software|github|api|backend|frontend|bug|debug|typescript|javascript|python|sql|deploy|render|supabase|vercel|arquitectura de software|devops|sre)\b/i;
const DESIGN_RX=/\b(diseno|ux|ui|interfaz|experiencia de usuario|flujo|pantalla|responsive|movil|branding|producto visual)\b/i;

const EXEC_DOMAINS=[
  ['technology',/\b(tecnologia|software|ia|inteligencia artificial|arquitectura|datos|backend|frontend|api|cloud|devops|ciberseguridad)\b/i],
  ['finance',/\b(finanzas|financiero|costo|coste|presupuesto|inversion|roi|ingresos|margen|ebitda|flujo de caja|valuacion)\b/i],
  ['product',/\b(producto|usuario|ux|ui|roadmap|adopcion|retencion|experiencia)\b/i],
  ['operations',/\b(operacion|operaciones|proceso|procesos|escala|capacidad|sla|automatizacion|productividad)\b/i],
  ['risk_legal',/\b(seguridad|riesgo|legal|juridic|ley|contrato|cumplimiento|compliance|privacidad|auditoria)\b/i],
  ['growth',/\b(marketing|marca|ventas|crecimiento|mercado|adquisicion|conversion|crm|clientes)\b/i],
  ['organization',/\b(empresa|negocio|organizacion|universidad|estrategia|direccion|equipo|recursos humanos|talento)\b/i]
];
...
function isBookIntent(message='',mode='general'){
  ...
  return bibliographicLookupIntent(q)||libraryRelevant(q,mode)||/\b(isbn|libro|libros|book|books|novela|obra literaria|autor|autora|biblioteca|open library|gutenberg)\b/i.test(q);
}
