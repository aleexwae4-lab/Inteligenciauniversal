...
export const GPU_CONTROL_PLANE_VERSION='wae-gpu-control-plane/v80';
export const GPU_SCHEDULER_VERSION='wae-gpu-scheduler/v80';
...
function hashKey(){return String(process.env.WAE_GPU_SCOPE_HASH_SECRET||process.env.SUPABASE_SERVICE_ROLE_KEY||'wae-gpu-v80-domain-separator')}
...
async function loadSnapshot(payload={}){
  ...
  const response=await callInternalSupabaseRpc({functionName:'wae_gpu_metrics_snapshot_v79',body:{p_scope_hash:scope},timeoutMs:intEnv('WAE_GPU_METRICS_READ_TIMEOUT_MS',700,250,2500),clientInfo:'wae-gpu-control-plane-v80'});
