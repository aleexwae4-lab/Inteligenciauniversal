...
const SOURCE_RX=/\b(fuentes?|citas?|referencias?|links?|enlaces?|investiga|investigacion|verifica)\b|\b(?:con|usa|aporta|incluye)\s+evidencia\b|\bevidencia\s+(?:web|externa|verificable|actual|de fuentes?)\b/i;
...
export function deriveRequirementContract(question='',mode='general'){
  ...
  const highRisk=HIGH_RISK_RX.test(q);
  const requiresSources=String(mode||'general')==='research'||CURRENT_RX.test(q)||SOURCE_RX.test(q)||highRisk;
...
export function evaluateRequirementCoverage({contract,answer='',sources=[]}={}){
  ...
  add('sources',c.requiresSources,hasSources,true,{sourceCount:Array.isArray(sources)?sources.length:0,highRisk:c.highRisk===true});
...
export function verificationInstruction(contract={}){
  ...
  if(contract.requiresSources)requirements.push(contract.highRisk?'usa evidencia real disponible; en temas de alto riesgo no adivines':'usa evidencia/fuentes reales disponibles');
