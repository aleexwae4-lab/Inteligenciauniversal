...
export function deriveRequirementContract(question='',mode='general'){
  ...
  // Local document preservation is evidence handling, not a request for external citations.
  const localArchive=/\b(conserva|guarda|archiva|preserva)\b.{0,70}\b(evidencia|archivo|documento|datos)\b/i.test(q)
    && !/\b(hoy|ahora|actual|reciente|noticias|web|internet|busca|investiga|verifica)\b/i.test(q);
  const requiresSources=String(mode||'general')==='research'||CURRENT_RX.test(q)||(SOURCE_RX.test(q)&&!localArchive);
