...
export function boundedWaewebStream(upstream,controller,timer) {
  ...
  output=new ReadableStream({
    async pull(target){
      ...
      try{
        ...
        if(ended)return;
        if(next.done){finish();target.close();return;}
  ...
  return {stream:output,stop:()=>{
    ...
    // A timed-out SSE must not look like a successful completed stream.
    try{output.cancel("waeweb_stream_timeout").catch(()=>{});}catch{}
  }};
}
