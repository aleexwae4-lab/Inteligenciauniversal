...
import { rankFallbackOrderV96, tailLatencyRiskV96, tailLatencyRouterCapabilitiesV96 } from './tail-latency-router-v96.js';
...
export function selectProviderRoute(args={}){
  ...
  if(!base.applied){
    return{...base,contract:PERFORMANCE_ROUTER_CONTRACT,version:PERFORMANCE_ROUTER_VERSION,persistentReputation:{available:persistent.available,capability},policy:{...(base.policy||{}),persistentLearning:true,multiInstanceConsistent:true,tailLatencyGuard:true}};
  }
  const now=Number(args.now||Date.now());
  const path=String(base?.task?.path||'STANDARD').toUpperCase();
  const candidates=(Array.isArray(base.candidates)?base.candidates:[]).map(row=>{
    ...
    const tail=tailLatencyRiskV96({path,p95LatencyMs:p?.capability?.metadata?.p95_latency_ms,confidence:p.confidence});
    const score=p.persistentCircuitOpen?-999:Number((Number(row.score||0)+p.adjustment-tail.penalty).toFixed(3));
    return{...row,model,baseScore:row.score,score,persistentAdjustment:Number(p.adjustment.toFixed(3)),persistentConfidence:p.confidence,persistentCircuitOpen:p.persistentCircuitOpen,persistentTailP95Ms:tail.p95LatencyMs,interactiveTailRisk:tail.risk,tailLatencyPenalty:tail.penalty};
  ...
  const rankedFallback=rankFallbackOrderV96({selected:selectedProvider,candidates,baseFallbackOrder:base.fallbackOrder,path});
  return{
    ...base,
    ...
    candidates,
    fallbackOrder:rankedFallback.order,
    persistentReputation:{available:persistent.available,loadedAt:persistent.loadedAt||null,capability},
    tailLatency:{version:rankedFallback.version,path,rankedFallback:true},
    policy:{...(base.policy||{}),persistentLearning:true,processLocalLearning:true,multiInstanceConsistent:true,crossDeployMemory:true,tailLatencyGuard:true,scoreRankedFallback:true,hardTailBlock:false}
  };
}
...
export function providerMeshSnapshot(options={}){
  ...
  return{...base,contract:PERFORMANCE_ROUTER_CONTRACT,version:PERFORMANCE_ROUTER_VERSION,memoryScope:'process_local_plus_persistent_control_plane',persistent:true,persistentAvailable:persistent.available,persistentLoadedAt:persistent.loadedAt||null,pendingPersistence:pending.length,tailLatency:tailLatencyRouterCapabilitiesV96(),baseModelWeightsChanged:false};
}

export function performanceRouterCapabilities(){return{version:PERFORMANCE_ROUTER_VERSION,contract:PERFORMANCE_ROUTER_CONTRACT,adaptive:true,qualityAware:true,evidenceAware:true,ttftAware:true,costAware:true,circuitBreaker:true,tailLatencyAware:true,scoreRankedFallback:true,learningScope:'operational_persistent_plus_process_local',persistentLearning:true,multiInstanceConsistent:true,crossDeployMemory:true,baseModelTraining:false}}
