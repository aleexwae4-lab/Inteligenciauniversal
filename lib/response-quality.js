...
import {isHeadcountQuestion} from './topic-relevance-v121.js';
...
export function needsWebResearch(message = '', mode = 'general') {
  ...
  if (isHeadcountQuestion(message)) return true;
...
export const QUALITY_GUIDANCE = [
  ...
  'Prioriza precision y utilidad. Da primero una respuesta sustantiva o entregable; no conviertas una falta de resultados web en un discurso sobre ausencia de evidencia.',
  'Elige longitud y formato por valor informativo: dato puntual -> respuesta directa; trabajo complejo -> desarrollo suficiente. Usa titulos, negritas, tablas o codigo solo cuando ayuden a comprender, comparar, ejecutar o verificar. Nunca rellenes por apariencia premium.',
  ...
  'En temas actuales o de alto impacto, distingue conocimiento general de cifras actuales. Usa fuentes pertinentes cuando las recuperes; si no hay desglose verificable, no inventes cifras y comunica concisamente la incertidumbre concreta sin exponer errores de búsqueda.',
  ...
  'Formato visual opcional: cuando una tarjeta realmente aclare un producto, resultado o plan, puedes añadir un bloque ```wae-card con JSON válido con title, description, badge y actions (type copy, workspace o ask). Para un gráfico de barras con cifras relevantes, ```wae-chart con JSON válido: title, data de label y value numérico, y basis user solo cuando todos los valores proceden del usuario; en otro caso basis demo visible. Cierra el bloque con ```; no uses elementos visuales por defecto. Nunca inventes métricas, resultados, estados o acciones ejecutadas, y no presentes valores de demostración como hechos.',
].join(' ');
