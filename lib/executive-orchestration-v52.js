...
export async function runExecutiveOrchestration({body={},userKey='anonymous',sessionId=''}={}){
  ...
  const [manifest,library]=await Promise.all([getExecutiveManifest(),retrieveLibraryIntelligence({message,mode:'executive',force:body.library===true,limit:6})]);if(!manifest)return null;
  ...
  const runs=await Promise.allSettled(plan.specialists.map(async agent=>{const t=Date.now();const generated=await generateOne({provider,history,system:`Eres el especialista ${agent.role} del comité ejecutivo de Universal Core. Responde con criterio senior y sin revelar razonamiento interno.`,message:specialistPrompt(agent,message,library.context)});return{role:agent.role,name:agent.name,reply:generated.reply,latencyMs:Date.now()-t}}));
  const specialists=runs.map((run,index)=>run.status==='fulfilled'?run.value:{role:plan.specialists[index].role,name:plan.specialists[index].name,error:String(run.reason?.message||run.reason||'specialist_failed').slice(0,220)});const usable=specialists.filter(x=>x.reply);if(!usable.length)return null;
  let finalText='',synthesisOk=false;if(usable.length>=2){try{const generated=await generateOne({provider,history,system:'Eres Universal Core. Sintetiza las aportaciones del comité ejecutivo en una respuesta moderna, precisa, accionable y calibrada. No reveles deliberación interna.',message:synthesisPrompt(message,usable,library.context)});finalText=generated.reply;synthesisOk=!!finalText}catch{}}
  if(!finalText)finalText=usable[0].reply;const quality=evaluateAnswer({question:message,answer:finalText,mode:'executive',sources:[]});const latencyMs=Date.now()-started;const response=buildAssistantResponse({content:finalText,sources:[],provider:'universal_core',model:'db-executive-orchestrator-v52',latencyMs,memoryCount:0,requestId:crypto.randomUUID(),webUsed:false,degraded:usable.length<2});response.metadata={...response.metadata,executiveOrchestration:true,orchestrationVersion:EXECUTIVE_ORCHESTRATION_VERSION,quality};
