...
export async function runExecutiveOrchestration({body={},userKey='anonymous',sessionId=''}={}){
  ...
  const runs=await Promise.allSettled(plan.specialists.map(async agent=>{
    ...
    return{role:agent.role,name:agent.name,reply:generated.reply,latencyMs:Date.now()-t,evidenceCount:Number(route.evidenceCount||0)};
  }));
  ...
  response.metadata={...response.metadata,executiveOrchestration:true,orchestrationVersion:EXECUTIVE_ORCHESTRATION_VERSION,evidenceRouter:EVIDENCE_ROUTER_VERSION,contextTrustPlane:CONTEXT_TRUST_PLANE_VERSION,quality};
  return{
