...
export const CANVAS_ENGINE_VERSION = 'universal-render-canvas-factory/v1';
...
const OUTPUT_RULES = [
  ...
  'Si el usuario no proporciona montos, no inventes precios, objetivos monetarios ni deudas (por ejemplo «Falta $5000»). Para datos sintéticos marca explícitamente DEMOSTRACIÓN.',
...
export function auditCanvas(html, kind = 'landing', request = '') {
  ...
  const unrequestedMoney = !/\$\s*\d|\b\d[\d.,]*\s*(?:MXN|USD|EUR)\b/i.test(String(request || '')) && /\$\s*\d|\b\d[\d.,]*\s*(?:MXN|USD|EUR)\b/i.test(source) && !/\b(?:demostraci[oó]n|datos simulados|datos ficticios)\b/i.test(source);
  const checks = {
    noUnrequestedMoney: !unrequestedMoney,
...
export async function createPremiumCanvas({ request, kind, brand = '', baseHtml = '', provider = 'auto', generate = generateWithFallback }) {
  ...
  const builder = await generate({
    ...
    maxTokens: Number(process.env.WAE_CANVAS_MAX_OUTPUT_TOKENS || 6400), mode: 'code', webEnabled: false
  });
  let html = hardenCanvasHtml(builder.text);
  let audit = auditCanvas(html, category, goal);
  let repaired = false;
  if (!audit.pass) {
    ...
    audit = auditCanvas(html, category, goal);
    repaired = true;
  }
