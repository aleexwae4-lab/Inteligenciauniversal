...
export function executionPlaneSnapshot(){
  ...
  return{
    ...
    policy:{failClosed:true,mutationsEnabled:MUTATIONS_ENABLED,humanApprovalForWrites:true,clientApprovalTrusted:false,rawSecretsInReceipts:false},
...
export async function executeCapability({capability,action,task,message,userKey,sessionId,approved=false,executors=DEFAULT_EXECUTORS,persistReceipt=persistExecutionReceipt}={}){
  ...
  if(!normalizedTask)return blockExecution({capability,adapter:'none',action:action||'none',riskLevel:'low',sideEffect:'none',requestDescriptor,errorCode:'task_required',started,userKey,sessionId,persistReceipt});
  if(!resolution)return blockExecution({capability,requestDescriptor,errorCode:'capability_not_executable',started,userKey,sessionId,persistReceipt,metadata:{failClosed:true}});
  ...
  if(mutative&&!MUTATIONS_ENABLED){
    return blockExecution({capability,adapter:adapter.id,action:resolvedAction,riskLevel:adapter.riskLevel,sideEffect:adapter.sideEffect,approvalRequired:true,approved:false,requestDescriptor,errorCode:'mutations_disabled',started,userKey,sessionId,persistReceipt,metadata:{failClosed:true}});
  }
  if(requiresApproval&&!approved){
    return blockExecution({capability,adapter:adapter.id,action:resolvedAction,riskLevel:adapter.riskLevel,sideEffect:adapter.sideEffect,approvalRequired:true,approved:false,requestDescriptor,errorCode:'approval_required',started,userKey,sessionId,persistReceipt});
  }
  if(!adapter.configured()){
    return blockExecution({capability,adapter:adapter.id,action:resolvedAction,riskLevel:adapter.riskLevel,sideEffect:adapter.sideEffect,approvalRequired:requiresApproval,approved:Boolean(approved),requestDescriptor,errorCode:'runtime_unconfigured',started,userKey,sessionId,persistReceipt});
  }

  const executor=executors[adapter.id];
  if(typeof executor!=='function')return blockExecution({capability,adapter:adapter.id,action:resolvedAction,riskLevel:adapter.riskLevel,sideEffect:adapter.sideEffect,requestDescriptor,errorCode:'executor_missing',started,userKey,sessionId,persistReceipt});
