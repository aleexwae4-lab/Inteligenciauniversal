...
import { inspectGithubRepository } from './github-intelligence-fabric.js';
import { iaGratisTools, IA_GRATIS_TOOL_FABRIC_VERSION } from './ia-gratis-v84.js';
...
export const EXECUTION_PLANE_VERSION='universal-execution-plane/v65-github-intelligence';
export const EXECUTION_RECEIPT_SCHEMA='universal-execution-receipt/v1';
export const EXECUTION_AUDIT_LEDGER_VERSION='execution-audit-ledger/v75';
...
export function executionReceiptPersistenceState(){
  const base=Boolean(process.env.SUPABASE_URL);
  const serviceRole=Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY);
  const tokenBridge=Boolean(process.env.SUPABASE_PUBLISHABLE_KEY&&process.env.WAE_RUNTIME_BRIDGE_TOKEN);
  const mode=base&&serviceRole?'service_role':base&&tokenBridge?'token_guarded_rls':'unconfigured';
  return{
    version:EXECUTION_AUDIT_LEDGER_VERSION,
    configured:mode!=='unconfigured',
    mode,
    durable:mode!=='unconfigured',
    writeOnlyFallback:mode==='token_guarded_rls',
    rawContentStored:false,
    identityHashesOnly:true,
  };
}
...
const TOOL_ADAPTER_IDS=Object.freeze({
  ...
  'github.repository_evaluate':'github_repository_evaluate_read',
  ...
  'ia_gratis.search':'ia_gratis_search_read',
  'ia_gratis.summarize':'ia_gratis_summarize_compute',
  'ia_gratis.translate':'ia_gratis_translate_compute',
  'ia_gratis.humanize':'ia_gratis_humanize_compute',
  'ia_gratis.detect':'ia_gratis_detect_compute',
});
...
export function executionPlaneSnapshot(){
  ...
  return{
    ...
    auditLedger:executionReceiptPersistenceState(),
    toolFabric:TOOL_FABRIC_VERSION,
    extensions:{iaGratis:IA_GRATIS_TOOL_FABRIC_VERSION},
    policy:{failClosed:true,mutationsEnabled:MUTATIONS_ENABLED,humanApprovalForWrites:true,clientApprovalTrusted:false,rawSecretsInReceipts:false,toolOutputIsUntrusted:true,externalProviderBudgetGuard:true},
...
async function persistExecutionReceipt(receipt){
  ...
  if(!state.configured)return{persisted:false,reason:'ledger_unconfigured',mode:state.mode,version:state.version};
  const base=String(process.env.SUPABASE_URL).replace(/\/$/,'');
  const row={...receipt};delete row.schema;
  const headers={'Content-Type':'application/json',Prefer:'return=minimal'};
  if(state.mode==='service_role'){
    const key=String(process.env.SUPABASE_SERVICE_ROLE_KEY||'');
    headers.apikey=key;
    headers.Authorization=`Bearer ${key}`;
  }else{
    headers.apikey=String(process.env.SUPABASE_PUBLISHABLE_KEY||'');
    headers['x-wae-runtime-token']=String(process.env.WAE_RUNTIME_BRIDGE_TOKEN||'');
  }
  try{
    ...
    return response.ok
      ?{persisted:true,mode:state.mode,version:state.version}
      :{persisted:false,reason:`ledger_http_${response.status}`,mode:state.mode,version:state.version};
  }catch(error){return{persisted:false,reason:text(error?.message||'ledger_transport_failure',180),mode:state.mode,version:state.version}}
}
...
function buildToolInput(toolId,task,input={}){
  ...
  if(toolId==='github.repository_evaluate')return{repository:text(supplied.repository,300),strategicNeed:text(supplied.strategicNeed||'',1200)};
  ...
  if(toolId==='text.inspect')return{content:String(supplied.content??'').slice(0,120000),name:text(supplied.name||'document.txt',240)};
  if(toolId==='data.csv_profile')return{content:String(supplied.content??'').slice(0,200000),delimiter:String(supplied.delimiter||',').slice(0,1)||','};
  if(toolId==='ia_gratis.search')return{query:text(supplied.query||task,12000)};
  if(toolId==='ia_gratis.summarize')return{text:text(supplied.text||task,20000),style:text(supplied.style||'concise',80)};
  if(toolId==='ia_gratis.translate')return{text:text(supplied.text||task,20000),target:text(supplied.target||'es',40)};
  if(toolId==='ia_gratis.humanize')return{text:text(supplied.text||task,20000)};
  if(toolId==='ia_gratis.detect')return{text:text(supplied.text||task,20000)};
  return{supplied};
}
...
const DEFAULT_EXECUTORS={
  ...
  github_repository_evaluate_read:async({input})=>inspectGithubRepository(input),
  ...
  ia_gratis_search_read:async({input})=>iaGratisTools.search(input),
  ia_gratis_summarize_compute:async({input})=>iaGratisTools.summarize(input),
  ia_gratis_translate_compute:async({input})=>iaGratisTools.translate(input),
  ia_gratis_humanize_compute:async({input})=>iaGratisTools.humanize(input),
  ia_gratis_detect_compute:async({input})=>iaGratisTools.detect(input),
};
...
export async function executeCapability({capability,action,task,message,input={},userKey,sessionId,approved=false,executors=DEFAULT_EXECUTORS,persistReceipt=persistExecutionReceipt}={}){
  ...
  if(!validation.ok){
    const errorCode=validation.error==='tool_input_required'&&['query','task'].includes(validation.field)?'task_required':validation.error;
