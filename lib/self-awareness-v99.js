...
import { providerConfiguration } from './web/providers-v1.js';
...
const WEB_CAPABILITY_RX=/\b(?:tienes?|cuentas? con|dispones? de|puedes|acceso|hay|existe|funciona|estado de|capacidad de)\b.{0,90}\b(?:navegador|buscador|internet|web|busqueda en tiempo real|busqueda online)\b|\b(?:navegador|buscador|internet|web)\b.{0,70}\b(?:tiempo real|actual|conectad|disponib|funciona)\b/i;
const WEB_TASK_RX=/\b(busca|investiga|consulta|abre|visita|recupera|encuentra|muestrame|verifica|resume|compara|analiza)\b|\b(?:puedes|podrias)\s+(?:buscar|consultar|investigar|abrir|visitar|navegar)\b.{0,100}\b(?:sobre|de|que|hoy|actual|ultima|noticias|github|facebook|youtube)\b/i;
...
export function classifySelfAwarenessV99(body={}){
  ...
  if(query.length<=175&&!WEB_TASK_RX.test(query)&&WEB_CAPABILITY_RX.test(query))return{eligible:true,kind:'web',query};
...
export function selfAwarenessSnapshotV99(stats={}){
  ...
  const webProviders=providerConfiguration().providers;
  const generalWebSearchConfigured=webProviders.some(p=>p.configured&&['tavily','brave','google_custom_search'].includes(p.id));
  ...
  return{
    ...
    capabilities:{
      reasoning:true,
      liveResearch:generalWebSearchConfigured,
      instantWebAnswersAvailable:true,
    ...
    webResearch:{generalSearchConfigured:generalWebSearchConfigured,instantAnswersAvailable:true,interactiveGraphicalBrowser:false,searchEndpoint:'/api/web/search',healthEndpoint:'/api/web/health',configurationIsNotHealthCheck:true},
    adaptiveCopilot:{
...
function capabilityLines(snapshot={}){
  ...
  return [
    ...
    snapshot.webResearch?.generalSearchConfigured ? '**Investigación actual:** búsqueda web general configurada y recuperación de evidencia (disponibilidad verificable por consulta).' : '**Investigación actual:** la capa web existe; por ahora solo hay respuestas instantáneas limitadas hasta habilitar un buscador general.',
...
export function buildSelfAwarenessReplyV99({kind='capability',stats={},query=''}={}){
  const snapshot=selfAwarenessSnapshotV99(stats);
  if(kind==='web'){
    const web=snapshot.webResearch;
    if(web.generalSearchConfigured){
      return 'Sí, Universal Core **tiene integrada una capa de investigación web**: puede buscar fuentes actuales y recuperar páginas para sustentar respuestas. No es un navegador gráfico como Chrome: es búsqueda y lectura web desde el backend. El motor de búsqueda general está configurado, pero su disponibilidad se confirma ejecutando una consulta; no debo confundir configuración con una búsqueda exitosa. Para consultar el estado técnico: \`/api/web/health\`. Si me pides «Busca las últimas novedades de GitHub con fuentes», debo usar esa capa y mostrar los enlaces recuperados.';
    }
    return 'Universal Core **sí tiene implementada una capa web** (búsqueda, recuperación de páginas y fuentes), pero este despliegue no tiene configurado un motor de búsqueda general. Solo queda la API de respuestas instantáneas de DuckDuckGo, cuya cobertura es limitada y no equivale a navegar toda la web. Mi respuesta anterior de que no existe navegador o acceso web fue demasiado absoluta. El siguiente paso es habilitar y verificar un proveedor de búsqueda general; el estado técnico está en \`/api/web/health\`. No voy a fingir que una consulta en tiempo real funcionó si no hay resultados verificables.';
  }
  ...
  if(/que sabes hacer/.test(normalizedQuery)){
    return `Puedo trabajar en varias capas: **razonamiento y análisis**, **investigación con evidencia**, **programación e ingeniería**, **datos y archivos**, **planificación y orquestación multiagente**, **memoria/contexto**, y **ejecución de herramientas cuando están habilitadas**. También puedo convertir una tarea en un plan, código, informe, procedimiento o siguiente acción verificable.
    
No presento como disponible una función externa si el runtime no la tiene habilitada. Si me das una tarea concreta, puedo demostrar la capacidad directamente en lugar de limitarme a enumerarla.`;
  }
  if(/que capacidades tienes|cuales son tus capacidades|capacidades tienes/.test(normalizedQuery)){
    return `Mis capacidades se organizan en seis bloques: **Razonamiento**, **Investigación**, **Ingeniería**, **Análisis de datos**, **Coordinación de herramientas/agentes** y **Continuidad de contexto**.
    
- **Pensar:** análisis, síntesis, planificación y resolución de problemas.
- **Investigar:** recuperación de información actual y evidencia cuando la ruta web está disponible.
- **Construir:** código, arquitectura, debugging y diseño de soluciones.
- **Analizar:** archivos, CSV, métricas, relaciones, anomalías y visualizaciones nativas.
- **Coordinar:** misiones, herramientas y agentes con controles de riesgo.
- **Continuidad:** memoria y contexto explícito para mantener objetivos entre turnos.

La disponibilidad concreta depende del runtime, permisos y herramientas habilitadas; puedo comprobarlo ejecutando la tarea.`;
  }
