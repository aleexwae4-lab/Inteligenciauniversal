...
export function shouldRecoverAnswerV101(status=200,payload={}){
  ...
  if(['METHOD_NOT_ALLOWED','ORIGIN_NOT_ALLOWED','RATE_LIMITED','UNAUTHORIZED','FORBIDDEN'].includes(code))return false;
  return Number(status)>=500||payload?.recoverable===true||!replyOfV101(payload)||GENERIC_FAILURE_RX.test(replyOfV101(payload));
}
