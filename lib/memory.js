...
export function memoryStatus() {
  return {
    ...
    version:'context-memory/v103.1-conversation-scoped',
    sessionAware:true,
...
export async function recallMemory(userKey, query, limit=8, sessionId='', conversationId='') {
  ...
  try {
    const res = await fetch(`${process.env.SUPABASE_URL}/rest/v1/rpc/wae_universal_memory_recall_v104`, {
...
export async function saveTurn(userKey, sessionId, userText, assistantText, meta={}) {
  ...
  try {
    ...
    const row={
      ...
      metadata:{...metadata,conversationId:conversationId||undefined,memoryVersion:'context-memory/v103.1'}
    };
