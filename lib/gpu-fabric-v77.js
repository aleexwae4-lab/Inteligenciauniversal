...
const intEnv=(name,fallback,min,max)=>{const parsed=Number.parseInt(process.env[name]||'',10);return clamp(Number.isFinite(parsed)?parsed:fallback,min,max)};
const numEnv=(name,fallback,min,max)=>{const parsed=Number.parseFloat(process.env[name]||'');return clamp(Number.isFinite(parsed)?parsed:fallback,min,max)};
