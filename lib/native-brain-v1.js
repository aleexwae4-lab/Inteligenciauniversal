...
function localKnowledge(message=''){
  ...
  if(/^(que puedes hacer|que sabes|que sabes hacer|cuales son tus capacidades|que capacidades tienes)$/.test(q))return 'Puedo **explicar, investigar, analizar, programar, diseñar, redactar, trabajar con archivos, estructurar proyectos y operar flujos multiagente**. Cuando una tarea requiere datos actuales o evidencia, debe activarse recuperación verificable; cuando es conocimiento estable o creación, respondo directamente.';
...
export function nativeBrainStatus(){
  ...
  return {version:VERSION,ready:true,architecture:'native-orchestrator-with-interchangeable-inference-engines',providers:providers.map(p=>({id:p.id,configured:p.configured,model:p.model})),localKernel:true,localCapabilities:['stable-facts','basic-math','identity','capabilities','safe-fallback'],responseSchema:'assistant-response/v1'};
}
...
export async function nativeBrainReply(payload={}){
  ...
  let nativePath='generative';

  if(!meaningful(text)&&local){
    text=local;provider='wae_native_kernel';model='native-knowledge-kernel-v1';degraded=false;nativePath='local-kernel';
  }

  if(!meaningful(text)){
    text='**Universal Core sigue operativo, pero ningún motor generativo completó esta consulta con calidad suficiente.** El núcleo nativo preservó tu solicitud; puedo resolverla mediante conocimiento local, herramientas o recuperación verificable cuando la tarea lo permita.';
    provider='wae_native_kernel';model='native-continuity-v1';degraded=true;nativePath='native-continuity';
  }

  const latencyMs=Date.now()-started;
  const response=buildAssistantResponse({content:text,sources:Array.isArray(result?.webSources)?result.webSources:[],provider,model,latencyMs,memoryCount:history.length,requestId:crypto.randomUUID(),conversationId:payload.conversation_id||payload.conversationId||null,webUsed:Array.isArray(result?.webSources)&&result.webSources.length>0,degraded});
  response.metadata={...response.metadata,nativeBrain:VERSION,nativePath,configuredProviders:configured.map(p=>p.id)};

  return {success:true,reply:text,speech_text:response.speechText,response,components:response.components,actions:response.actions,provider,model,degraded,native_brain:VERSION,native_path:nativePath,latencyMs,failures,web_sources:Array.isArray(result?.webSources)?result.webSources:[]};
}
