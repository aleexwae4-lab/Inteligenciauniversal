...
function localKnowledge(message=''){
  ...
  if(/^(?:que es|explica|define|sabes que es)(?: la)? neuroplasticidad$|^neuroplasticidad$/.test(q))return '**La neuroplasticidad es la capacidad del sistema nervioso, especialmente del cerebro, para modificar su estructura y funcionamiento en respuesta a la experiencia, el aprendizaje o una lesión.** Esto ocurre mediante cambios en la fuerza y organización de las conexiones neuronales y, en ciertos contextos, mediante reorganización de redes. Es fundamental para aprender, formar recuerdos y recuperar algunas funciones después de daño cerebral.';
