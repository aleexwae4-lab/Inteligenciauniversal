...
const timedFetch=async(url,options={},ms=7000)=>{
  const r=await fetch(url,{...options,signal:AbortSignal.timeout(ms),headers:{'Accept':'application/json','User-Agent':'WAEUniversalCore/1.0',...(options.headers||{})}});
...
export async function retrieveResearch(query,{mode='auto'}={}){
  ...
  const freshness=/\b(?:hoy|ahora|actual|ultim|recient|en vivo|tiempo real|precio|cotizaci|tasas? de interes|noticias|202[5-9])\b/i.test(query);
  if(mode==='auto'&&freshness&&!process.env.TAVILY_API_KEY)return {ok:false,code:'general_web_not_configured',channel:'web',results:[],checkedAt:new Date().toISOString()};
