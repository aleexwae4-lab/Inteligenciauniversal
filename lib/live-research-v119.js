...
export function researchCapabilities(){
  return {version:LIVE_RESEARCH_VERSION,generalWeb:{provider:waewebConfigured()?'waeweb-connect/v1':'tavily',configured:waewebConfigured()||!!process.env.TAVILY_API_KEY,verified:false},
...
export async function retrieveResearch(query,{mode='auto'}={}){
  ...
  let channel=mode==='auto'?(waewebConfigured()||process.env.TAVILY_API_KEY?'web':(current||freshness)?'news':scholarly?'academic':'encyclopedia'):mode;
  if(channel==='web'&&waewebConfigured()&&query.length<=180){
    try{
      const evidence=await searchWaewebEvidence(query,{fresh:current||freshness});
      const sources=verifiedRecords('waeweb-connect/v1',evidence.results.map(x=>({
        title:x.title,url:x.url,snippet:x.snippet,publishedAt:x.publishedAt,scope:x.scope
      })));
      if(sources.length||!process.env.TAVILY_API_KEY)return {ok:true,channel:'web',provider:'waeweb-connect/v1',
