...
export async function executeMission(payload={}){
  ...
  if(['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      generated=await generateWithFallback({provider:requestedProvider,system,message:enrichedMessage,history,clientRequestId});
  ...
  if(shouldRunChallenger({message,mode,quality,degraded:generated.degraded===true,requestedProvider})){
    tournamentUsed=true;
    try{
      const challenger=await generateWithFallback({provider:requestedProvider,system:`${system}${challengerInstruction(quality)}`,message:enrichedMessage,history,clientRequestId});
  ...
  if(quality.critical&&!tournamentUsed&&!['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      const repaired=await generateWithFallback({provider:requestedProvider,system:`${system}${repairInstruction(quality)}\n\nPreserva estrictamente el hilo conversacional inmediato y la identidad Universal Core.`,message:enrichedMessage,history,clientRequestId});
  ...
  if(frontierQuality.needsUpgrade&&!tournamentUsed&&!repairAttempted&&!['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      const upgraded=await generateWithFallback({provider:requestedProvider,system:`${system}${frontierUpgradeInstructionV102(frontierQuality)}`,message:enrichedMessage,history,clientRequestId});
