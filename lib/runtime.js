...
export function runtimeHealth(){const providers=providerRegistry();const tools=toolRegistry();const generativeReady=providers.some(p=>p.configured);return{ok:true,service:'wae-universal-runtime',version:'1.5.0-capability-kernel',responseSchema:'assistant-response/v1',providers,tools,memory:memoryStatus(),responsePolicy:'premium-rich-voice-safe',capabilityKernel:{schema:CAPABILITY_KERNEL_VERSION,receipts:true,parallelReadonly:true,writePolicy:'explicit-approval-required'},continuity:{configured:true,provider:'universal_continuity_core',model:'universal-core-continuity-v1',externalProvider:false,costUsd:0},generativeReady,ready:true}}
