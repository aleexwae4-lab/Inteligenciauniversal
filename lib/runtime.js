...
import { buildActionGateway, actionGatewayInstruction, publicActionGatewayContract, ACTION_GATEWAY_VERSION } from './action-gateway-v1.js';
import { buildAgentExecutionFabric, agentExecutionInstruction, publicAgentExecutionContract, AGENT_EXECUTION_FABRIC_VERSION } from './agent-execution-fabric-v1.js';
...
import { runTools, executeWebSearch, formatToolContext, toolRegistry } from './tools.js';
...
import { normalizeMultimodalAttachments, multimodalCapabilities, multimodalTextContext } from './multimodal-input-v1.js';
...
import { resolveCurrentFactQuery, currentFactInstruction, publicCurrentFactResolution, CURRENT_FACT_RESOLUTION_VERSION } from './current-fact-resolution-v1.js';
import { knowledgeExpansionInstruction, publicKnowledgeExpansion, KNOWLEDGE_EXPANSION_VERSION } from './knowledge-expansion-v4.js';
import { buildExpertOrchestration, expertOrchestrationInstruction, publicExpertOrchestration, UNIVERSAL_EXPERT_ORCHESTRATOR_VERSION } from './universal-expert-orchestrator-v1.js';
import { identityAuthorityInstruction, publicIdentityAuthority, UNIVERSAL_IDENTITY_AUTHORITY_VERSION } from './universal-identity-authority-v1.js';
import { buildMissionDirector, missionDirectorInstruction, publicMissionDirector, UNIVERSAL_MISSION_DIRECTOR_VERSION } from './universal-mission-director-v1.js';
import { buildMissionExecutionState, missionExecutionInstruction, publicMissionExecution, UNIVERSAL_MISSION_EXECUTION_VERSION } from './universal-mission-execution-v1.js';
import { recoverMissionTools, missionRecoveryInstruction, publicMissionRecovery, UNIVERSAL_MISSION_RECOVERY_VERSION } from './universal-mission-recovery-v1.js';\nimport { buildMissionProofDossier, missionProofDossierInstruction, publicMissionProofDossier, UNIVERSAL_MISSION_PROOF_DOSSIER_VERSION } from './universal-mission-proof-dossier-v1.js';
import { buildMissionAudit, missionAuditInstruction, publicMissionAudit, UNIVERSAL_MISSION_AUDIT_VERSION } from './universal-mission-audit-v1.js';
...
import { buildAnswerTraceability, publicAnswerTraceability } from './answer-traceability-v1.js';
import {
...
function sanitizeAttachments(attachments=[]){return normalizeMultimodalAttachments(attachments).slice(0,8).map(a=>({...a,name:a.name.slice(0,180),mime:a.mime.slice(0,120),text:a.text.slice(0,120000),dataUrl:a.dataUrl.slice(0,8000000),url:a.url.slice(0,4000)}))}
function attachmentContext(attachments=[]){if(!attachments.length)return'';return `\n\nARCHIVOS ADJUNTOS:\n${attachments.map(a=>`\n--- ${a.name} (${a.mime}) ---\n${a.text||'[archivo binario: usar entrada multimodal compatible]'}`).join('\n')}`}
...
export async function executeMission(payload={}){
  ...
  const actionGateway=buildActionGateway({decisionWorkspace,toolRegistry:toolRegistry(),toolResults:[]});
  const agentExecutionFabric=buildAgentExecutionFabric({tools:toolRegistry(),actionGateway});
  const worldMission=planWorldMission(message,{attachments});
  const expertOrchestration=buildExpertOrchestration(message,{attachments,mode});
const missionDirector=buildMissionDirector(message,expertOrchestration);
  const missionExecution=buildMissionExecutionState({message,director:missionDirector,toolResults:toolResults||[],executionProof:null});
  ...
  if(quick){
    ...
    response.metadata={...response.metadata,quality,frontierQuality,contextIntegrity:gated.contextIntegrity,multimodal:multimodalCapabilities(attachments),cognitivePolicy,repairAttempted:false,premiumUpgradeAttempted:false,fastLane:true,userContext,supremacy:{path:'instant_conversation',cacheHit:false,tournamentUsed:false}};
  ...
  const [memory,initialToolResults]=await Promise.all([
  ...
  const recoveredMissionTools=await recoverMissionTools(initialToolResults,{message,webSearch:executeWebSearch,maxRetries:1});
  const toolResults=recoveredMissionTools.toolResults;
  const missionRecoveryRun=recoveredMissionTools.recovery;
  ...
  // Actualiza el bucle con evidencia obtenida en esta misma misión. Una sola pasada,
  // acotada y sin resolver conflictos automáticamente.
  let verificationLoop=buildMultimodalVerificationLoop({synthesis:multimodalEvidenceSynthesis,composer:multimodalAnswerComposer,toolResults});
  // Segunda pasada activa, estrictamente acotada: máximo 6 comprobaciones y sin recursión.
  const verificationWebAvailable=toolResults.find(item=>item.tool==='web_search'&&item.ok&&Array.isArray(item.data));
  if(verificationLoop?.tasks?.some(x=>x.status==='pending')&&verificationWebAvailable){
    verificationLoop=await runMultimodalVerificationPass(verificationLoop,{search:executeWebSearch,maxTasks:6});
    verificationLoop=evaluateVerificationEvidence(verificationLoop);
  }
  const multimodalEvidenceDecision=buildEvidenceDecision({verification:verificationLoop,synthesis:multimodalEvidenceSynthesis,truthConflict:multimodalTruthConflict,temporalCausal:multimodalTemporalCausal});
  const multimodalEvidenceQuality=buildEvidenceQuality({verification:verificationLoop});
  const multimodalEvidenceCalibration=buildEvidenceCalibration({verification:verificationLoop,quality:multimodalEvidenceQuality,decision:multimodalEvidenceDecision});
  const currentFactResolution=resolveCurrentFactQuery(message);
  const finalActionGateway=buildActionGateway({decisionWorkspace,toolRegistry:toolRegistry(),toolResults});
  ...
  const system=`${agent.system}\n\nReglas del runtime: identifica límites reales; si una integración no está disponible dilo; prioriza seguridad y reversibilidad; no afirmes que ejecutaste acciones externas salvo que aparezcan en EVIDENCIA DE HERRAMIENTAS.${PREMIUM_RESPONSE_POLICY}${epistemicInstruction(initialEpistemic)}${frontierInstruction}${continuityInstruction}\n\n${routeInstruction}${privateUserContext}${industrialSystemInstruction(industrialMission)}${professionalSystemInstruction(professionalMission)}${worldSystemInstruction(worldMission)}${multimodalIntelligenceInstruction(multimodalIntelligence)}${evidenceGraphInstruction(multimodalEvidenceGraph)}${crossMediaInstruction(crossMediaReasoning)}${evidenceLocationsInstruction(evidenceLocations)}${findingsInstruction(multimodalFindings)}${investigationInstruction(multimodalInvestigation)}${truthConflictInstruction(multimodalTruthConflict)}${temporalCausalInstruction(multimodalTemporalCausal)}${evidenceSynthesisInstruction(multimodalEvidenceSynthesis)}${answerComposerInstruction(multimodalAnswerComposer)}${verificationLoopInstruction(verificationLoop)}${evidenceDecisionInstruction(multimodalEvidenceDecision)}${evidenceQualityInstruction(multimodalEvidenceQuality)}${evidenceCalibrationInstruction(multimodalEvidenceCalibration)}${currentFactInstruction(currentFactResolution)}${knowledgeExpansionInstruction(message)}${expertOrchestrationInstruction(expertOrchestration)}${analyticsInstruction(analyticsReport)+dataIntelligenceInstruction(dataIntelligenceWorkspace)+visualizationInstruction(visualizationSpecs)+decisionWorkspaceInstruction(decisionWorkspace)+actionGatewayInstruction(actionGateway)+agentExecutionInstruction(agentExecutionFabric)}\n\n${ANSWER_COMPOSITION_GUIDANCE}`;
  ...
  const webToolNotice=(payload.web_enabled===true||cognitivePolicy.autoResearch)
    ...
    : '';
  const enrichedMessage=`${message}${attachmentContext(attachments)}${multimodalTextContext(attachments)}${formatMemoryContext(memory)}${formatToolContext(toolResults)}${webToolNotice}${economics?`\n\nCÁLCULO DETERMINISTA CON DATOS RECIBIDOS:\n${JSON.stringify(economics)}`:''}`;
  ...
  if(['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      generated=await generateWithFallback({provider:requestedProvider,system,message:enrichedMessage,history,attachments});
  ...
  const latencyMs=Date.now()-started;
  const visualBlocks=visualizationSpecs.map(spec=>'\\n\\n```wae-chart\\n'+JSON.stringify(spec)+'\\n```').join('');
  const composed=normalizeAnswerComposition(generated.text+visualBlocks);
  ...
  const answerTraceability=buildAnswerTraceability({answer:generated.text,synthesis:multimodalEvidenceSynthesis,verification:verificationLoop,sources});
  ...
  const completedMissionExecution=buildMissionExecutionState({message,director:missionDirector,toolResults,executionProof});\n  const missionProofDossier=buildMissionProofDossier({message,missionExecution:completedMissionExecution,missionRecovery:missionRecoveryRun,executionProof,toolResults,sources,verification:verificationLoop,traceability:answerTraceability,quality,epistemic,missionOrchestration:completedMission});
  const missionAudit=buildMissionAudit({dossier:missionProofDossier,executionProof,recovery:missionRecoveryRun,toolResults,mission:completedMission?.mission});
  ...
  response.metadata={...response.metadata,quality,frontierQuality,epistemic,multimodal:multimodalCapabilities(attachments),multimodalIntelligence:publicMultimodalIntelligence(multimodalIntelligence),multimodalEvidenceGraph:publicEvidenceGraph(multimodalEvidenceGraph),crossMediaReasoning:publicCrossMediaReasoning(crossMediaReasoning),evidenceLocations:publicEvidenceLocations(evidenceLocations),multimodalFindings:publicFindings(multimodalFindings),multimodalInvestigation:publicInvestigation(multimodalInvestigation),multimodalTruthConflict:publicTruthConflict(multimodalTruthConflict),multimodalTemporalCausal:publicTemporalCausal(multimodalTemporalCausal),multimodalEvidenceSynthesis:publicEvidenceSynthesis(multimodalEvidenceSynthesis),multimodalAnswerComposer:publicAnswerComposer(multimodalAnswerComposer),multimodalVerificationLoop:publicVerificationLoop(verificationLoop),multimodalEvidenceDecision:publicEvidenceDecision(multimodalEvidenceDecision),multimodalEvidenceQuality:publicEvidenceQuality(multimodalEvidenceQuality),multimodalEvidenceCalibration:publicEvidenceCalibration(multimodalEvidenceCalibration),answerTraceability:publicAnswerTraceability(answerTraceability),currentFactResolution:publicCurrentFactResolution(currentFactResolution),epistemicGuardVersion:EPISTEMIC_GUARD_VERSION,contextIntegrity,cognitivePolicy,repairAttempted,premiumUpgradeAttempted,userContext,industrialEngineering:publicIndustrialPlan(industrialMission),professionalEconomics:publicProfessionalPlan(professionalMission),worldIntelligence:publicWorldPlan(worldMission),operationalCapabilities:operationalPlan,governmentCapabilities:governmentPlan,analytics:{version:ANALYTICS_ENGINE_VERSION,report:analyticsReport},multimodal_intelligence:publicMultimodalIntelligence(multimodalIntelligence),multimodal_evidence_graph:publicEvidenceGraph(multimodalEvidenceGraph),cross_media_reasoning:publicCrossMediaReasoning(crossMediaReasoning),evidence_locations:publicEvidenceLocations(evidenceLocations),multimodal_findings:publicFindings(multimodalFindings),multimodal_investigation:publicInvestigation(multimodalInvestigation),multimodal_truth_conflict:publicTruthConflict(multimodalTruthConflict),multimodal_temporal_causal:publicTemporalCausal(multimodalTemporalCausal),multimodal_evidence_synthesis:publicEvidenceSynthesis(multimodalEvidenceSynthesis),multimodal_answer_composer:publicAnswerComposer(multimodalAnswerComposer),multimodal_verification_loop:publicVerificationLoop(verificationLoop),multimodal_evidence_decision:publicEvidenceDecision(multimodalEvidenceDecision),multimodal_evidence_quality:publicEvidenceQuality(multimodalEvidenceQuality),multimodal_evidence_calibration:publicEvidenceCalibration(multimodalEvidenceCalibration),answer_traceability:publicAnswerTraceability(answerTraceability),current_fact_resolution:publicCurrentFactResolution(currentFactResolution),knowledgeExpansion:publicKnowledgeExpansion(message),knowledge_expansion:publicKnowledgeExpansion(message),expertOrchestration:publicExpertOrchestration(expertOrchestration),
missionDirector:publicMissionDirector(missionDirector),
mission_director:publicMissionDirector(missionDirector),
missionExecution:publicMissionExecution(completedMissionExecution),
mission_execution:publicMissionExecution(completedMissionExecution),
missionRecoveryRun:publicMissionRecovery(missionRecoveryRun),\nmissionProofDossier:publicMissionProofDossier(missionProofDossier),
missionAudit:publicMissionAudit(missionAudit),
mission_audit:publicMissionAudit(missionAudit),\nmission_proof_dossier:publicMissionProofDossier(missionProofDossier),
mission_recovery_run:publicMissionRecovery(missionRecoveryRun),expert_orchestration:publicExpertOrchestration(expertOrchestration),visualization:{version:NATIVE_VISUALIZATION_VERSION,specs:visualizationSpecs},decisionWorkspace:{version:DECISION_WORKSPACE_VERSION,data:decisionWorkspace},executionProof:{version:EXECUTION_PROOF_VERSION,...executionProof},economics,universalEnvelope,progressiveIntelligence:progressiveContract(),providerStreaming:streamingContract(),adaptiveRouting:adaptiveContract(),runtimeSlo:runtimeSloContract(),runtimeOperations:runtimeOperations(),missionOrchestration:completedMission,missionExecution:publicMissionExecution(completedMissionExecution),missionRecoveryRun:publicMissionRecovery(missionRecoveryRun),missionRecovery,operationalScore:turnOperationalScore({latencyMs,fallbackCount:generated.failures?.length||0}).score,supremacy:{path:'generation',cacheHit:false,tournamentUsed,candidateCount}};
  ...
  return{reply:generated.text,response,speech_text:response.speechText,components:response.components,actions:response.actions,web_sources:sources,request_id:requestId,response_schema:response.schema,provider:generated.provider,model:generated.model,degraded:generated.degraded===true,agent:{id:agent.id,name:agent.name},tools:toolResults,memory:{recalled:memory.length,persistent:memoryConfigured,version:'context-memory/v103'},usage:generated.usage||null,latencyMs,fallbackFailures:generated.failures||[],quality,frontier_quality:frontierQuality,context_integrity:contextIntegrity,cognitive_policy:cognitivePolicy,user_context:userContext,industrial_engineering:publicIndustrialPlan(industrialMission),professional_economics:publicProfessionalPlan(professionalMission),world_intelligence:publicWorldPlan(worldMission),expert_orchestration:publicExpertOrchestration(expertOrchestration),operational_capabilities:operationalPlan,government_capabilities:governmentPlan,analytics:{version:ANALYTICS_ENGINE_VERSION,report:analyticsReport},multimodal_intelligence:publicMultimodalIntelligence(multimodalIntelligence),visualization:{version:NATIVE_VISUALIZATION_VERSION,specs:visualizationSpecs},decision_workspace:{version:DECISION_WORKSPACE_VERSION,data:decisionWorkspace},action_gateway:{version:ACTION_GATEWAY_VERSION,data:finalActionGateway||actionGateway},agent_execution:{version:AGENT_EXECUTION_FABRIC_VERSION,data:agentExecutionFabric},execution_proof:executionProof,economic_projection:economics,universal_envelope:universalEnvelope,repair_attempted:repairAttempted,premium_upgrade_attempted:premiumUpgradeAttempted,mission_orchestration:completedMission,mission_execution:publicMissionExecution(completedMissionExecution),mission_recovery_run:publicMissionRecovery(missionRecoveryRun),mission_proof_dossier:publicMissionProofDossier(missionProofDossier),mission_recovery:missionRecovery,epistemic,epistemic_guard_version:EPISTEMIC_GUARD_VERSION,supremacy:{path:'generation',cache_hit:false,tournament_used:tournamentUsed,candidate_count:candidateCount}};
}