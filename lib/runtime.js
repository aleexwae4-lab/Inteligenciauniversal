...
import { buildContinuityResult } from './continuity-core.js';
...
export function runtimeHealth(){const providers=providerRegistry();const tools=toolRegistry();return{ok:true,service:'wae-universal-runtime',version:'1.3.0',responseSchema:'assistant-response/v1',providers,tools,memory:memoryStatus(),responsePolicy:'premium-rich-voice-safe',continuity:{configured:true,provider:'universal_continuity_core',model:'deterministic-evidence-v1'},ready:providers.some(p=>p.configured)}}
...
export async function executeMission(payload={}){
  ...
  if(requestedProvider==='continuity_core'){
    generated=buildContinuityResult({message,attachments,memory,toolResults,failures:[]});
  }else{
    try{
      generated=await generateWithFallback({provider:requestedProvider,system,message:enrichedMessage,history});
    }catch(error){
      generated=buildContinuityResult({message,attachments,memory,toolResults,failures:error?.failures||[{provider:requestedProvider,error:String(error?.message||error).slice(0,500)}]});
    }
  }
