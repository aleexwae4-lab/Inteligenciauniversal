...
export async function executeMission(payload={}){
  ...
  if(quick){
    ...
    response.metadata={...response.metadata,quality,cognitivePolicy,repairAttempted:false,fastLane:true,userContext,supremacy:{path:'instant_conversation',cacheHit:false,tournamentUsed:false}};
  ...
  if(cacheEligible){
    ...
    if(hit){
      ...
      response.metadata={...response.metadata,quality,cognitivePolicy,repairAttempted:false,cacheHit:true,cacheSimilarity:hit.similarity,userContext,supremacy:{path:'semantic_cache',cacheHit:true,tournamentUsed:false}};
  ...
  const system=`${agent.system}\n\nReglas del runtime: identifica límites reales; si una integración no está disponible dilo; prioriza seguridad y reversibilidad; no afirmes que ejecutaste acciones externas salvo que aparezcan en EVIDENCIA DE HERRAMIENTAS.${PREMIUM_RESPONSE_POLICY}${privateUserContext}`;
  ...
  let generated,repairAttempted=false,tournamentUsed=false,candidateCount=1;
  ...
  if(cacheEligible&&quality.pass&&generated.degraded!==true){
  ...
  response.metadata={...response.metadata,quality,cognitivePolicy,repairAttempted,userContext,supremacy:{path:'generation',cacheHit:false,tournamentUsed,candidateCount}};
