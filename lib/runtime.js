...
export function runtimeHealth(){const providers=providerRegistry();const tools=toolRegistry();const generativeReady=providers.some(p=>p.configured);return{ok:true,service:'wae-universal-runtime',version:'1.4.2-conversation-recovery',responseSchema:'assistant-response/v1',providers,tools,memory:memoryStatus(),responsePolicy:'premium-rich-voice-safe',continuity:{configured:true,provider:'universal_continuity_core',model:'universal-core-continuity-v1',externalProvider:false,costUsd:0,generalGeneration:false},generativeReady,ready:true}}
...
export async function executeMission(payload={}){
  ...
  if(quick){
    generated={text:quick,usage:null,responseId:null,provider:'universal_core',model:'universal-core-conversation-fast-v2',failures:[],degraded:false};
