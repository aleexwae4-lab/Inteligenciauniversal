...
export async function executeMission(payload={}) {
  ...
  const generated = await generateWithFallback({ provider:String(payload.provider || 'auto'), system, message:enrichedMessage, history, mode:agent.id, webEnabled:needsWebResearch(message, mode) });
