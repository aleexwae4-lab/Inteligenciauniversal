...
export async function executeMission(payload={}){
  ...
  const cacheEligible=!contextualFollowup&&!userContextState.affectsGeneration&&payload.web_enabled!==true&&!cognitivePolicy.autoResearch&&!industrialMission&&semanticCacheEligible({message,mode,provider:requestedProvider,history,attachments,tools:Array.isArray(payload.tools)?payload.tools:[],memoryConfigured});
  if(cacheEligible){
