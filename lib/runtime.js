...
export async function executeMission(payload={}) {
  ...
  await saveTurn(userKey, sessionId, message, reply, { provider:generated.provider, model:generated.model, agent:agent.id, tools:toolResults.map(x=>x.tool), industrialDomain:industrialMission?.domains?.map(x=>x.id)||[], professionalDomain:professionalMission?.domains?.map(x=>x.id)||[] });
