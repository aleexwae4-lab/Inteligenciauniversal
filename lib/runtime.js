...
import { formatToolContext } from './tools.js';
import { runToolFabric, toolFabricRegistry, TOOL_FABRIC_VERSION } from './tool-fabric.js';
...
export function runtimeHealth(){const providers=providerRegistry();const tools=toolFabricRegistry();const generativeReady=providers.some(p=>p.configured);return{ok:true,service:'wae-universal-runtime',version:'1.5.0-tool-fabric-v9',responseSchema:'assistant-response/v1',providers,tools,toolFabric:{schema:TOOL_FABRIC_VERSION,policy:'least-privilege-readonly',receipts:true,parallelReadOnly:true,maxTools:3},memory:memoryStatus(),responsePolicy:'premium-rich-voice-safe',continuity:{configured:true,provider:'universal_continuity_core',model:'universal-core-continuity-v1',externalProvider:false,costUsd:0},generativeReady,ready:true}}
...
export async function executeMission(payload={}){
  ...
  const fabric=payload.disableTools===true
    ? {schema:TOOL_FABRIC_VERSION,plan:{schema:TOOL_FABRIC_VERSION,policy:'disabled',selected:[],denied:[],parallel:false,maxTools:0},results:[],receipts:[],elapsedMs:0}
    : await runToolFabric({agent,message,requestedTools:Array.isArray(payload.tools)?payload.tools:[]});
  const toolResults=fabric.results;
  ...
  await saveTurn(userKey,sessionId,message,generated.text,{provider:generated.provider,model:generated.model,agent:agent.id,tools:toolResults.map(x=>x.tool),toolFabric:TOOL_FABRIC_VERSION,toolReceipts:fabric.receipts.map(x=>x.id),requestId,responseSchema:response.schema,degraded:generated.degraded===true});
  return{reply:generated.text,response,speech_text:response.speechText,components:response.components,actions:response.actions,web_sources:sources,request_id:requestId,response_schema:response.schema,provider:generated.provider,model:generated.model,degraded:generated.degraded===true,agent:{id:agent.id,name:agent.name},tools:toolResults,tool_fabric:{schema:TOOL_FABRIC_VERSION,policy:fabric.plan.policy,parallel:fabric.plan.parallel,selected:fabric.plan.selected.map(x=>x.id),denied:fabric.plan.denied,receipts:fabric.receipts,elapsedMs:fabric.elapsedMs},memory:{recalled:memory.length,persistent:memoryStatus().configured},usage:generated.usage||null,latencyMs,fallbackFailures:generated.failures||[]};
}
