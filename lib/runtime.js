...
export async function executeMission(payload={}) {
  ...
  if (isCoreSelfQuery(message)) return { reply:coreSelfResponse({question:message}), provider:'wae_core', model:'runtime_capabilities', agent:{id:'general',name:'Universal Core'}, tools:[], usage:null, latencyMs:Date.now()-started, grounded:true };
  ...
  if(payload.canvas!==true&&payload.canvas_blueprint!==true){
    ...
    if(dated)return {reply:dated.reply,provider:'wae_research_registry',model:dated.id,agent:{id:'research',name:'Universal Core'},tools:[],sources:[dated.source],usage:null,latencyMs:Date.now()-started,grounded:true,asOf:dated.asOf,liveResearch:false};
  }
  ...
  const generated = await generateWithFallback({ provider:String(payload.provider || 'auto'), system:groundedSystem, message:enrichedMessage, history, mode:agent.id, webEnabled:researchEnabled, qualityGate:answer=>irrelevantEvidenceAnswer(answer,message)||coreComparisonIssue(answer,message)||purposeResponseIssue(answer,message,{verifiedEmail,verifiedWeb}), retryColdStart:attachments.length===0&&requestedTools.length===0 });
