...
export async function executeMission(payload={}){
  ...
  const cacheEligible=!industrialMission&&!contextualFollowup&&!userContextState.affectsGeneration&&semanticCacheEligible({message,mode,provider:requestedProvider,history,attachments,tools:Array.isArray(payload.tools)?payload.tools:[],memoryConfigured});
  if(cacheEligible){
