...
import { CAPABILITY_KERNEL_VERSION, publicToolMetadata } from './capability-kernel.js';
...
function continuityGeneration({system,message,history=[],failures=[]}){
  ...
  return{text:payload.choices?.[0]?.message?.content||'No pude completar la generación avanzada en este intento. Conservé el contexto para que puedas reintentar sin perder la conversación.',usage:payload.usage||null,responseId:payload.id||null,provider:'universal_continuity_core',model:payload.model||'universal-core-continuity-v1',failures,degraded:true};
}

export function runtimeHealth(){const providers=providerRegistry();const tools=toolRegistry();const generativeReady=providers.some(p=>p.configured);return{ok:true,service:'wae-universal-runtime',version:'1.4.0-deep-orchestration',responseSchema:'assistant-response/v1',providers,tools,memory:memoryStatus(),responsePolicy:'premium-rich-voice-safe',capabilityKernel:{schema:CAPABILITY_KERNEL_VERSION,receipts:true,parallelReadonly:true,writePolicy:'explicit-approval-required'},continuity:{configured:true,provider:'universal_continuity_core',model:'universal-core-continuity-v1',externalProvider:false,costUsd:0},generativeReady,ready:true}}

export function publicRuntimeHealth(){
  const health=runtimeHealth();
  return{
    ok:health.ok,service:health.service,version:health.version,responseSchema:health.responseSchema,
    tools:health.tools.map(publicToolMetadata),memory:health.memory,responsePolicy:health.responsePolicy,
    capabilityKernel:health.capabilityKernel,
    continuity:{configured:health.continuity.configured,externalProvider:false,costUsd:0},
    generativeReady:health.generativeReady,ready:health.ready
  };
}

export function publicMissionResult(result={}){
  const metadata={...(result.response?.metadata||{})};
  delete metadata.provider;
  delete metadata.model;
  const response=result.response?{...result.response,metadata}:result.response;
  const tools=Array.isArray(result.tools)?result.tools.map(item=>({tool:item.tool,ok:item.ok===true,error:item.ok?undefined:item.error,receipt:item.receipt||null})):[];
  const clean={...result,response,tools};
  delete clean.provider;
  delete clean.model;
  delete clean.fallbackFailures;
  return clean;
}
...
export async function executeMission(payload={}){
  ...
  const toolResults=payload.disableTools===true?[]:await runTools({
    agent,message,requestedTools:Array.isArray(payload.tools)?payload.tools:[],
    approval:payload.toolApproval===true,budgetMs:payload.toolBudgetMs
  });
  const capabilityPlan=toolResults.plan||null;
  const toolReceipts=toolResults.map(item=>item.receipt).filter(Boolean);
  const system=`${agent.system}\n\nReglas del runtime: identifica límites reales; si una integración no está disponible dilo; prioriza seguridad y reversibilidad; no afirmes que ejecutaste acciones externas salvo que aparezcan en EVIDENCIA DE HERRAMIENTAS. Las capacidades de escritura requieren aprobación explícita antes de ejecutarse.${PREMIUM_RESPONSE_POLICY}`;
