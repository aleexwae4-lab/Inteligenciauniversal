...
import { capabilityProofSnapshot } from './capability-proof-v153.js';
...
import { ANSWER_COMPOSITION_GUIDANCE, normalizeAnswerComposition, compositionIssue, compositionQuality, answerCompositionContract } from './answer-composition-v138.js';
...
function deterministicTurn({started,trace,reply,provider,model,agent={id:'general',name:'Universal Core'},tools=[],sources=[],grounded=true,extras={}}){
  ...
  const e2e=trace.snapshot({deterministic:true});
  const operational=turnOperationalScore({latencyMs,e2e,fallbackCount:0});
  const composition=compositionQuality(reply);
  const composedReply=normalizeAnswerComposition(reply);
  const response=buildResponseEnvelope({reply:composedReply,sources,provider,model,agent,latencyMs,memory:{recalled:0},tools,fallbackFailures:[],composition});
  if(response?.metadata)response.metadata.operationalScore=operational.score;
  return {
    reply:composedReply,sources,provider,model,agent,tools,usage:null,latencyMs,grounded,operational,composition,
...
export function runtimeHealth() {
  ...
  return {
    ...
    adaptiveRouting:adaptiveContract(),
    runtimeSlo:runtimeSloContract(),
    answerComposition:answerCompositionContract(),
...
export async function executeMission(payload={},options={}) {
  ...
  if (payload.canvas_blueprint === true) {
    ...
    const latencyMs=Date.now()-started;
    return { reply:generated.text,provider:generated.provider,model:generated.model,agent:{id:'code',name:'Canvas Blueprint'},tools:[],usage:generated.usage||null,latencyMs,operational:turnOperationalScore({latencyMs,fallbackCount:generated.failures?.length||0}),canvas_blueprint:true };
  }
  if (payload.canvas === true) {
    ...
    const latencyMs=Date.now()-started;
    return {reply:generated.text,provider:generated.provider,model:generated.model,agent:{id:'code',name:'Canvas HTML'},tools:[],usage:generated.usage||null,latencyMs,operational:turnOperationalScore({latencyMs,fallbackCount:generated.failures?.length||0}),canvas:true};
  }
  if (isCoreSelfQuery(message)) {
    ...
    return deterministicTurn({
      started,trace,reply,provider:'wae_core',model:'runtime_capabilities/v153',
      extras:{capabilityMatrix,capabilityProof}
    });
  }
  ...
  try {
    ...
    // Provider exhaustion must never collapse the visible chat into a blank/error state.
    // Final bounded rescue is deterministic, local and zero-cost.
    const continuityPayload = openAIContinuityResponse({
      messages: [
        { role:'system', content:groundedSystem },
        ...history.slice(-12).map(x=>({ role:x.role, content:String(x.text ?? x.content ?? '').slice(0,12000) })),
        { role:'user', content:enrichedMessage }
      ]
    });
    generated = {
      text:continuityPayload.choices?.[0]?.message?.content || 'Universal Core mantiene la solicitud en modo de continuidad.',
      provider:'universal_continuity_core',
      model:continuityPayload.model,
      usage:continuityPayload.usage || null,
      failures:error?.failures || [{ provider:'runtime', model:null, error:String(error?.code || 'provider_failed').slice(0,80) }],
      degraded:true
    };
    trace.end('provider','recovered',{provider:generated.provider,model:generated.model,fallbackCount:generated.failures.length,degraded:true});
  }
  trace.begin('sources');
  ...
  const rawReply = removeRedundantParagraphs(appendSourceLinks(generated.text, realSources, message),message,mode,attachments.length>0);
  const composition=compositionQuality(rawReply);
  const reply = normalizeAnswerComposition(rawReply);
  ...
  const latencyMs=Date.now()-started;
  const e2e=trace.snapshot();
  const operational=turnOperationalScore({latencyMs,e2e,fallbackCount:generated.failures?.length||0});
  ...
  if(responseEnvelope?.metadata)responseEnvelope.metadata.degraded=generated.degraded===true;
  if(responseEnvelope?.metadata)responseEnvelope.metadata.operationalScore=operational.score;
