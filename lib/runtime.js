...
export async function executeMission(payload={}) {
  ...
  const toolAgent=payload.web_enabled===false?{...agent,tools:agent.tools.filter(x=>x!=='web_search')}:agent;
  const toolResults = await runTools({ agent:toolAgent, message, requestedTools:researchEnabled ? [...requestedTools, 'web_search'] : requestedTools.filter(x=>x!=='web_search') });
