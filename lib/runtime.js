...
export async function executeMission(payload={}) {
  ...
  const toolResults = await runTools({ agent, message, requestedTools:needsWebResearch(message, mode) ? [...requestedTools, 'web_search'] : requestedTools });
  ...
  await saveTurn(userKey, sessionId, message, reply, { provider:generated.provider, model:generated.model, agent:agent.id, tools:toolResults.map(x=>x.tool) });
