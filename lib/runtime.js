...
export async function executeMission(payload={}){
  ...
  const quick=userContextState.affectsGeneration||contextualFollowup?null:casualCoreReply(message);
  if(quick){
  ...
  const cacheEligible=!contextualFollowup&&!userContextState.affectsGeneration&&semanticCacheEligible({message,mode,provider:requestedProvider,history,attachments,tools:Array.isArray(payload.tools)?payload.tools:[],memoryConfigured});
  if(cacheEligible){
  ...
  response.metadata={...response.metadata,quality,frontierQuality,contextIntegrity,cognitivePolicy,repairAttempted,premiumUpgradeAttempted,userContext,supremacy:{path:'generation',cacheHit:false,tournamentUsed,candidateCount}};
