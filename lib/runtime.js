...
export async function executeMission(payload={}) {
  ...
  const enrichedMessage = `${message}${formatPreferences(payload.preferences)}${attachmentContext(attachments)}${formatMemoryContext(memory)}${formatToolContext(toolResults)}`;
