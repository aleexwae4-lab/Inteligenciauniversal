...
import { createConversationTrace } from './conversation-e2e-v130.js';
...
export async function executeMission(payload={}) {
  ...
  if (isCoreSelfQuery(message)) { trace.end('router','ok',{strategy:'self_query'}); trace.end('provider','skipped',{strategy:'deterministic'}); return { reply:coreSelfResponse({question:message}), provider:'wae_core', model:'runtime_capabilities', agent:{id:'general',name:'Universal Core'}, tools:[], usage:null, latencyMs:Date.now()-started, grounded:true, e2e:trace.snapshot({deterministic:true}) }; }
  const shortComparison=coreComparisonFastAnswer(message);
  if(shortComparison){trace.end('router','ok',{strategy:'comparison_fast'});trace.end('provider','skipped',{strategy:'deterministic'});return {reply:shortComparison,provider:'wae_core',model:'verified_product_comparison/v1',agent:{id:'general',name:'Universal Core'},tools:[],usage:null,latencyMs:Date.now()-started,grounded:true,e2e:trace.snapshot({deterministic:true})};}
  ...
  if(payload.canvas!==true&&payload.canvas_blueprint!==true){
    ...
    if(dated){trace.end('router','ok',{strategy:'dated_registry'});trace.end('provider','skipped',{strategy:'deterministic'});trace.end('sources','ok',{count:1});return {reply:dated.reply,provider:'wae_research_registry',model:dated.id,agent:{id:'research',name:'Universal Core'},tools:[],sources:[dated.source],usage:null,latencyMs:Date.now()-started,grounded:true,asOf:dated.asOf,liveResearch:false,e2e:trace.snapshot({deterministic:true})};}
  }
