...
function deterministicTurn({started,trace,reply,provider,model,agent={id:'general',name:'Universal Core'},tools=[],sources=[],grounded=true,extras={}}){
  ...
  const composition=compositionQuality(composedReply);
...
export async function executeMission(payload={},options={}) {
  ...
  const reply = normalizeAnswerComposition(removeRedundantParagraphs(appendSourceLinks(generated.text, realSources, message),message,mode,attachments.length>0));
