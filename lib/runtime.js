...
export async function executeMission(payload={}){
  ...
  const cacheEligible=!contextualFollowup&&!userContextState.affectsGeneration&&!industrialMission&&semanticCacheEligible({message,mode,provider:requestedProvider,history,attachments,tools:Array.isArray(payload.tools)?payload.tools:[],memoryConfigured});
  if(cacheEligible){
  ...
  frontierQuality=assessFrontierQualityV102({question:message,mode,quality,sources,latencyMs:Date.now()-started,degraded:generated.degraded===true});
  const explicitContinuity=['continuity_core','universal_continuity_core'].includes(requestedProvider);
  // Explicit continuity is a deterministic preservation/extraction lane. Its intentionally
  // concise output is degraded by design and must not be rejected by generative prose gates.
  if((quality.critical||frontierQuality.hardReject)&&!explicitContinuity){
