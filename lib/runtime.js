...
export async function executeMission(payload={}){
  ...
  const system=`${agent.system}\n\nReglas del runtime: identifica límites reales; si una integración no está disponible dilo; prioriza seguridad y reversibilidad; no afirmes que ejecutaste acciones externas salvo que aparezcan en EVIDENCIA DE HERRAMIENTAS.${PREMIUM_RESPONSE_POLICY}${frontierInstruction}${continuityInstruction}\n\n${routeInstruction}${privateUserContext}${industrialSystemInstruction(industrialMission)}`;
  ...
  const webToolNotice=(payload.web_enabled===true||cognitivePolicy.autoResearch)
    ...
    : '';
  const enrichedMessage=`${message}${attachmentContext(attachments)}${formatMemoryContext(memory)}${formatToolContext(toolResults)}${webToolNotice}`;
  ...
  if(['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      generated=await generateWithFallback({provider:requestedProvider,system,message:enrichedMessage,history,requestKey:requestKeyFor('primary')});
  ...
  if(shouldRunChallenger({message,mode,quality,degraded:generated.degraded===true,requestedProvider})){
    tournamentUsed=true;
    try{
      const challenger=await generateWithFallback({provider:requestedProvider,system:`${system}${challengerInstruction(quality)}`,message:enrichedMessage,history,requestKey:requestKeyFor('challenger')});
  ...
  if(quality.critical&&!tournamentUsed&&!['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      const repaired=await generateWithFallback({provider:requestedProvider,system:`${system}${repairInstruction(quality)}\n\nPreserva estrictamente el hilo conversacional inmediato y la identidad Universal Core.`,message:enrichedMessage,history,requestKey:requestKeyFor('repair')});
  ...
  if(frontierQuality.needsUpgrade&&!tournamentUsed&&!repairAttempted&&!['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      const upgraded=await generateWithFallback({provider:requestedProvider,system:`${system}${frontierUpgradeInstructionV102(frontierQuality)}`,message:enrichedMessage,history,requestKey:requestKeyFor('upgrade')});
  ...
  response.metadata={...response.metadata,quality,frontierQuality,contextIntegrity,cognitivePolicy,repairAttempted,premiumUpgradeAttempted,userContext,industrialEngineering:publicIndustrialPlan(industrialMission),supremacy:{path:'generation',cacheHit:false,tournamentUsed,candidateCount}};
