...
export async function executeMission(payload={}){
  ...
  const cacheEligible=payload.web_enabled!==true&&!cognitivePolicy.autoResearch&&!contextualFollowup&&!userContextState.affectsGeneration&&!industrialMission&&semanticCacheEligible({message,mode,provider:requestedProvider,history,attachments,tools:Array.isArray(payload.tools)?payload.tools:[],memoryConfigured});
  if(cacheEligible){
  ...
  if((payload.web_enabled===true||cognitivePolicy.autoResearch)&&!toolsForMission.includes('web_search'))toolsForMission.push('web_search');
  const [memory,toolResults]=await Promise.all([
    recallMemory(userKey,message,8,sessionId),
    payload.disableTools===true?Promise.resolve([]):runTools({agent,message,requestedTools:toolsForMission})
  ]);
