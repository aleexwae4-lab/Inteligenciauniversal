...
export async function executeMission(payload={}) {
  ...
  const groundedSystem=system+(comparison?'\n\n'+CORE_COMPARISON_GROUNDING:'')+(purpose?'\n\n'+PURPOSE_GROUNDING:'');
  ...
  const generated = await generateWithFallback({ provider:String(payload.provider || 'auto'), system:groundedSystem, message:enrichedMessage, history, mode:agent.id, webEnabled:researchEnabled, qualityGate:answer=>coreComparisonIssue(answer,message)||purposeResponseIssue(answer,message,{verifiedEmail,verifiedWeb}) });
