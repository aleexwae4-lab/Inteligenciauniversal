...
export async function executeMission(payload={}){
  ...
  const requestedMode=String(payload.mode||payload.agent||'general'),cognitivePolicy=inferCognitivePolicy(message,requestedMode),mode=cognitivePolicy.mode,agent=getAgent(mode),userKey=String(payload.userKey||payload.sessionId||'anonymous').slice(0,160),sessionId=String(payload.sessionId||'').slice(0,160),history=Array.isArray(payload.history)?payload.history:[],attachments=sanitizeAttachments(payload.attachments),requestedProvider=String(payload.provider||'auto');
  const userContextState=userContextStateV92(payload),userContext=publicUserContextV92(userContextState),privateUserContext=userContextSystemInstructionV92(payload.preferences||{});
  const quick=userContextState.affectsGeneration?null:casualCoreReply(message);
  if(quick){
    const quality=evaluateAnswer({question:message,answer:quick,mode,sources:[]}),latencyMs=Date.now()-started;
    const frontierQuality=assessFrontierQualityV102({question:message,mode,quality,sources:[],latencyMs,degraded:false});
    ...
    response.metadata={...response.metadata,quality,frontierQuality,cognitivePolicy,repairAttempted:false,premiumUpgradeAttempted:false,fastLane:true,userContext,supremacy:{path:'instant_conversation',cacheHit:false,tournamentUsed:false}};
  ...
  if(cacheEligible){
    ...
    if(hit){
      ...
      if(!frontierQuality.needsUpgrade&&!frontierQuality.hardReject){
        ...
        response.metadata={...response.metadata,quality,frontierQuality,cognitivePolicy,repairAttempted:false,premiumUpgradeAttempted:false,cacheHit:true,cacheSimilarity:hit.similarity,userContext,supremacy:{path:'semantic_cache',cacheHit:true,tournamentUsed:false}};
        void Promise.resolve(saveTurn(userKey,sessionId,message,hit.text,{provider:hit.provider,model:hit.model,agent:agent.id,tools:[],requestId,responseSchema:response.schema,degraded:false,qualityScore:quality.score,qualityPass:quality.pass,repairAttempted:false,cognitivePath:'semantic_cache'})).catch(()=>{});
  ...
  const system=`${agent.system}\n\nReglas del runtime: identifica límites reales; si una integración no está disponible dilo; prioriza seguridad y reversibilidad; no afirmes que ejecutaste acciones externas salvo que aparezcan en EVIDENCIA DE HERRAMIENTAS.${PREMIUM_RESPONSE_POLICY}${frontierInstruction}${privateUserContext}`;
  ...
  let generated,repairAttempted=false,premiumUpgradeAttempted=false,tournamentUsed=false,candidateCount=1;
  ...
  if(shouldRunChallenger({message,mode,quality,degraded:generated.degraded===true,requestedProvider})){
    ...
    try{
      ...
      const challengerQuality=evaluateAnswer({question:message,answer:challenger.text,mode,sources:challengerSources});
      candidateCount=2;
      const best=selectBestCandidate([
        {text:generated.text,quality,sources,degraded:generated.degraded===true,generated},
        {text:challenger.text,quality:challengerQuality,sources:challengerSources,degraded:false,generated:{...challenger,degraded:false}}
      ]);
      if(best){generated=best.generated;quality=best.quality;sources=best.sources}
  ...
  if(quality.critical&&!tournamentUsed&&!['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      ...
      const candidateSources=mergeSources(sources,normalizeProviderSources(repaired.webSources)),repairedQuality=evaluateAnswer({question:message,answer:repaired.text,mode,sources:candidateSources});
  ...
  if(frontierQuality.needsUpgrade&&!tournamentUsed&&!repairAttempted&&!['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      ...
      const candidateQuality=evaluateAnswer({question:message,answer:upgraded.text,mode,sources:candidateSources});
      ...
      if(preferFrontierCandidateV102({currentQuality:quality,candidateQuality,currentReport:frontierQuality,candidateReport:candidateFrontier})){
        generated={...upgraded,degraded:false};
        quality=candidateQuality;
        sources=candidateSources;
        frontierQuality=candidateFrontier;
      }
    }catch{}
  }
  ...
  if(quality.critical||frontierQuality.hardReject){
    throw Object.assign(new Error('quality_gate_rejected'),{code:'LOW_QUALITY',statusCode:503,failures:generated.failures||[],quality,frontierQuality});
  }
  ...
  response.metadata={...response.metadata,quality,frontierQuality,cognitivePolicy,repairAttempted,premiumUpgradeAttempted,userContext,supremacy:{path:'generation',cacheHit:false,tournamentUsed,candidateCount}};
  await saveTurn(userKey,sessionId,message,generated.text,{provider:generated.provider,model:generated.model,agent:agent.id,tools:toolResults.map(x=>x.tool),requestId,responseSchema:response.schema,degraded:generated.degraded===true,qualityScore:quality.score,qualityPass:quality.pass,repairAttempted:repairAttempted||premiumUpgradeAttempted,cognitivePath:cognitivePolicy.path});
  return{reply:generated.text,response,speech_text:response.speechText,components:response.components,actions:response.actions,web_sources:sources,request_id:requestId,response_schema:response.schema,provider:generated.provider,model:generated.model,degraded:generated.degraded===true,agent:{id:agent.id,name:agent.name},tools:toolResults,memory:{recalled:memory.length,persistent:memoryConfigured},usage:generated.usage||null,latencyMs,fallbackFailures:generated.failures||[],quality,frontier_quality:frontierQuality,cognitive_policy:cognitivePolicy,user_context:userContext,repair_attempted:repairAttempted,premium_upgrade_attempted:premiumUpgradeAttempted,supremacy:{path:'generation',cache_hit:false,tournament_used:tournamentUsed,candidate_count:candidateCount}};
}
