...
import { resolveEntityGrounding, entityGroundingInstruction, publicEntityGrounding } from './entity-grounding-v1.js';
...
const missionDirector=buildMissionDirector(message,expertOrchestration);
  ...
  const entityGrounding=resolveEntityGrounding(message);
  ...
  if((currentFactResolution.isCurrentFact||entityGrounding.explicitEntity)&&!toolsForMission.includes('web_search'))toolsForMission.push('web_search');
  ...
  const [memory,initialToolResults]=await Promise.all([
    ...
    payload.disableTools===true?Promise.resolve([]):runTools({agent,message,requestedTools:toolsForMission,allowKeylessWeb:payload.web_enabled===true||cognitivePolicy.autoResearch||entityGrounding.explicitEntity})
  ]);
  ...
  if(['continuity_core','universal_continuity_core'].includes(requestedProvider)){
    ...
    try{
      ...
      // Provider exhaustion belongs to the HTTP recovery plane, not the generation
      // plane. Re-throw so /api/chat can execute rescueMission -> emergencyGenerate
      // before bounded Continuity. Continuity remains an explicit/offline lane only.
      error.recoveryStage = 'provider_exhaustion';
      throw error;
    }
  }
