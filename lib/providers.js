...
export function adaptiveResponseContract(message='') {
  ...
  const code=/```|\b(codigo|typescript|javascript|python|sql|api|backend|frontend|debug|bug|refactor|github|deploy|supabase|render|vercel)\b/.test(q);
...
function edgeHeaders() { return {'Content-Type':'application/json','apikey':process.env.SUPABASE_PUBLISHABLE_KEY,'Origin':process.env.PUBLIC_APP_URL || 'https://wae-inteligencia-universal.onrender.com','X-Client-Info':'wae-inteligencia-universal-render-fallback/1.6.0'}; }
export function edgeRequestPolicy(message=''){
  const q=String(message||'');
  const research=/Investiga y verifica con evidencia web reciente|\b(hoy|actual|reciente|latest|today|current|noticias|news|fuentes?|evidencia|investiga|investigación|jurisprudencia|ley vigente|precio|cotización)\b/i.test(q);
  const code=/ingeniería de producción|```|\b(código|typescript|javascript|python|sql|api|backend|frontend|debug|bug|github|deploy)\b/i.test(q);
  const design=/diseño de producto|\b(ux|ui|interfaz|flujo|pantalla|responsive|branding)\b/i.test(q);
  const analysis=/conclusión primero|\b(analiza|análisis|audita|diagnóstico|estrategia|riesgo|roi|arquitectura)\b/i.test(q);
  return {mode:research?'research':code?'code':design?'design':analysis?'analysis':'general',webEnabled:research};
}

async function edgeChat({boot,message,mode,webEnabled}) {
...
async function waeUniversalEdgeResponse({ message, history }) {
  const boot = await jsonFetch(edgeUrl(), { method:'POST', headers:edgeHeaders(), body:JSON.stringify({ action:'bootstrap' }) }, 20000);
  ...
  const enrichedMessage = recent ? `Conversación reciente:\n${recent}\n\nSolicitud actual del usuario:\n${message}` : message;
  const route=edgeRequestPolicy(message);
  let data,rescuePath=null;
  try {
    data=await edgeChat({boot,message:enrichedMessage,mode:route.mode,webEnabled:route.webEnabled});
  } catch (error) {
    const status=Number(error?.status||0);
    const retryable=[429,500,502,503,504].includes(status) || /all_models_unavailable|provider|timeout|temporar/i.test(String(error?.message||''));
    if (!route.webEnabled && retryable && shouldEvidenceRescue(message)) {
      const rescueMessage=`Recupera evidencia verificable y responde sólo con lo que pueda sostenerse con las fuentes disponibles. Si no hay síntesis generativa, entrega una respuesta estructurada basada en evidencia sin inventar conclusiones.\n\n${enrichedMessage}`;
      data=await edgeChat({boot,message:rescueMessage,mode:'research',webEnabled:true});
...
function gatewayHeaders() { return {'Authorization':`Bearer ${process.env.WAE_SUPABASE_MACHINE_KEY}`,'Content-Type':'application/json','X-Client-Info':'wae-inteligencia-universal-render/1.6.0'}; }
...
async function waeSupabaseResponse({ system, message, history }) {
  const messages = [{ role:'system', content:system },...cleanHistory(history),{ role:'user', content:message }];
  const initial = await jsonFetch(gatewayUrl(), {method:'POST',headers:gatewayHeaders(),body:JSON.stringify(gatewayBody('chat', {input:message,messages,wait_ms:12000,auto_research:true,context:{ surface:'inteligencia_universal_render', channel:'web', sensitivity:'INTERNAL' }}))}, 30000);
...
async function openAIResponse({ model, system, message, history }) { const input=[{role:'system',content:system},...cleanHistory(history),{role:'user',content:message}];const data=await jsonFetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model,input})});const text=textFromOpenAI(data);if(!text)throw new Error('OpenAI devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
async function anthropicResponse({ model, system, message, history }) { const messages=[...cleanHistory(history),{role:'user',content:message}];const data=await jsonFetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01','Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:Number(process.env.ANTHROPIC_MAX_TOKENS||4096),system,messages})});const text=(data.content||[]).filter(x=>x.type==='text').map(x=>x.text).join('\n').trim();if(!text)throw new Error('Anthropic devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
async function geminiResponse({ model, system, message, history }) { const contents=cleanHistory(history).map(x=>({role:x.role==='assistant'?'model':'user',parts:[{text:x.content}]}));contents.push({role:'user',parts:[{text:message}]});const url=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(process.env.GEMINI_API_KEY)}`;const data=await jsonFetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:system}]},contents})});const text=(data.candidates?.[0]?.content?.parts||[]).map(p=>p.text||'').join('\n').trim();if(!text)throw new Error('Gemini devolvió una respuesta sin texto utilizable');return{text,usage:data.usageMetadata||null,responseId:data.responseId||null}; }
async function xaiResponse({ model, system, message, history }) { const data=await jsonFetch('https://api.x.ai/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${process.env.XAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model,input:[{role:'system',content:system},...cleanHistory(history),{role:'user',content:message}]})});const text=textFromOpenAI(data);if(!text)throw new Error('xAI devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
async function openRouterResponse({ model, system, message, history }) { const data=await jsonFetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENROUTER_API_KEY}`,'Content-Type':'application/json','HTTP-Referer':process.env.PUBLIC_APP_URL||'https://wae-inteligencia-universal.onrender.com','X-Title':'WAE Inteligencia Universal'},body:JSON.stringify({model,messages:[{role:'system',content:system},...cleanHistory(history),{role:'user',content:message}]})});const text=data.choices?.[0]?.message?.content?.trim();if(!text)throw new Error('OpenRouter devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
...
export async function generateWithFallback({ provider='auto', system, message, history=[] }) {
  ...
  for(const p of ordered){try{const result=await callers[p.id]({model:p.model,system:systemWithContract,message,history});const text=guardFinalOutput(result.text);return{...result,text,provider:result.provider||p.id,model:result.model||p.model,failures}}catch(error){failures.push({provider:p.id,model:p.model,error:String(error.message||error).slice(0,500)})}}
