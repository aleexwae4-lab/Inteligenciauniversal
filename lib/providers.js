...
async function edgeChat({boot,message,mode,webEnabled,attachments=[]}) {
...
async function waeUniversalEdgeResponse({ message, history }) {
  ...
  try {
    data=await edgeChat({boot,message:coreQuery,mode:route.mode,webEnabled:route.webEnabled,attachments});
  } catch (error) {
    ...
    if (!route.webEnabled && retryable && shouldEvidenceRescue(coreQuery)) {
      data=await edgeChat({boot,message:coreQuery,mode:'research',webEnabled:true,attachments});
