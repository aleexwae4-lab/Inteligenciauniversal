...
export async function warmProviderConnections() {
  if (!edgeConfigured()) return { edge:{ configured:false, ready:false } };
  try {
    await bootstrapEdgeSession();
    return { edge:{ configured:true, ready:true } };
  } catch (error) {
    console.warn('[Provider warmup]', JSON.stringify({ provider:'wae_edge', ready:false, error:String(error?.message || error).slice(0,160) }));
    return { edge:{ configured:true, ready:false, error:String(error?.code || error?.message || 'warmup_failed').slice(0,120) } };
  }
}
