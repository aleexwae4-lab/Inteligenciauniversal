import { openAIInputParts, anthropicContentParts, geminiParts, multimodalPromptSuffix, multimodalCapabilities } from './multimodal-input-v1.js';
...
async function edgeChat({boot,message,mode,webEnabled,internalContext='',attachments=[]}) {
  return jsonFetch(edgeUrl(), {
    method:'POST',headers:edgeHeaders(),
    body:JSON.stringify({action:'chat',session_id:boot.session_id,session_secret:boot.session_secret,message,mode,web_enabled:webEnabled,internal_context:String(internalContext||'').slice(0,24000),routing_variant:'candidate',attachments})
  }, 60000);
}

async function waeUniversalEdgeResponse({ message, history, attachments=[] }) {
  ...
  let data;
  try {
    data=await edgeChat({boot,message:coreQuery,mode:route.mode,webEnabled:route.webEnabled,internalContext,attachments});
  } catch (error) {
...
async function waeSupabaseResponse({ system, message, history, attachments=[] }) {
  const coreQuery=extractCoreUserQuery(message);
  const messages = [{ role:'system', content:system },...cleanHistory(history),{ role:'user', content:coreQuery }];
  const initial = await jsonFetch(gatewayUrl(), {method:'POST',headers:gatewayHeaders(),body:JSON.stringify(gatewayBody('chat', {input:coreQuery,messages,wait_ms:12000,auto_research:true,context:{ surface:'inteligencia_universal_render', channel:'web', sensitivity:'INTERNAL', multimodal:multimodalCapabilities(attachments), attachments }}))}, 30000);
...
export async function generateWithFallback({ provider='auto', system, message, history=[], attachments=[] }) {
  ...
  for(const p of ordered){
    try{
      const result=await callers[p.id]({model:p.model,system:systemWithContract,message,history,attachments});
