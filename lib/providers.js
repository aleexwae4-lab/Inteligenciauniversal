...
function guardFinalOutput(raw) {
  ...
  for (const tag of tags) {
    ...
    if (idx > lastClose) { lastClose = idx; lastEnd = idx + marker.length; }
  }
...
function edgeHeaders() {
  return {
    ...
    'X-Client-Info':'wae-inteligencia-universal-render-fallback/1.5'
  };
}
...
async function waeUniversalEdgeResponse({ message, history }) {
  ...
  const data = await jsonFetch(edgeUrl(), {
    ...
    body:JSON.stringify({
      action:'chat',session_id:boot.session_id,session_secret:boot.session_secret,message:enrichedMessage,
      mode:route.mode,web_enabled:route.webEnabled,routing_variant:'candidate',attachments:[]
    })
  }, 60000);

  const text = guardFinalOutput(data?.reply);
  return {
    text,usage:null,responseId:data.conversation_id || null,provider:data.provider || 'wae_edge',model:data.model || 'iu-gpt-runtime-v13',edgeRuntime:data.runtime || null,
    webSources:Array.isArray(data.web_sources)?data.web_sources:(Array.isArray(data?.response?.sources)?data.response.sources:[]),
    routingPath:data.routing_path || null,taskCategory:data.task_category || null,routerVariant:data.router_variant || 'candidate'
  };
}
...
function gatewayHeaders() { return {'Authorization':`Bearer ${process.env.WAE_SUPABASE_MACHINE_KEY}`,'Content-Type':'application/json','X-Client-Info':'wae-inteligencia-universal-render/1.5'}; }
...
async function waeSupabaseResponse({ system, message, history }) {
  ...
  const initial = await jsonFetch(gatewayUrl(), {
    method:'POST',headers:gatewayHeaders(),body:JSON.stringify(gatewayBody('chat', {input:message,messages,wait_ms:12000,auto_research:true,context:{ surface:'inteligencia_universal_render', channel:'web', sensitivity:'INTERNAL' }}))
  }, 30000);
...
async function openAIResponse({ model, system, message, history }) {
  ...
  const data = await jsonFetch('https://api.openai.com/v1/responses', {method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model,input})});
  const text = textFromOpenAI(data); if (!text) throw new Error('OpenAI devolvió una respuesta sin texto utilizable');
...
async function anthropicResponse({ model, system, message, history }) {
  ...
  const data = await jsonFetch('https://api.anthropic.com/v1/messages', {method:'POST',headers:{'x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01','Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:Number(process.env.ANTHROPIC_MAX_TOKENS || 4096),system,messages})});
  const text = (data.content || []).filter(x=>x.type==='text').map(x=>x.text).join('\n').trim(); if (!text) throw new Error('Anthropic devolvió una respuesta sin texto utilizable');
  return { text, usage:data.usage || null, responseId:data.id || null };
}

async function geminiResponse({ model, system, message, history }) {
  const contents = cleanHistory(history).map(x=>({role:x.role==='assistant'?'model':'user',parts:[{text:x.content}]})); contents.push({role:'user',parts:[{text:message}]});
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(process.env.GEMINI_API_KEY)}`;
  const data = await jsonFetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:system}]},contents})});
  const text = (data.candidates?.[0]?.content?.parts || []).map(p=>p.text || '').join('\n').trim(); if (!text) throw new Error('Gemini devolvió una respuesta sin texto utilizable');
...
async function xaiResponse({ model, system, message, history }) {
  const data = await jsonFetch('https://api.x.ai/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${process.env.XAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model,input:[{role:'system',content:system},...cleanHistory(history),{role:'user',content:message}]})});
  const text = textFromOpenAI(data); if (!text) throw new Error('xAI devolvió una respuesta sin texto utilizable');
  return { text, usage:data.usage || null, responseId:data.id || null };
}

async function openRouterResponse({ model, system, message, history }) {
  const data = await jsonFetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENROUTER_API_KEY}`,'Content-Type':'application/json','HTTP-Referer':process.env.PUBLIC_APP_URL || 'https://wae-inteligencia-universal.onrender.com','X-Title':'WAE Inteligencia Universal'},body:JSON.stringify({model,messages:[{role:'system',content:system},...cleanHistory(history),{role:'user',content:message}]})});
  const text = data.choices?.[0]?.message?.content?.trim(); if (!text) throw new Error('OpenRouter devolvió una respuesta sin texto utilizable');
  return { text, usage:data.usage || null, responseId:data.id || null };
}
...
export async function generateWithFallback({ provider='auto', system, message, history=[] }) {
  ...
  const ordered = provider === 'auto' ? registry.filter(x=>x.configured) : [...registry.filter(x=>x.id===provider&&x.configured),...registry.filter(x=>x.id!==provider&&x.configured)];
