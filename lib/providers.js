...
async function edgeChat({boot,message,mode,webEnabled,internalContext=''}) {
  return jsonFetch(edgeUrl(), {
    ...
    body:JSON.stringify({action:'chat',session_id:boot.session_id,session_secret:boot.session_secret,message,mode,web_enabled:webEnabled,internal_context:String(internalContext||'').slice(0,24000)})
  }, 60000);
}
...
async function waeUniversalEdgeResponse({ message, history }) {
  ...
  const data=await edgeChat({boot,message:coreQuery,mode:route.mode,webEnabled:route.webEnabled,internalContext});
