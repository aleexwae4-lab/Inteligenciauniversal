...
function guardFinalOutput(raw) {
  ...
  if (hasUnclosedInternalTag || promptLeak || internalPlanning || !text) {
    throw new Error('unsafe_reasoning_output');
  }
  return text;
}
...
function edgeHeaders() {
  return {
    ...
    'X-Client-Info':'wae-inteligencia-universal-render-fallback/1.3'
  };
}
...
async function waeUniversalEdgeResponse({ message, history }) {
  ...
  return {
    ...
    model:data.model || 'iu-gpt-runtime-v13',
...
function gatewayHeaders() {
  return {
    ...
    'Content-Type':'application/json',
    'X-Client-Info':'wae-inteligencia-universal-render/1.3'
  };
}
...
async function waeSupabaseResponse({ system, message, history }) {
  ...
  const initial = await jsonFetch(gatewayUrl(), {
    ...
    body:JSON.stringify(gatewayBody('chat', {
      ...
      wait_ms:12000,
      auto_research:true,
      context:{ surface:'inteligencia_universal_render', channel:'web', sensitivity:'INTERNAL' }
    }))
  }, 30000);

  const direct = String(initial?.output_text || initial?.answer || '').trim();
  if (direct) return { text:guardFinalOutput(direct), usage:null, responseId:initial?.id || initial?.runtime_request_id || null };
  ...
  while (Date.now() < deadline) {
    ...
    if (text) return { text:guardFinalOutput(text), usage:null, responseId:requestId };
...
const callers = {
  wae_edge:waeUniversalEdgeResponse,
