...
async function waeUniversalEdgeResponse({ message, history, requestKey }) {
  let boot = await bootstrapEdgeSession();
  const clientRequestId=typeof requestKey==='string'&&/^[A-Za-z0-9_-]{16,128}$/.test(requestKey)?requestKey:'wae_'+crypto.randomUUID().replaceAll('-','');
  const coreQuery=extractCoreUserQuery(message);
  ...
  let data;
  try {
    data=await edgeChat({boot,message:coreQuery,mode:route.mode,webEnabled:route.webEnabled,internalContext,clientRequestId});
  } catch (error) {
...
export async function generateWithFallback({ provider='auto', system, message, history=[], requestKey=null }) {
  ...
  for(const p of ordered){
    try{
      const result=await callers[p.id]({model:p.model,system:systemWithContract,message,history,requestKey});
