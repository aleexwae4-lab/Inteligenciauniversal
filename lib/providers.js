...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false, qualityGate=null, budgetMs=null, retryColdStart=false, attemptTimeoutMs=null, onProviderEvent=null }) {
  const registry = providerRegistry();
  ...
  const failures = [];
  const complexMode = ['research','analysis','code','design','executive'].includes(String(mode||'').toLowerCase()) || webEnabled===true;
  const effectiveBudgetMs = Number.isFinite(Number(budgetMs)) ? Number(budgetMs) : (complexMode ? 30000 : 24000);
  const effectiveAttemptTimeoutMs = Number.isFinite(Number(attemptTimeoutMs)) ? Number(attemptTimeoutMs) : (complexMode ? 14000 : 10000);
  // A latency firewall prevents one unhealthy upstream from consuming the whole interactive turn.
  ...
  const queue=[...ordered,...(retryColdStart&&edge&&ordered.length===1?[edge]:[])];
  let edgeTimedOut=false;
  const deadline=Date.now()+Math.min(30000,Math.max(12000,effectiveBudgetMs));
  for (let index=0;index<queue.length;index++) {
    ...
    const internalProvider=p.id==='wae_edge'||p.id==='wae_supabase';
    if(!healing.allowed){
      ...
      // Internal WAE engines get one bounded recovery probe before continuity.
      // A quarantine is a routing hint, not proof that the runtime is dead.
    ...
    if(providerCircuitOpen(p.id)){
      ...
      // Internal providers receive one bounded probe. This prevents a stale
      // breaker from forcing Continuity while the underlying runtime is healthy.
      if(!internalProvider) continue;
    }
    const remaining=deadline-Date.now();
    if(remaining<1500)break;
    const controller=new AbortController();
    const providerAttemptDefault=p.id==='wae_supabase'?14000:(p.id==='wae_edge'?10000:effectiveAttemptTimeoutMs);
    const maxAttempt=Math.min(15000,Math.max(20,providerAttemptDefault));
