...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false }) {
  ...
  for (const p of ordered) {
    try {
      ...
      const reason = nonGenerativeReason({ ...result, text:result.text, provider:result.provider || p.id, model:result.model || p.model });
      if (reason) throw Object.assign(new Error('non_generative_response:'+reason),{code:'NON_GENERATIVE_RESPONSE'});
