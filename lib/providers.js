...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false, qualityGate=null, budgetMs=38000, retryColdStart=false, attemptTimeoutMs=16500 }) {
  ...
  const deadline=Date.now()+Math.min(50000,Math.max(6000,Number(budgetMs)||38000));
  for (let index=0;index<queue.length;index++) {
    ...
    const maxAttempt=Math.min(16500,Math.max(20,Number(attemptTimeoutMs)||16500));
