...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false, qualityGate=null, budgetMs=null, retryColdStart=false, attemptTimeoutMs=null, onProviderEvent=null }) {
  ...
  const effectiveBudgetMs = Number.isFinite(Number(budgetMs)) ? Number(budgetMs) : (complexMode ? 24000 : 14000);
  const effectiveAttemptTimeoutMs = Number.isFinite(Number(attemptTimeoutMs)) ? Number(attemptTimeoutMs) : (complexMode ? 12000 : 7000);
  ...
  const deadline=Date.now()+Math.min(30000,Math.max(6000,effectiveBudgetMs));
  for (let index=0;index<queue.length;index++) {
    ...
    const maxAttempt=Math.min(15000,Math.max(20,effectiveAttemptTimeoutMs));
