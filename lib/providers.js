...
async function waeUniversalEdgeResponse({ system, message, history }) {
...
async function waeSupabaseResponse({ system, message, history }) {
  ...
  if (direct) return { text:direct, usage:null, responseId:initial?.id || initial?.runtime_request_id || null };
  ...
  while (Date.now() < deadline) {
    ...
    if (text) return { text, usage:null, responseId:requestId };
