import { nonGenerativeReason } from './non-generative-guard.js';
...
async function waeUniversalEdgeResponse({ system, message, history, mode='general', webEnabled=false }) {
  const boot = await jsonFetch(edgeUrl(), {
    method:'POST', headers:edgeHeaders(), body:JSON.stringify({ action:'bootstrap' })
  }, 20000);
  ...
  // Preserve specialist guidance even for the first turn of a new Canvas request.
  const enrichedMessage = `Instrucciones del runtime (no son parte de la solicitud del usuario):\n${system.slice(0,6000)}\n\n${recent?`Conversación reciente:\n${recent}\n\n`:''}Solicitud actual del usuario:\n${message}`;
...
async function waeSupabaseResponse({ system, message, history }) {
  ...
  while (Date.now() < deadline) {
    ...
    last = await jsonFetch(gatewayUrl(), {
      method:'POST', headers:gatewayHeaders(), body:JSON.stringify(gatewayBody('status',{ runtime_request_id:requestId }))
    },15000);
...
export function providerRegistry() {
  return [
    { id:'wae_edge', configured:edgeConfigured(), model:'iu-gpt-runtime-v11' },
...
async function openAIResponse({ model, system, message, history }) {
  ...
  const data = await jsonFetch('https://api.openai.com/v1/responses', {
    method:'POST', headers:{'Authorization':`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'}, body:JSON.stringify({model,input})
  });
...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false }) {
  ...
  for (const p of ordered) {
    try {
      const result = await callers[p.id]({model:p.model,system,message,history,mode,webEnabled});
      const qualityFailure = degradedAnswer(result?.text);
      if (qualityFailure) {
        failures.push({provider:p.id,model:p.model,error:qualityFailure});
        continue;
      }
      const boundary=nonGenerativeReason({ ...result, provider:result.provider || p.id, model:result.model || p.model });
      if(boundary){failures.push({provider:p.id,model:p.model,error:'non_generative_response:'+boundary});continue;}
