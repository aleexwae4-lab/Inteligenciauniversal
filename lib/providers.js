...
async function edgeChat({boot,message,mode,webEnabled,internalContext='',clientRequestId}) {
  return jsonFetch(edgeUrl(), {
    method:'POST',headers:edgeHeaders(),
    body:JSON.stringify({action:'chat',session_id:boot.session_id,session_secret:boot.session_secret,message,mode,web_enabled:webEnabled,internal_context:String(internalContext||'').slice(0,24000),routing_variant:'candidate',client_request_id:clientRequestId})
  }, 60000);
}

async function waeUniversalEdgeResponse({ message, history, clientRequestId }) {
  let boot = await bootstrapEdgeSession();
  const stableClientRequestId=/^[A-Za-z0-9_-]{16,128}$/.test(String(clientRequestId||''))?String(clientRequestId):'wae_'+crypto.randomUUID().replaceAll('-','');
  const coreQuery=extractCoreUserQuery(message);
  ...
  let data;
  try {
    data=await edgeChat({boot,message:coreQuery,mode:route.mode,webEnabled:route.webEnabled,internalContext,clientRequestId:stableClientRequestId});
  } catch (error) {
...
export async function generateWithFallback({ provider='auto', system, message, history=[], clientRequestId=null }) {
  ...
  for(const p of ordered){
    try{
      const result=await callers[p.id]({model:p.model,system:systemWithContract,message,history,clientRequestId});
