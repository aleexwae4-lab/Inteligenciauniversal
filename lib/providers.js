...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false, qualityGate=null, budgetMs=35000 }) {
  ...
  const failures = [];
  const deadline=Date.now()+Math.min(50000,Math.max(6000,Number(budgetMs)||35000));
  for (const p of ordered) {
    const remaining=deadline-Date.now();
    if(remaining<1500)break;
    const controller=new AbortController();
    const timer=setTimeout(()=>controller.abort(),Math.min(16500,remaining));
    try {
