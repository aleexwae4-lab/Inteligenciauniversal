...
function edgeHeaders() { return {'Content-Type':'application/json','apikey':process.env.SUPABASE_PUBLISHABLE_KEY,'Origin':process.env.PUBLIC_APP_URL || 'https://wae-inteligencia-universal.onrender.com','X-Client-Info':'wae-inteligencia-universal-render-fallback/1.5.1'}; }
function edgeRequestPolicy(message=''){
...
async function waeUniversalEdgeResponse({ message, history }) {
  ...
  const data = await jsonFetch(edgeUrl(), {
    ...
    body:JSON.stringify({action:'chat',session_id:boot.session_id,session_secret:boot.session_secret,message:enrichedMessage,mode:route.mode,web_enabled:route.webEnabled,attachments:[]})
  }, 60000);
  const text = guardFinalOutput(data?.reply);
  return {text,usage:null,responseId:data.conversation_id || null,provider:data.provider || 'wae_edge',model:data.model || 'iu-gpt-runtime-v13',edgeRuntime:data.runtime || null,webSources:Array.isArray(data.web_sources)?data.web_sources:(Array.isArray(data?.response?.sources)?data.response.sources:[]),routingPath:data.routing_path || null,taskCategory:data.task_category || null,routerVariant:data.router_variant || null};
}
...
function gatewayHeaders() { return {'Authorization':`Bearer ${process.env.WAE_SUPABASE_MACHINE_KEY}`,'Content-Type':'application/json','X-Client-Info':'wae-inteligencia-universal-render/1.5.1'}; }
...
export async function generateWithFallback({ provider='auto', system, message, history=[] }) {
  ...
  const failures=[];
  for(const p of ordered){try{const result=await callers[p.id]({model:p.model,system,message,history});const text=guardFinalOutput(result.text);return{...result,text,provider:result.provider||p.id,model:result.model||p.model,failures}}catch(error){failures.push({provider:p.id,model:p.model,error:String(error.message||error).slice(0,500)})}}
