...
export async function generateWithFallback({ provider='auto', system, message, history=[], mode='general', webEnabled=false, qualityGate=null, budgetMs=46000, retryColdStart=false, attemptTimeoutMs=38000, onProviderEvent=null }) {
  ...
  for (let index=0;index<queue.length;index++) {
    ...
    try {
      onProviderEvent?.({type:'attempt',provider:p.id,model:p.model,streaming:!!p.streaming});
      const caller=p.streaming&&streamCallers[p.id]?streamCallers[p.id]:callers[p.id];
      const result = await caller({model:p.model,system,message,history,mode,webEnabled,signal:controller.signal,onProviderEvent});
      ...
      if (qualityFailure) {
        ...
        onProviderEvent?.({type:'quality_rejected',provider:p.id,model:p.model,code:qualityFailure});
        continue;
      }
      providerCircuitSuccess(p.id);
      onProviderEvent?.({type:'complete',provider:result.provider||p.id,model:result.model||p.model,streaming:!!result.streaming,ttftMs:result.streaming?.ttftMs??null,chunks:result.streaming?.chunks??0});
      ...
      onProviderEvent?.({type:'failed',provider:p.id,model:p.model,code});
