import { deriveTruthSpeedPolicy } from './truth-speed.js';

const timeout = (ms = 30000) => AbortSignal.timeout(Math.max(1000,Number(ms)||30000));
...
async function jsonFetch(url, options, ms = 30000) {
...
export function extractCoreUserQuery(value='') {
  ...
  const boundary='SOLICITUD DEL USUARIO:';
  const boundaryIndex=text.lastIndexOf(boundary);
  if(boundaryIndex>=0)text=text.slice(boundaryIndex+boundary.length).trim();
...
export function adaptiveResponseContract(message='') {
  ...
  if (creative) return `CONTRATO DE RESPUESTA: entrega directamente el artefacto solicitado. No sustituyas el contenido por explicaciones sobre lo que podrías hacer. ${calibration}`;
  if (history) return `CONTRATO DE RESPUESTA: cuenta la historia de forma cronológica en 3 a 5 fases significativas, priorizando causalidad y hechos sobre detalle decorativo. ${calibration}`;
  if (factual) return `CONTRATO DE RESPUESTA: responde la pregunta factual inmediatamente en un párrafo corto o pocas viñetas. No conviertas una pregunta simple en un informe ejecutivo. ${calibration}`;
  if (analytical) return `CONTRATO DE RESPUESTA: abre con la conclusión, después evidencia, supuestos, riesgos, trade-offs y acciones concretas. ${calibration}`;
  ...
  return `CONTRATO DE RESPUESTA: usa la mínima estructura necesaria para ser claro. Sé breve en preguntas simples y amplía sólo cuando la tarea requiera análisis o implementación. ${calibration}`;
}
...
function edgeHeaders() { return {'Content-Type':'application/json','apikey':process.env.SUPABASE_PUBLISHABLE_KEY,'Origin':process.env.PUBLIC_APP_URL || 'https://wae-inteligencia-universal.onrender.com','X-Client-Info':'wae-inteligencia-universal-render-fallback/1.6.1'}; }
export function edgeRequestPolicy(message=''){
  const q=extractCoreUserQuery(message);
  const truth=deriveTruthSpeedPolicy({message:q,mode:'general',provider:'auto'});
  const explicitResearch=/\b(investiga|investigación|fuentes?|citas?|verifica|benchmark|estadística|mercado|competidor|buscar en (?:la )?web)\b|\b(?:con|usa|aporta|incluye)\s+evidencia\b|\bevidencia\s+(?:web|externa|verificable|actual|de fuentes?)\b/i.test(q);
  const research=truth.evidenceRequired||explicitResearch;
...
async function edgeChat({boot,message,mode,webEnabled,attachments=[],timeoutMs=30000}) {
  return jsonFetch(edgeUrl(), {
    method:'POST',headers:edgeHeaders(),
    body:JSON.stringify({action:'chat',session_id:boot.session_id,session_secret:boot.session_secret,message,mode,web_enabled:webEnabled,attachments})
  }, timeoutMs);
}

async function waeUniversalEdgeResponse({ message, history, timeoutMs=30000 }) {
  const deadline=Date.now()+Math.max(6000,Number(timeoutMs)||30000),remaining=()=>Math.max(1500,deadline-Date.now());
  const boot = await jsonFetch(edgeUrl(), { method:'POST', headers:edgeHeaders(), body:JSON.stringify({ action:'bootstrap' }) }, Math.min(8000,remaining()));
  ...
  const attachments=[];
  if (recent) attachments.push({name:'conversation-context.txt',type:'text/plain',text:recent.slice(0,24000)});
  if (String(message||'').trim()!==coreQuery) attachments.push({name:'runtime-context.txt',type:'text/plain',text:String(message||'').slice(0,24000)});
  const route=edgeRequestPolicy(coreQuery);
  let data,rescuePath=null;
  try {
    data=await edgeChat({boot,message:coreQuery,mode:route.mode,webEnabled:route.webEnabled,attachments,timeoutMs:remaining()});
  } catch (error) {
    const status=Number(error?.status||0);
    const retryable=[429,500,502,503,504].includes(status) || /all_models_unavailable|provider|timeout|temporar|abort/i.test(String(error?.message||''));
    if (!route.webEnabled && retryable && shouldEvidenceRescue(coreQuery) && remaining()>4000) {
      data=await edgeChat({boot,message:coreQuery,mode:'research',webEnabled:true,attachments,timeoutMs:remaining()});
      rescuePath='automatic_evidence_rescue';
    } else {
      throw error;
    }
  }
  const text = guardFinalOutput(data?.reply);
  return {text,usage:null,responseId:data.conversation_id || null,provider:data.provider || 'wae_edge',model:data.model || 'iu-gpt-runtime-v13',edgeRuntime:data.runtime || null,webSources:Array.isArray(data.web_sources)?data.web_sources:(Array.isArray(data?.response?.sources)?data.response.sources:[]),routingPath:data.routing_path || null,taskCategory:data.task_category || null,routerVariant:data.router_variant || null,rescuePath,degraded:data?.degraded===true};
}
...
function gatewayHeaders() { return {'Authorization':`Bearer ${process.env.WAE_SUPABASE_MACHINE_KEY}`,'Content-Type':'application/json','X-Client-Info':'wae-inteligencia-universal-render/1.6.1'}; }
...
async function waeSupabaseResponse({ system, message, history, timeoutMs=30000 }) {
  const deadline=Date.now()+Math.max(6000,Number(timeoutMs)||30000),remaining=()=>Math.max(1500,deadline-Date.now());
  const coreQuery=extractCoreUserQuery(message);
  const messages = [{ role:'system', content:system },...cleanHistory(history),{ role:'user', content:coreQuery }];
  const initial = await jsonFetch(gatewayUrl(), {method:'POST',headers:gatewayHeaders(),body:JSON.stringify(gatewayBody('chat', {input:coreQuery,messages,wait_ms:Math.min(10000,remaining()),auto_research:true,context:{ surface:'inteligencia_universal_render', channel:'web', sensitivity:'INTERNAL' }}))}, Math.min(remaining(),18000));
  ...
  while (remaining()>1800) {
    await new Promise(resolve => setTimeout(resolve,Math.min(600,Math.max(100,remaining()-1500))));
    last = await jsonFetch(gatewayUrl(), {method:'POST',headers:gatewayHeaders(),body:JSON.stringify(gatewayBody('status',{ runtime_request_id:requestId }))},Math.min(remaining(),6000));
...
async function openAIResponse({ model, system, message, history, timeoutMs=30000 }) { const input=[{role:'system',content:system},...cleanHistory(history),{role:'user',content:extractCoreUserQuery(message)}];const data=await jsonFetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model,input})},timeoutMs);const text=textFromOpenAI(data);if(!text)throw new Error('OpenAI devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
async function anthropicResponse({ model, system, message, history, timeoutMs=30000 }) { const messages=[...cleanHistory(history),{role:'user',content:extractCoreUserQuery(message)}];const data=await jsonFetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01','Content-Type':'application/json'},body:JSON.stringify({model,max_tokens:Number(process.env.ANTHROPIC_MAX_TOKENS||4096),system,messages})},timeoutMs);const text=(data.content||[]).filter(x=>x.type==='text').map(x=>x.text).join('\n').trim();if(!text)throw new Error('Anthropic devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
async function geminiResponse({ model, system, message, history, timeoutMs=30000 }) { const contents=cleanHistory(history).map(x=>({role:x.role==='assistant'?'model':'user',parts:[{text:x.content}]}));contents.push({role:'user',parts:[{text:extractCoreUserQuery(message)}]});const url=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(process.env.GEMINI_API_KEY)}`;const data=await jsonFetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:system}]},contents})},timeoutMs);const text=(data.candidates?.[0]?.content?.parts||[]).map(p=>p.text||'').join('\n').trim();if(!text)throw new Error('Gemini devolvió una respuesta sin texto utilizable');return{text,usage:data.usageMetadata||null,responseId:data.responseId||null}; }
async function xaiResponse({ model, system, message, history, timeoutMs=30000 }) { const data=await jsonFetch('https://api.x.ai/v1/responses',{method:'POST',headers:{'Authorization':`Bearer ${process.env.XAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model,input:[{role:'system',content:system},...cleanHistory(history),{role:'user',content:extractCoreUserQuery(message)}]})},timeoutMs);const text=textFromOpenAI(data);if(!text)throw new Error('xAI devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
async function openRouterResponse({ model, system, message, history, timeoutMs=30000 }) { const data=await jsonFetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Authorization':`Bearer ${process.env.OPENROUTER_API_KEY}`,'Content-Type':'application/json','HTTP-Referer':process.env.PUBLIC_APP_URL||'https://wae-inteligencia-universal.onrender.com','X-Title':'WAE Inteligencia Universal'},body:JSON.stringify({model,messages:[{role:'system',content:system},...cleanHistory(history),{role:'user',content:extractCoreUserQuery(message)}]})},timeoutMs);const text=data.choices?.[0]?.message?.content?.trim();if(!text)throw new Error('OpenRouter devolvió una respuesta sin texto utilizable');return{text,usage:data.usage||null,responseId:data.id||null}; }
...
export async function generateWithFallback({ provider='auto', system, message, history=[] }) {
  const registry=providerRegistry(),all=provider==='auto'?registry.filter(x=>x.configured):[...registry.filter(x=>x.id===provider&&x.configured),...registry.filter(x=>x.id!==provider&&x.configured)];
  if(!all.length){const err=new Error('No hay proveedores IA configurados en el servidor');err.code='NO_PROVIDER';throw err}
  const route=edgeRequestPolicy(extractCoreUserQuery(message)),policy=deriveTruthSpeedPolicy({message:extractCoreUserQuery(message),mode:route.mode,provider});
  const ordered=all.slice(0,policy.maxProviderAttempts),deadline=Date.now()+policy.timeoutMs;
  const failures=[],systemWithContract=`${system}\n\n${adaptiveResponseContract(message)}`;
  for(const p of ordered){
    const remaining=deadline-Date.now();
    if(remaining<1800)break;
    const attemptCap=policy.complexity==='fast'?12000:policy.complexity==='standard'?18000:25000;
    const attemptMs=Math.max(1500,Math.min(attemptCap,remaining));
    try{
      const result=await callers[p.id]({model:p.model,system:systemWithContract,message,history,timeoutMs:attemptMs});
      const text=guardFinalOutput(result.text);
      return{...result,text,provider:result.provider||p.id,model:result.model||p.model,failures,latencyPolicy:{version:policy.version,complexity:policy.complexity,budgetMs:policy.timeoutMs,attemptMs,maxProviderAttempts:policy.maxProviderAttempts}};
    }catch(error){
      failures.push({provider:p.id,model:p.model,error:String(error.message||error).slice(0,500),attemptMs});
    }
  }
  const err=new Error(Date.now()>=deadline?'Presupuesto de latencia agotado':'Todos los proveedores configurados fallaron');err.code=Date.now()>=deadline?'LATENCY_BUDGET_EXHAUSTED':'ALL_PROVIDERS_FAILED';err.failures=failures;err.latencyPolicy=policy;throw err;
}
