...
const COUNCIL_MODES=new Set(['analysis','executive','code','design']);
...
export function councilEligible({route,body={}}={}){
  ...
  if(body.council_mode===true)return true;
  if(!COUNCIL_MODES.has(mode))return false;
  return task.path==='DEEP';
}
...
export async function deliberateMission({body={},userKey='',route}={}){
  ...
  const userContextState=userContextStateV92(body),userContext=publicUserContextV92(userContextState);

  const memory=await recallMemory(String(userKey||''),message,4);
  const settled=await Promise.allSettled(providers.map(provider=>generateCandidate({provider,message,history:body.history,memory,preferences:body.preferences})));
  const failures=[],raw=[];
  ...
  try{
    ...
    const synthesized=await generateWithFallback({
      provider:alternate,
      system:synthesisSystem(message,body.preferences),
  ...
  response.metadata={...response.metadata,council,quality,userContext};
  ...
  return{
    ...
    usage:final.generated?.usage||null,latencyMs,fallbackFailures:failures,quality,user_context:userContext,repair_attempted:false,fast_lane:false,
