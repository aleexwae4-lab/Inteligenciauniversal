...
export function buildTruthConflictEngine(attachments=[],findings=[]){
  ...
  const fs=(Array.isArray(findings)?findings:[]).filter(f=>findingText(f)).slice(0,120).map(f=>({...f,sourceFileIds:Array.isArray(f.sourceFileIds)?f.sourceFileIds:(f.sourceFileId?[f.sourceFileId]:[])})).filter(f=>f.sourceFileIds.length);
  const signals=[];
