...
export function evaluateAnswer({question='',answer='',mode='general',sources=[]}={}){
  ...
  // Never grant a short definition question a relevance floor when its key
  // concept is missing from the candidate answer (e.g. neuroplasticidad vs
  // maladaptación). Long/creative prompts retain the existing soft floor.
  const definition=responseProfile(q)==='factual'&&/^(?:[¿?\s]*)?(?:qué|que|what)\s+(?:es|significa|is|does)\b/i.test(q);
  const missingDefinitionTopic=definition&&qWords.length>0&&overlap===0;
  const definitionDeflected=definition&&/^(?:entiendo que (?:enviaste|envías|haces)|necesito (?:un poco )?m[aá]s contexto|para poder ayudarte|te refieres (?:al|a)|podr[ií]as (?:aclarar|especificar))/i.test(a.replace(/^[#*>\s]+/,''));
  let relevance=missingDefinitionTopic?.12:q.length<55?Math.max(.72,overlap):Math.max(.25,overlap);
  ...
  if(relevance<.45)score=Math.min(score,missingDefinitionTopic?.40:.55);
  if(definitionDeflected)score=Math.min(score,.2);
  ...
  if(missingDefinitionTopic)reasons.push('definition_topic_missing');
  if(definitionDeflected)reasons.push('definition_deflected');
