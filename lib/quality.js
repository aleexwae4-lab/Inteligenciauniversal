...
const EVIDENCE_RESEARCH_RX=/\b(?:busca|investiga|encuentra|mu[eé]strame|dame|aporta|cita|verifica)\b[\s\S]{0,60}\b(?:evidencia|evidence)\b|\b(?:evidencia|evidence)\b[\s\S]{0,40}\b(?:sobre|cient[ií]fica|actual|externa|fuentes?|sources?)\b/i;
...
export function inferCognitivePolicy(message='',requestedMode='general'){
  ...
  if(mode==='general'){
    if(RESEARCH_RX.test(q)||CURRENT_RX.test(q)||EVIDENCE_RESEARCH_RX.test(q))mode='research';
    else if(CODE_RX.test(q))mode='code';
    else if(DESIGN_RX.test(q))mode='design';
    else if(ANALYSIS_RX.test(q))mode='analysis';
  }
  const autoResearch=mode==='research'||CURRENT_RX.test(q)||RESEARCH_RX.test(q)||EVIDENCE_RESEARCH_RX.test(q);
...
export function evaluateAnswer({question='',answer='',mode='general',sources=[]}={}){
  ...
  let evidence=.9;
  const research=mode==='research'||CURRENT_RX.test(q)||RESEARCH_RX.test(q)||EVIDENCE_RESEARCH_RX.test(q);
  if(research){
