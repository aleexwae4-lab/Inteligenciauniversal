...
import { deriveTruthSpeedPolicy, auditGrounding, truthSpeedInstruction } from './truth-speed.js';

const CURRENT_RX=/\b(hoy|ahora|actual(?:es|idad|izado|izada)?|reciente|últim[oa]s?|latest|today|current|news|noticias|precio|cotización|jurisprudencia|reforma|ley vigente|verifica|fuentes?|web)\b/i;
...
export function inferCognitivePolicy(message='',requestedMode='general'){
  ...
  if(mode==='general'){
    if(RESEARCH_RX.test(q)||CURRENT_RX.test(q))mode='research';
    else if(CODE_RX.test(q))mode='code';
    else if(DESIGN_RX.test(q))mode='design';
    else if(ANALYSIS_RX.test(q))mode='analysis';
  }
  const autoResearch=mode==='research'||CURRENT_RX.test(q)||RESEARCH_RX.test(q);
  const requirements=deriveRequirementContract(q,mode);
  const truthSpeed=deriveTruthSpeedPolicy({message:q,mode});
  return {mode,autoResearch,path:autoResearch?'evidence-first':mode==='code'?'engineering':mode==='analysis'?'reasoning':mode==='design'?'product':'general',responseProfile:responseProfile(q),requirements,truthSpeed};
}
...
export function routingPrefix(policy={}){
  ...
  return `${truth}${profile}${calibration}${verified}${USER_BOUNDARY}`;
}
...
export function evaluateAnswer({question='',answer='',mode='general',sources=[]}={}){
  ...
  let evidence=.9;
  const research=mode==='research'||CURRENT_RX.test(q)||RESEARCH_RX.test(q);
  if(research){
  ...
  const truthSpeed=deriveTruthSpeedPolicy({message:q,mode});
  const grounding=auditGrounding({answer:a,sources,policy:truthSpeed});
  ...
  for(const reason of grounding.reasons)reasons.push(`grounding:${reason}`);
  if(requirementCoverage.coverage<.75)reasons.push('requirement_coverage_low');

  let score=.26*relevance+.16*length+.12*structure+.12*evidence+.08*(1-Math.min(1,repetition))+.16*requirements+.10*grounding.confidence;
  if(relevance<.45)score=Math.min(score,.55);
  if(requirementCoverage.hardFailure)score=Math.min(score,.59);
  else if(requirementCoverage.coverage<.6)score=Math.min(score,.62);
  if(grounding.hardFailure)score=Math.min(score,.35);
  ...
  const critical=score<.42||requirementCoverage.hardFailure||grounding.hardFailure;
  const pass=score>=.68&&!critical;
  return {
    schema:'universal-quality/v1',
    verifier:'truth-speed-governor/v36',
    ...
    signals:{
      ...
      grounding:Number(grounding.confidence.toFixed(3))
    }
  };
}
