...
export function inferCognitivePolicy(message='',requestedMode='general'){
  ...
  return {mode,autoResearch,path:autoResearch?'evidence-first':mode==='code'?'engineering':mode==='analysis'?'reasoning':mode==='design'?'product':'general',responseProfile:responseProfile(q)};
}
...
export function evaluateAnswer({question='',answer='',mode='general',sources=[]}={}){
  ...
  const relevance=q.length<55?Math.max(.72,overlap):Math.max(.25,overlap);
  ...
  let score=.32*relevance+.26*length+.18*structure+.14*evidence+.10*(1-Math.min(1,repetition));
