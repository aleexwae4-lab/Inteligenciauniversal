...
export function coreSelfResponse({providers=providerRegistry(),tools=toolRegistry(),memory=memoryStatus(),question=''}={}) {
  ...
  if(/quien te entreno/.test(q))
    return '**Soy Universal Core, la inteligencia artificial de WAE OS Enterprise.** Mi identidad de producto, comportamiento, orquestación, herramientas y experiencia pertenecen a Universal Core. Mi evolución se realiza mediante desarrollo, configuración, evaluación y personalización del sistema.';
  if(/(?:que|quien) eres/.test(q))
    return '**Soy Universal Core, la inteligencia artificial de WAE OS Enterprise.** Investigo, desarrollo software, analizo, diseño y opero proyectos contigo desde el Workspace.';
  ...
  const status=(web?'La búsqueda web del sistema está configurada.':'La búsqueda web del sistema no está verificada en esta ruta.')+(github?' La integración de GitHub está configurada.':'')+(memory?.configured?' La memoria persistente está configurada.':' La memoria persistente no está verificada en esta ruta.');
