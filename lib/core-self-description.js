...
import { capabilityProofSnapshot, capabilityProofLine } from './capability-proof-v153.js';

export const CORE_CAPABILITY_VERSION='universal-capability-registry/v153-proof-layer';
...
export function capabilitySnapshot({providers=providerRegistry(),tools=toolRegistry(),memory=memoryStatus()}={}) {
  ...
  const web=configured(tools,'waeweb_search')||configured(tools,'web_search');
  ...
  return {
    ...
    operational:{
      ...
      gameStudioV13:true,
      gameStudioV14:true,
      characterAnimationAssetPipeline:true,
      cinematicPresentationPipeline:true,
      questDialogueSaveEngine:true,
      combatInventoryEngine:true,
      npcEngine3D:true,
      gameplayLogicBuilder:true,
      worldBuilder3D:true,
      projects:true,
      temporaryCollaboration:collaboration.temporaryRooms===true
    },
    foundry:{version:'wae-product-foundry/v5',browserExecutable:['web_app','website','dashboard','presentation','game_2d','game_3d','simulation_3d','creative_tool','pwa','digital_product'],sourceTargets:['mobile_app','desktop_app','api_service','automation'],threeD:{engine:'WebGL/WebGL2 vanilla',studio:'wae-game-studio/v14',nativeFallback:true,sceneGraph:true,physics:'AABB/local',particles:true,webAudio:true,levelRuntime:true,sceneEditor:true,sceneHierarchy:true,entityInspector:true,transformXYZ:true,transformGizmo:true,materialEditor:true,sceneUndo:true,scenePersistence:true,multiSceneWorlds:true,prefabs:true,spawnPoints:true,triggers:true,sceneTransitions:true,missions:true,cameraEditor:true,lightingEditor:true,worldPersistence:true,visualLogicBuilder:true,eventRules:true,logicConditions:true,gameplayActions:true,logicPlaytest:true,logicPersistence:true,npcEngine:true,npcStateMachine:true,npcPerception:true,npcPatrol:true,npcChase:true,npcInteraction:true,npcDialogue:true,npcFactions:true,npcGoals:true,npcLogicEvents:true,npcPersistence:true,combatEngine:true,healthSystem:true,staminaSystem:true,inventory:true,consumables:true,pickups:true,lootTables:true,npcDamage:true,checkpoints:true,respawn:true,combatLogicEvents:true,combatPersistence:true,questEngine:true,questObjectives:true,questChains:true,dialogueTrees:true,dialogueChoices:true,storyVariables:true,journal:true,saveSlots:true,autosave:true,gameStateRestore:true,storyLogicEvents:true,storyPersistence:true,nativeAssetPipeline:true,proceduralAssets:true,characterController:true,thirdPersonController:true,animationStateMachine:true,proceduralRig:true,animationClips:true,animationTransitions:true,characterPrefabs:true,externalRuntimeRequired:false},export:['zip-source','wae-project-json','html-preview']},
    intelligence:{
...
function capabilityAnswer(snapshot){
  const proof=capabilityProofSnapshot({capabilityMatrix:snapshot});
  ...
  return [
    ...
    '- **Ingeniería, Fábrica de software y Product Foundry:** arquitectura, código, debugging, pruebas, APIs, backend/frontend y construcción de productos digitales. La Fábrica v5 puede generar juegos 2D/3D WebGL, simulaciones 3D, apps web/PWA, dashboards, presentaciones, herramientas creativas y paquetes fuente para móvil, escritorio, APIs y automatizaciones. Game Studio v13 conserva World Builder, Logic Builder, NPC Engine, Combat Engine y Quest/Dialogue/Save y añade Character, Animation & Asset Pipeline: personajes third-person, controlador, locomoción, rigs procedurales, clips, state machine, prefabs y catálogo de assets nativos; persiste assets.json/animation.json/character.json junto con story.json/savegame.json/combat.json/npc.json/logic.json/world.json/scene.json/main.js y conserva recuperación 3D nativa sin proveedores externos.',
    ...
    `- **Experiencia de trabajo:** Workspace, Canvas, Product Foundry, proyectos, ${memory}, evidencia visual, lectura por voz y colaboración temporal en tiempo real. ${github}.`,
    capabilityProofLine(proof)
  ].join('\n\n');
}
...
function advancedCapabilityAnswer(snapshot,history=[]){
  ...
  return [
    ...
    '- **Convertir una idea en activo:** especificación → arquitectura → implementación → QA → documentación → entrega en Workspace/Canvas/Product Foundry; incluye experiencias WebGL 3D ejecutables en navegador y paquetes fuente multi-target.',
...
export function coreSelfResponse({providers=providerRegistry(),tools=toolRegistry(),memory=memoryStatus(),question='',history=[]}={}) {
  ...
  return '**Soy Universal Core, la plataforma de inteligencia de WAE OS Enterprise.** Integro investigación, ingeniería, análisis, agentes y Workspace para convertir consultas en entregables y proyectos.\n\n'
    ...
    +'**Problemas complejos:** puedo abordar retos mundiales con evidencia mediante línea base, mapa del sistema, opciones, piloto, escala y verificación, sin fingir intervención material.\n\n'
