...
export const CORE_CAPABILITY_VERSION='universal-capability-registry/v151-combat-engine';
...
export function capabilitySnapshot({providers=providerRegistry(),tools=toolRegistry(),memory=memoryStatus()}={}) {
  ...
  return {
    ...
    foundry:{version:'wae-product-foundry/v5',browserExecutable:['web_app','website','dashboard','presentation','game_2d','game_3d','simulation_3d','creative_tool','pwa','digital_product'],sourceTargets:['mobile_app','desktop_app','api_service','automation'],threeD:{engine:'WebGL/WebGL2 vanilla',studio:'wae-game-studio/v11',nativeFallback:true,sceneGraph:true,physics:'AABB/local',particles:true,webAudio:true,levelRuntime:true,sceneEditor:true,sceneHierarchy:true,entityInspector:true,transformXYZ:true,transformGizmo:true,materialEditor:true,sceneUndo:true,scenePersistence:true,multiSceneWorlds:true,prefabs:true,spawnPoints:true,triggers:true,sceneTransitions:true,missions:true,cameraEditor:true,lightingEditor:true,worldPersistence:true,visualLogicBuilder:true,eventRules:true,logicConditions:true,gameplayActions:true,logicPlaytest:true,logicPersistence:true,npcEngine:true,npcStateMachine:true,npcPerception:true,npcPatrol:true,npcChase:true,npcInteraction:true,npcDialogue:true,npcFactions:true,npcGoals:true,npcLogicEvents:true,npcPersistence:true,combatEngine:true,healthSystem:true,staminaSystem:true,inventory:true,consumables:true,pickups:true,lootTables:true,npcDamage:true,checkpoints:true,respawn:true,combatLogicEvents:true,combatPersistence:true,externalRuntimeRequired:false},export:['zip-source','wae-project-json','html-preview']},
    intelligence:{
...
function capabilityAnswer(snapshot){
  ...
  return [
    ...
    '- **Ingeniería, Fábrica de software y Product Foundry:** arquitectura, código, debugging, pruebas, APIs, backend/frontend y construcción de productos digitales. La Fábrica v5 puede generar juegos 2D/3D WebGL, simulaciones 3D, apps web/PWA, dashboards, presentaciones, herramientas creativas y paquetes fuente para móvil, escritorio, APIs y automatizaciones. Game Studio v11 conserva World Builder, Logic Builder y NPC Engine y añade Combat, Health & Inventory: vida, stamina, inventario, consumibles, pickups, loot, daño NPC, checkpoints, respawn y eventos de combate conectados a la lógica; persiste combat.json junto con npc.json/logic.json/world.json/scene.json/main.js y conserva recuperación 3D nativa.',
