...
function sourceQuality(source={}){
  ...
  let score=.35;
  const isHttps=/^https:\/\//i.test(String(source.url||''));
  const institutionalDomain=/\.(gov|gob\.mx|edu|ac\.|int)$/i.test(h);
  if(isHttps)score+=.1;
  if(institutionalDomain)score+=.2;
  ...
  return {title,url:String(source.url||'').slice(0,1200),host:h,provider:clean(source.provider,100),publishedAt:source.published_at||null,qualitySignal:Number(Math.min(1,score).toFixed(3)),basis:{https:isHttps,institutionalDomain,dated:Boolean(source.published_at),substantiveSnippet:snippet.length>=120}};
}
