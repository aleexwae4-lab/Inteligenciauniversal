...
async function stableReferenceFallback(message=''){
  ...
  // Never let fuzzy encyclopedia search silently substitute a different concept.
  // For direct definition questions, the selected title must substantially match
  // the requested concept; otherwise this fallback abstains and another lane wins.
  const requestedKey=clean(query,220).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
  ...
  try{
    ...
    const titleKey=title.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
    const directDefinition=/^\s*[¿?]*\s*(qué|que|quién|quien|cuál|cual)\s+(es|son)\s+/i.test(message);
    if(directDefinition&&requestedKey.length>=4&&!titleKey.includes(requestedKey)&&!requestedKey.includes(titleKey))return null;
