...
function systemFor({intent,memory='',attachments='',researchContext='',toolContext=''}){
...
export function nativeBrainStatus(){
  ...
  return {version:NATIVE_BRAIN_VERSION,missionControl:MISSION_CONTROL_VERSION,ready:true,architecture:'native-quality-council-memory-aware-self-repair-orchestrator',primaryInference:'auto',providers:providers.map(p=>({id:p.id,configured:p.configured,model:p.model})),emergencyProviders:emergencyProviderRegistry().map(p=>({id:p.id,model:p.model})),memory:memoryStatus(),lanes:['local-kernel','quality-council','memory-context','knowledge-fabric','inference-fabric-auto','quality-self-repair','stable-reference','emergency-inference','verified-rescue'],qualityCouncil:true,selfRepair:true,memoryAwareRouting:true,contextDependentFollowups:true,universalRouting:true,responseSchema:'assistant-response/v1'};
}
...
export async function nativeBrainReply(payload={}){
  ...
  const local=!needsContext&&!mission.toolIds.length&&!mission.requiresLive&&!intent.current&&!intent.highImpact&&!attachments.length?nativeLocalReply(message):null;
  if(local){
  ...
  if(intent.factual&&!mission.toolIds.length&&!intent.requiresEvidence&&!attachments.length&&!needsContext){
  ...
  try{
    const system=systemFor({intent,memory:memoryContext,attachments:attachmentContext(attachments),researchContext,toolContext});
