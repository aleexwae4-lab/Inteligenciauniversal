...
import { classifySelfAwarenessV99, buildSelfAwarenessReplyV99, selfAwarenessSnapshotV99 } from './self-awareness-v99.js';
...
async function repairGenerated({payload,intent,system,message,history,generated,sources,started=Date.now()}){
  ...
  // Respect the client-facing native route deadline. A late repair must not push a viable answer past the HTTP timeout.
  const repairBudget=Math.min(COMPLEX_MODES.has(intent.mode)?10000:6500,(COMPLEX_MODES.has(intent.mode)?29000:17000)-(Date.now()-started)-650);
  if(repairBudget<1700)return {generated,text:originalText,sources,quality:originalQuality,repair:{applied:false,reason:'deadline_budget'}};
  try{
    ...
    const revised=await bounded(()=>generateWithFallback({provider:'auto',system:recovery,message,history}),repairBudget,'NATIVE_QUALITY_REPAIR');
...
export async function nativeBrainReply(payload={}){
  ...
  // The mobile UI calls this native route BEFORE /api/chat. Answer runtime-capability
  // questions from the actual capability registry, not a provider's generic disclaimer.
  const webAwareness=attachments.length?{eligible:false}:classifySelfAwarenessV99({...payload,message,mode:payload.mode||'general'});
  if(webAwareness.eligible&&webAwareness.kind==='web'){
    const intent=inferNativeIntent(message,'general');
    const text=buildSelfAwarenessReplyV99({kind:'web'});
    const out=envelope({payload,history,text,provider:'universal_core',model:'web-capability-registry-v1',path:'grounded-web-capability',started,intent});
    out.self_awareness=selfAwarenessSnapshotV99();
    void persist(out,{userKey:clean(payload.userKey||payload.sessionId||'anonymous',160),sessionId:clean(payload.sessionId||payload.session_id,200),conversationId:clean(payload.conversation_id||payload.conversationId,200),message});
    return out;
  }
  ...
  try{
    ...
    if(meaningful(generated?.text)&&canUseEvidence){
      const repaired=await repairGenerated({payload,intent,system,message,history,generated,sources:effectiveSources,started});
      // A missing research candidate is NOT a score of zero: that formerly admitted every meaningful but weak draft.
      const betterThanResearch=!!researchCandidate&&!repaired.quality.critical&&!repaired.quality.requirementCoverage?.hardFailure&&repaired.quality.score>=researchCandidate.quality.score+.025;
      if(acceptableQuality(repaired.quality,{intent,message})||betterThanResearch){
