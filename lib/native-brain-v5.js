...
import { classifyGroundedCompetitionV120, buildGroundedCompetitionReplyV120, GROUNDED_COMPETITION_VERSION } from './grounded-competition-v120.js';
...
function systemFor({intent,memory='',attachments='',researchContext=''}){
...
export function nativeBrainStatus(){
  ...
  return {version:NATIVE_BRAIN_VERSION,ready:true,architecture:'native-quality-council-memory-aware-self-repair-orchestrator',primaryInference:'auto',providers:providers.map(p=>({id:p.id,configured:p.configured,model:p.model})),emergencyProviders:emergencyProviderRegistry().map(p=>({id:p.id,model:p.model})),memory:memoryStatus(),lanes:['local-kernel','quality-council','memory-context','knowledge-fabric','inference-fabric-auto','quality-self-repair','stable-reference','emergency-inference','verified-rescue'],qualityCouncil:true,selfRepair:true,memoryAwareRouting:true,contextDependentFollowups:true,universalRouting:true,responseSchema:'assistant-response/v1'};
}
...
export async function nativeBrainReply(payload={}){
  ...
  // A direct question about Universal Core competing with a named platform is
  // about this runtime's own capabilities, not that platform's market share.
  // Never turn it into an unsupported competitive verdict or a web fact claim.
  const competition=classifyGroundedCompetitionV120(payload);
  if(competition.eligible&&!attachments.length&&!needsContext){
    const reply=buildGroundedCompetitionReplyV120(competition);
    const result=envelope({payload,history,text:reply,provider:'wae_native_kernel',model:GROUNDED_COMPETITION_VERSION,path:'grounded-competition',started,intent,selection:{strategy:'capability-availability',candidates:[]}});
    result.comparison={verified_superiority:false,version:GROUNDED_COMPETITION_VERSION,targets:competition.targets.map(t=>t.name)};
    persist(result,{userKey,sessionId,conversationId,message});trace(result);return result;
  }

  const local=!needsContext&&!intent.current&&!intent.highImpact&&!attachments.length?nativeLocalReply(message):null;
  if(local){
    const localQuality=qualityFor({payload,intent,text:local,sources:[]});
    if(acceptableQuality(localQuality,{intent,message})){
      const out=envelope({payload,history,text:local,provider:'wae_native_kernel',model:'native-knowledge-kernel-v1',path:'local-kernel-first',started,intent,selection:{strategy:'deterministic-kernel',candidates:[{path:'local-kernel',score:localQuality.score}]}});
      persist(out,{userKey,sessionId,conversationId,message});trace(out);return out;
    }
  }

  if(intent.factual&&!intent.requiresEvidence&&!attachments.length&&!needsContext){
    ...
    if(council&&acceptableQuality(council.quality,{intent,message})){
      const out=envelope({payload,history,text:council.text,provider:council.provider,model:council.model,path:council.path,started,degraded:council.degraded===true,sources:council.sources||[],failures:council.failures||[],intent,memoryCount:0,selection:council.selection});
      persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
    }
  }

  const memory=await bounded(()=>recallMemory(userKey,message,10,sessionId,conversationId),2400,'NATIVE_MEMORY').catch(()=>[]);
  ...
  try{
    const system=systemFor({intent,memory:memoryContext,attachments:attachmentContext(attachments),researchContext});
    ...
    const deadline=COMPLEX_MODES.has(intent.mode)?27_000:17_000;
    ...
    const effectiveSources=generatedSources.length?generatedSources:(researchCandidate?.sources||[]);
  ...
  if(intent.factual&&!intent.requiresEvidence&&!attachments.length){
    ...
    if(reference){
      ...
      if(acceptableQuality(quality,{intent,message})){
        const out=envelope({payload,history,text:reference.text,provider:reference.provider,model:reference.model,path:'stable-reference',started,degraded:true,sources:reference.sources,failures:primaryFailure?.failures||[],intent,memoryCount:memory.length,selection:{strategy:'reference-fallback',candidates:[{path:'stable-reference',score:quality.score}]}});
        persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
      }
    }
  }
  ...
  try{
    ...
    if(meaningful(text)&&(!intent.requiresEvidence||emergencySources.length>0)){
      ...
      if(quality.score>=.48){
        const out=envelope({payload,history,text,provider:emergency.provider||'emergency',model:emergency.model||'emergency',path:'emergency-inference',started,degraded:true,sources:emergencySources,failures:primaryFailure?.failures||[],intent,memoryCount:memory.length,selection:{strategy:'emergency-quality-floor',candidates:[{path:'emergency',score:quality.score}]}});
        persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
      }
    }
  }catch{}
  ...
  const out=envelope({payload,history,text,provider:'wae_native_kernel',model:'native-continuity-v5',path:'quality-bounded-continuity',started,degraded:true,failures:primaryFailure?.failures||[],intent,memoryCount:memory.length,selection:{strategy:'quality-floor-reject'}});
  persist(out,{userKey,sessionId,conversationId,message});trace(out,out.failures);return out;
}
