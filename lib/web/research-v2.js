...
export function webIntelligenceHealthV2(){
  const providerState=providerConfiguration();
  const generalSearchConfigured=providerState.providers.some(p=>p.configured&&['tavily','brave','google_custom_search'].includes(p.id));
  return{version:UNIVERSAL_WEB_INTELLIGENCE_V2,status:generalSearchConfigured?'configured':'partial',searchCapabilities:{generalSearchConfigured,keylessInstantAnswersConfigured:true,interactiveBrowser:false,liveQueryVerified:false,verification:'execute /api/web/search to confirm live availability'},registry:sourceRegistryHealth(),providers:providerState,security:{legacy:webSecurityHealth(),direct:directRetrievalHealth()},specialists:{version:WEB_SPECIALISTS_VERSION,domains:['law','medicine','science','software','cybersecurity','finance','business']},persistence:webEvidencePersistenceState(),cacheEntries:CACHE.size,metrics:webIntelligenceMetricsV2(),capabilities:['parallel general+specialist discovery','scientific knowledge federation','direct robots-aware retrieval','HTML/JSON-LD/JSON/XML/RSS/Atom extraction','PDF page intelligence','optional PDF tables','authority/freshness ranking','deduplication','contradiction detection','page/section citations','governed evidence persistence']};
}
