...
function excerptWords(value='',limit=24){
  const parts=String(value||'').trim().split(/\s+/).filter(Boolean);
  if(parts.length<=limit)return parts.join(' ');
  return`${parts.slice(0,limit).join(' ')}…`;
}
function deterministicFocusedReply(records=[],focus={}){
  const usable=records.filter(r=>text(r.abstract||'',1000)&&r.focused_score>=.22).slice(0,2);
  if(!usable.length)return{reply:'',cited:[]};
  const fragments=[],cited=[];
  usable.forEach((record,index)=>{
    const candidate=sentenceCandidates(record,focus)[0]?.sentence||record.abstract;
    const excerpt=excerptWords(candidate,24);
    if(!excerpt)return;
    const key=`K${index+1}`;fragments.push(`${excerpt} [${key}]`);cited.push({key,record});
  });
  if(!fragments.length)return{reply:'',cited:[]};
  const intro=focus.language==='es'?'**Respuesta breve:**':'**Short answer:**';
  return{reply:`${intro} ${fragments.join(' ')}`,cited};
}
...
export async function runFocusedFactualAnswer({body={},fetchImpl=fetch}={}){
  ...
  const response=buildAssistantResponse({content:reply,sources,provider:'universal_core',model:blocked?'context-output-firewall-v56':'focused-factual-extractive-v83',latencyMs,memoryCount:0,requestId:crypto.randomUUID(),webUsed:sources.length>0,degraded:true});
  response.metadata={...(response.metadata||{}),focusedFactual:FOCUSED_FACTUAL_VERSION,focus:focus.focus,focusDomain:focus.domain,focusedQuery:retrieval.query,focusedScore:top.focused_score,sourceStatus:retrieval.status,extractiveFallback:true};
  return{success:true,reply,speech_text:response.speechText,response,components:response.components,actions:response.actions,provider:'universal_core',model:blocked?'context-output-firewall-v56':'focused-factual-extractive-v83',web_sources:sources,degraded:true,quality:{critical:false,relevant:true,focused_score:top.focused_score},recovery:{active:true,path:'focused-factual-v83',focus:focus.focus,domain:focus.domain},latencyMs,agent:{id:'focused-factual',name:'Universal Core Factual Answer Engine'}};
}
