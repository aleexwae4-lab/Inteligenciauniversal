...
export function deterministicKnowledgeFallback(result={}){
  ...
  const lines=top.map(record=>{const originalIndex=(result.records||[]).indexOf(record);const author=record.authors?.[0]?.name?` — ${record.authors[0].name}`:'';const date=record.publicationDate?` (${String(record.publicationDate).slice(0,10)})`:'';const type=record.quality?.scientific?.study_type;const qualifier=type?` — ${type}`:'';return `- **${record.title}**${author}${date}${qualifier} [K${originalIndex+1}]`;});
...
export async function runKnowledgeAnswer({body={},userKey='anonymous'}={}){
  ...
  if(!citationState.reply||citationState.cited.length===0){
    citationGateFallback=true;
    if(String(body.recovery_context||'')==='general_concept')return null;
    citationState=sanitizeCitationMarkers(deterministicKnowledgeFallback(retrieval),valid);
  }
