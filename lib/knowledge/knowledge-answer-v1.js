...
const KNOWLEDGE_RX=/\b(estudio|estudios|paper|papers|art[ií]culo cient[ií]fico|investigaci[oó]n|evidencia cient[ií]fica|pubmed|openalex|crossref|arxiv|doi|ensayo cl[ií]nico|meta.?an[aá]lisis|systematic review|revisi[oó]n sistem[aá]tica|teorema|theorem|biolog[ií]a|f[ií]sica|physics|qu[ií]mica|chemistry|gen[eé]tica|genetics|econom[ií]a|economics|filosof[ií]a|philosophy|historia|history|cient[ií]fico|scientific|acad[eé]mic|academic)\b/i;
...
export async function runKnowledgeAnswer({body={},userKey='anonymous'}={}){
  ...
  const reply=citationState.reply;if(!reply)return null;
  const sources=sourceList(retrieval,citationState.cited.length?citationState.cited:null);const latencyMs=Date.now()-started;
  const quality=evaluateAnswer({question,answer:reply,mode:'research',sources});
  const response=buildAssistantResponse({content:reply,sources,provider:'universal_core',model:generated?.model||'knowledge-fabric-evidence-fallback-v1',latencyMs,memoryCount:0,requestId:retrieval.request_id,webUsed:sources.length>0,degraded:!generated?.text});
  response.metadata={...(response.metadata||{}),knowledgeFabric:UNIVERSAL_KNOWLEDGE_FABRIC_VERSION,knowledgeAnswer:KNOWLEDGE_ANSWER_VERSION,knowledgeSources:retrieval.sources_selected,citationKeys:citationState.cited,documentsRetrieved:retrieval.documents_retrieved,documentsUsed:retrieval.documents_used,quality};
  return{success:true,reply,speech_text:response.speechText,response,components:response.components,actions:response.actions,provider:'universal_core',model:generated?.model||'knowledge-fabric-evidence-fallback-v1',web_sources:sources,knowledge:{version:UNIVERSAL_KNOWLEDGE_FABRIC_VERSION,request_id:retrieval.request_id,sources_selected:retrieval.sources_selected,source_status:retrieval.source_status,documents_retrieved:retrieval.documents_retrieved,documents_used:retrieval.documents_used,citations:retrieval.citations,latency_ms:retrieval.retrieval_latency_ms},degraded:!generated?.text,latencyMs,quality,agent:{id:'knowledge-fabric',name:'WAE Universal Knowledge Fabric'}};
}
