...
export function rerankKnowledgeRecords(records=[],understanding={}){
  ...
  return arr(records).map(record=>{
    ...
    const base=0.58*relevance+0.32*authority.score+0.10*indexRank;
    ...
    return{...record,quality:{...(record.quality||{}),authority,relevance:Number(relevance.toFixed(4)),rerank_score:Number(rerankScore.toFixed(4)),rerank_query_version:'multilingual-scientific/v70.2'}};
...
export async function searchKnowledge(query,options={}){
  ...
  const rankingUnderstanding={...understanding,rerank_normalized:rerankNormalized};
  ...
  const integrityConflicts=detectResearchIntegrityConflicts(profiled);
  const deduped=deduplicateKnowledgeRecords(profiled);
