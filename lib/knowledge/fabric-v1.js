...
export function buildSourceQuery(understanding={},sourceId=''){
  ...
  const entity=understanding.entity_grounding;
  if(!SCIENTIFIC_SOURCES.has(id)){
    if(entity?.explicitEntity){
      const terms=arr(entity.queryTerms);
      return text([original,...terms].join(' '),700);
    }
    return original;
  }
