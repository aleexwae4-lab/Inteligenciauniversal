...
export const UNIVERSAL_KNOWLEDGE_FABRIC_VERSION='universal-knowledge-fabric/v70-scientific-hybrid';
const CACHE=new Map();
...
export function rerankKnowledgeRecords(records=[],understanding={}){
  ...
  return arr(records).map(record=>{
    const authority=evidenceAuthority(record,understanding);const relevance=lexicalRelevance(record,understanding);
    const indexRank=clamp(record.quality?.index_rank||0);const base=0.52*relevance+0.38*authority.score+0.10*indexRank;
    const multiplier=scientificContext?Number(record.quality?.scientific?.ranking_multiplier||1):1;
    const rerankScore=clamp(base*multiplier);
    return{...record,quality:{...(record.quality||{}),authority,relevance:Number(relevance.toFixed(4)),rerank_score:Number(rerankScore.toFixed(4))}};
...
export async function searchKnowledge(query,options={}){
  ...
  const sourceIds=selectKnowledgeSources(understanding,options);const useGlobalIndex=options.includeGlobalIndex!==false&&globalHybridIndexConfigured();
  const cacheKey=JSON.stringify([understanding.normalized,sourceIds,understanding.language,Number(options.limit)||12,useGlobalIndex]);const cached=CACHE.get(cacheKey);if(cached&&Date.now()-cached.at<CACHE_TTL_MS)return{...cached.value,request_id:requestId,cache_hit:true,total_latency_ms:Date.now()-started};
  ...
  const connectorPromise=Promise.allSettled(sourceIds.map(id=>connectors.get(id).search(understanding.query,{limit:perSource,language:understanding.language})));
  ...
  if(useGlobalIndex){records.push(...indexResult.records);statuses.push({source_id:'wae_global_index',status:indexResult.status,count:indexResult.records.length,latency_ms:indexResult.latency_ms,retrieval_mode:indexResult.retrieval_mode,error:indexResult.reason||undefined})}
  const profiled=applyScientificEvidenceProfiles(records,understanding);const integrityConflicts=detectResearchIntegrityConflicts(profiled);
  const deduped=deduplicateKnowledgeRecords(profiled);const ranked=rerankKnowledgeRecords(deduped,understanding).slice(0,Math.max(1,Math.min(Number(options.limit)||12,30)));const citations=ranked.map(citationForRecord);const frames=ranked.map(untrustedEvidenceFrame);const scientific=scientificEvidenceSummary(ranked);
  const toolsUsed=[...sourceIds.map(id=>`knowledge:${id}`),...(useGlobalIndex?[`knowledge:${HYBRID_INDEX_VERSION}`]:[])];
  const result={version:UNIVERSAL_KNOWLEDGE_FABRIC_VERSION,request_id:requestId,query:understanding.query,understanding,sources_selected:sourceIds,source_status:statuses,index_status:indexResult,documents_retrieved:records.length,documents_after_dedup:deduped.length,documents_used:ranked.length,records:ranked,citations,evidence_frames:frames,scientific_evidence:scientific,integrity_conflicts:integrityConflicts,retrieval_latency_ms:Date.now()-started,total_latency_ms:Date.now()-started,cache_hit:false,provenance:{models_used:[],tools_used:toolsUsed,ranking_version:'scientific-hybrid/v70',retrieved_at:new Date().toISOString(),private_vector:privateVectorArchitecture()}};
  ...
  void persistRetrievalLedger({request_id:requestId,organization_id:options.organizationId||null,query:understanding.query,sources_used:[...sourceIds,...(useGlobalIndex?['wae_global_index']:[])],documents_retrieved:records.length,documents_used:ranked.length,ranking_scores:ranked.map(r=>({id:r.id,score:r.quality?.rerank_score,study_type:r.quality?.scientific?.study_type,integrity:r.quality?.scientific?.integrity?.status})),citations,models_used:[],tools_used:toolsUsed,retrieval_time_ms:result.retrieval_latency_ms,generation_time_ms:0,total_latency_ms:result.total_latency_ms,metadata:{fabric_version:UNIVERSAL_KNOWLEDGE_FABRIC_VERSION,domains:understanding.domains,language:understanding.language,scientific_evidence:scientific,integrity_conflicts:integrityConflicts,index_status:indexResult.status}});
  return result;
}
...
export function existingKnowledgeArchitecture(){return{reused:['wae_rag_chunks (pgvector + HNSW)','wae_rag_chunks FTS (GIN)','wae_global_documents_v1 (tsvector + trigram + authority ranking)','wae_global_search_v4','wae_retrieve_knowledge_v91 (tenant-aware vector hybrid)','wae_knowledge_entities','wae_knowledge_relationships','wae_knowledge_index_queue','wae_knowledge_manifests','wae_research_runs','wae_research_evidence','wae_library_public_catalog_v52'],legacy_or_duplicated:['embeddings (jsonb embedding)','vectors (jsonb embedding)','wae_semantic_documents.embedding (jsonb)'],new_control_plane:['wae_knowledge_sources_v1','wae_license_registry_v1','wae_knowledge_retrieval_ledger_v1','wae_knowledge_source_health_v1'],scientific_layer:['scientific-evidence/v2','research-integrity/v2','Europe PMC connector'],vector_policy:privateVectorArchitecture()}}
