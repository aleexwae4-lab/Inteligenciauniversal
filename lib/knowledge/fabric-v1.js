import { createKnowledgeConnectors } from './connectors-v1.js';
import { getKnowledgeSource, listKnowledgeSources, sourceRegistrySnapshot } from './source-registry-v1.js';
...
const DOMAIN_RULES=[
  ['medicine',/\b(medic|clinical|health|salud|enfermedad|tratamiento|therapy|drug|f[aá]rmaco|paciente|diagn[oó]stico|biomed|epidemi|cancer|cardio|neuro)\b/i],
  ...
  ['literature',/\b(literat|novel|novela|book|libro|poem|poes[ií]a|writer|escritor)\b/i],
...
export function understandKnowledgeQuery(query='',options={}){
  const q=text(query,1500);const domains=DOMAIN_RULES.filter(([,rx])=>rx.test(q)).map(([id])=>id);if(!domains.length)domains.push('general_knowledge');
  ...
  const temporal=/\b(ultimo|último|reciente|actual|latest|recent|current|202[4-9])\b/i.test(q);
  return{query:q,normalized:norm(q),domains:[...new Set(domains)],language,temporal,request_type:options.mode==='research'?'research':'search'};
}
...
export function selectKnowledgeSources(understanding={},options={}){
  ...
  const ordered=[];
  for(const domain of arr(understanding.domains))for(const id of PRIORITY[domain]||[])if(connectors.has(id)&&!ordered.includes(id))ordered.push(id);
