...
function safeForRecycle(message=''){
  ...
  if(q.length<3 || q.length>2400) return false;
  if(META_CASUAL_RX.test(q)) return false;
  if(/\b(que sabes hacer|que capacidades tienes|cuales son tus capacidades|capacidades tienes|de que eres capaz|como puedes ayudarme|como funcionas)\b/i.test(q)) return false;
  return !SENSITIVE_OR_VOLATILE_RX.test(q) && !MEMORY_INTENT_RX.test(q);
}
...
function deterministicProtocolReply(message=''){
  ...
  if(/que sabes hacer/.test(q)) return 'Puedo ayudarte directamente con análisis, investigación, programación, datos, planificación, documentos, herramientas y orquestación de tareas. Si me das un problema concreto, lo trabajo en lugar de limitarme a enumerar capacidades.';
  if(/que capacidades tienes|cuales son tus capacidades|capacidades tienes/.test(q)) return 'Mis capacidades abarcan razonamiento, investigación con evidencia, ingeniería, análisis de datos, coordinación de herramientas/agentes y continuidad de contexto. La disponibilidad concreta depende del runtime y sus permisos.';
