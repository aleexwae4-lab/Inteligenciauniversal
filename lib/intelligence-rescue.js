...
const FACTUAL_HINT_RX=/^\s*(qu[eé]|qui[eé]n|cu[aá]l|cu[aá]nto|d[oó]nde|cu[aá]ndo|define|explica|c[oó]mo funciona)\b/i;
...
async function edgeJson(payload,timeoutMs=EDGE_TIMEOUT_MS){
  ...
  const res=await fetch(url,{method:'POST',headers:{'content-type':'application/json','apikey':key,'Origin':process.env.PUBLIC_APP_URL||'https://wae-inteligencia-universal.onrender.com','X-Client-Info':'wae-universal-rescue/2.0'},body:JSON.stringify(payload),signal:AbortSignal.timeout(timeoutMs),cache:'no-store'});
  const data=await res.json().catch(()=>({}));
