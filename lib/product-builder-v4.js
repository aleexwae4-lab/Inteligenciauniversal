...
const forbiddenHtml=/<iframe\b|<object\b|<embed\b|<base\b|<(?:script|link|img|source|video|audio)\b[^>]*\b(?:src|href)\s*=\s*['"]?(?:https?:|\/\/)|<form\b[^>]*\baction\s*=\s*['"]?https?:|javascript\s*:/i;
...
export function inspectProject(files) {
  ...
  const extraFiles=cleaned.filter(f=>!REQUIRED.includes(f.name));
  const validExtras=extraFiles.every(f=>/\.(?:md|json)$/i.test(f.name)&&(!/\.json$/i.test(f.name)||(()=>{try{JSON.parse(f.content);return true}catch{return false}})()));
  const checks={
...
function prompt({request,files,kind}){
  return [
    ...
    'No utilices CDN, frameworks, imágenes remotas, recursos externos, fetch, websockets, imports dinámicos ni solicitudes de red. No dependas de localStorage: la vista previa corre en un sandbox de origen opaco.',
