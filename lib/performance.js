const DEFAULT_GATE = Object.freeze({
  schema:'universal-performance-gate/v3',available:false,routing_state:'HOLD',candidate_promotable:false,stream_ready:false,fast_lane_ready:false,
  stream_ready_count:0,best_stream_ttft_ms:0,stream_freshness_minutes:30,latest_stream_success_at:null,
...
const DEFAULT_SELF_IMPROVEMENT = Object.freeze({
  contract:'universal-core-self-improvement/v1',available:false,mode:'blended_shadow_router_training',router_learning_active:false,
  base_model_weights_changed:false,raw_content_used:false,window_hours:24,
  signals:{total:0,positive:0,negative:0,neutral:0,avg_operational_score:0,learned_routes:0},
  latest_cycle:null,top_learned_routes:[],training_readiness:{explicit_feedback_examples:0,rejected_examples:0},supremacy_gate:null
});
...
export function normalizeSelfImprovement(input={}){
  const raw=Array.isArray(input)?(input[0]||{}):(input||{}),signals=raw.signals&&typeof raw.signals==='object'?raw.signals:{},training=raw.training_readiness&&typeof raw.training_readiness==='object'?raw.training_readiness:{};
  return{contract:String(raw.contract||DEFAULT_SELF_IMPROVEMENT.contract),available:true,mode:String(raw.mode||DEFAULT_SELF_IMPROVEMENT.mode),router_learning_active:bool(raw.router_learning_active),base_model_weights_changed:bool(raw.base_model_weights_changed),raw_content_used:bool(raw.raw_content_used),window_hours:Math.max(1,Math.trunc(finite(raw.window_hours,24))),signals:{total:Math.max(0,Math.trunc(finite(signals.total))),positive:Math.max(0,Math.trunc(finite(signals.positive))),negative:Math.max(0,Math.trunc(finite(signals.negative))),neutral:Math.max(0,Math.trunc(finite(signals.neutral))),avg_operational_score:finite(signals.avg_operational_score),learned_routes:Math.max(0,Math.trunc(finite(signals.learned_routes)))},latest_cycle:raw.latest_cycle&&typeof raw.latest_cycle==='object'?raw.latest_cycle:null,top_learned_routes:Array.isArray(raw.top_learned_routes)?raw.top_learned_routes.slice(0,8):[],training_readiness:{explicit_feedback_examples:Math.max(0,Math.trunc(finite(training.explicit_feedback_examples))),rejected_examples:Math.max(0,Math.trunc(finite(training.rejected_examples)))},supremacy_gate:raw.supremacy_gate&&typeof raw.supremacy_gate==='object'?raw.supremacy_gate:null,evaluated_at:raw.evaluated_at||null};
}
...
export function defaultSelfImprovement(reason='unavailable'){return{...DEFAULT_SELF_IMPROVEMENT,reason:String(reason||'unavailable')}}
...
export async function fetchPerformanceGate({windowHours=6,minOk=20}={}){try{return normalizePerformanceGate(await postRpc('iu_performance_gate_v3',{p_window_hours:Math.max(1,Math.min(168,Number(windowHours)||6)),p_min_ok:Math.max(5,Math.min(1000,Number(minOk)||20))}))}catch(error){return defaultPerformanceGate(error?.name==='TimeoutError'?'timeout':error?.code||'request_failed')}}
...
export async function fetchSelfImprovement({windowHours=24}={}){try{return normalizeSelfImprovement(await postRpc('iu_self_improvement_snapshot_v1',{p_window_hours:Math.max(1,Math.min(168,Number(windowHours)||24))}))}catch(error){return defaultSelfImprovement(error?.name==='TimeoutError'?'timeout':error?.code||'request_failed')}}