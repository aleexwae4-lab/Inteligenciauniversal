...
export async function fetchPerformanceGate({windowHours=6,minOk=20}={}) {
  const base=String(process.env.SUPABASE_URL || PUBLIC_SUPABASE_URL).replace(/\/$/,'');
  const key=String(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_PUBLISHABLE_KEY || process.env.SUPABASE_ANON_KEY || PUBLIC_SUPABASE_KEY);
