...
export function speechText(value=''){
  let t=cleanText(value).normalize('NFKC').replace(/\r\n?/g,'\n');
  t=t
    .replace(/<br\s*\/?>/gi,'\n')
...
export function buildResponseEnvelope({reply='',sources=[],provider=null,model=null,agent=null,latencyMs=0,memory=null,tools=[],fallbackFailures=[],composition=null}={}){
  ...
  return {
    ...
    metadata:{
      ...
      compositionValid:composition?.valid!==false,
      compositionNormalized:composition?.normalized===true,
      compositionHeadings:Number(composition?.headings)||0,
      compositionTables:Number(composition?.tables)||0,
      compositionOrderedItems:Number(composition?.orderedItems)||0
    }
  };
}
