...
function sourceValue(item) {
  if (typeof item === 'string') return clean(item, 2_000);
  if (!item || typeof item !== 'object') return '';
  return clean(item.url ?? item.uri ?? item.href ?? item.source_url ?? item.sourceUrl ?? item.title ?? item.name ?? '', 2_000);
}

export function canonicalBenchmarkSources(items = []) {
  return [...new Set((Array.isArray(items) ? items : []).map(sourceValue).filter(Boolean))].sort().slice(0, 40);
}

export function benchmarkSourcesHash(items = []) {
  return sha256(JSON.stringify(canonicalBenchmarkSources(items)));
}
...
function canonicalAttestationPayload(attestation = {}) {
  return [
    ...
    clean(attestation.sourcesHash, 128),
...
export function gptComparatorReadiness(registry = providerRegistry()) {
  const row = (Array.isArray(registry) ? registry : []).find(item => safeProvider(item?.id) === VERIFIED_GPT_REFERENCE_PROVIDER) || null;
  ...
  return {
    ...
    model,
    providerRoutingModel: safeModel(row?.model) || null,
    reasoningEffort,
    researchWebSearch: true,
    sourceProvenanceRequired: true,
    referenceId: versioned ? versionedReferenceId(VERIFIED_GPT_REFERENCE_PROVIDER, model) : null,
    versioned,
    executable: row?.configured === true && versioned,
    reason: row?.configured === true && versioned ? 'ready' : row?.configured === true ? 'reference_model_not_versioned' : 'openai_provider_not_configured',
  };
}
...
export function createRuntimeAttestation({
  ...
  const attestation = {
    ...
    sourcesHash: benchmarkSourcesHash(sources),
...
export function verifyRuntimeAttestation({ attestation, answer, sources = [], caseId, promptHash, candidateId, secretOverride = '' } = {}) {
  const secret = secretValue(secretOverride);
  ...
  if (clean(row.sourcesHash, 128) !== benchmarkSourcesHash(sources)) reasons.push('sources_hash_mismatch');
  ...
  return {
    ...
    sourcesHash: clean(row.sourcesHash, 128) || null,
...
export function verifiedGptArenaManifest() {
  ...
  return {
    ...
    referencePolicy: verifiedGptReferencePolicy(),
    responseHash: 'SHA-256',
    sourcesHash: 'SHA-256',
    ...
    fairResearchToolsRequired: true,
    claimPolicy: {
      ...
      forbidden: 'Global superiority, unversioned GPT-family claims, pasted or simulated competitor answers, unsigned responses, source-list tampering, mixed reference models, incomplete suites, or claims when OpenAI is not actually configured and executed.',
    },
  };
}
...
export function certifyVerifiedGptRun({ entries = [], targetId = 'universal_core', referenceId = '', secretOverride = '' } = {}) {
  ...
  for (const testCase of suite) {
    ...
    const targetCheck = verifyRuntimeAttestation({
      attestation: attestationFor(entry, target), answer: target.answer ?? target.text ?? '', sources: target.sources ?? [], caseId: testCase.id,
      promptHash: testCase.promptHash, candidateId: targetId, secretOverride,
    });
    const referenceCheck = verifyRuntimeAttestation({
      attestation: attestationFor(entry, reference), answer: reference.answer ?? reference.text ?? '', sources: reference.sources ?? [], caseId: testCase.id,
  ...
  return {
    ...
    claim: claimAllowed
      ? `Measured advantage over ${clean(referenceId, 160)} on ${ADVERSARIAL_EVIDENCE_VERSION}, with signed v93 runtime provenance for every paired response and source set. This does not establish universal superiority.`
