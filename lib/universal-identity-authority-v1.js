...
export function buildIdentityAuthorityReply(kind='knowledge',snapshot={}){
  ...
  const providers=providerConfiguration().providers||[];
  if(kind==='knowledge'){
...
export function publicIdentityAuthority(snapshot={}){
  return {version:UNIVERSAL_IDENTITY_AUTHORITY_VERSION,identity:'Universal Core',providerIdentityIsolated:true,staticKnowledgeIsNotUniversalCutoff:true,liveResearchDependsOnRuntime:true,configuredProviders:(providerConfiguration().providers||[]).filter(x=>x?.configured).map(x=>x.id).slice(0,8)};
}
