...
import {extractVisibleVisualReply} from './visual-final.js';
...
const statusFor=code=>code==='iu_daily_visual_limit'||code==='iu_free_vision_rate_limited'?429:code==='iu_invalid_session'||code==='iu_session_required'?401:code==='iu_image_invalid'||code==='iu_frames_invalid'?422:503;
...
const fallbackMessages={
 ...
 iu_free_vision_rate_limited:'Los modelos de visión gratuitos están saturados. Tu imagen sigue lista; vuelve a intentar después sin activar servicios de pago.',
 iu_no_verified_free_visual_provider:'No hay ahora un modelo visual confirmado como gratuito y disponible. La imagen sigue preparada.',
...
export async function forwardVisual(body={},env=process.env,request=fetch){
 ...
 const reply=extractVisibleVisualReply(data.reply);
 if(!reply||data.grounded!==true||data.pixel_transport!=='inline_data_uri')
  throw failure('visual_gateway_ungrounded','El motor no entregó una respuesta visual final verificable. Tu captura sigue preparada.',502);
 return {reply,model:data.model||null,provider:data.provider||null,grounded:true,pixelTransport:data.pixel_transport,
