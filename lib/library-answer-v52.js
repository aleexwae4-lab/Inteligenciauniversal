...
import { libraryRelevant, retrieveLibraryIntelligence, publicLibraryMetadata } from './library-intelligence-v52.js';

export const LIBRARY_ANSWER_VERSION='library-answer/v52';
...
export function shouldUseLibraryAnswer(body={}){
  ...
  return /\b(libro|libros|obra|obras|autor|autora|bibliografia|biblioteca|novela|ensayo|capitulo|literatura|open library|gutenberg)\b/.test(q)||body?.library===true;
}
...
export async function runLibraryAnswer({body={},userKey='anonymous'}={}){
  ...
  const history=Array.isArray(body.history)?body.history.slice(-10).filter(x=>x&&['user','assistant'].includes(x.role)).map(x=>({role:x.role,content:String(x.text??x.content??'').slice(0,9000)})):[];
  const generated=await generateWithFallback({
    provider:body.provider||'auto',
    system:'Eres Universal Core con acceso a una biblioteca cognitiva federada. Responde de forma moderna y útil. Usa los metadatos bibliográficos solo para existencia, autoría, edición y temas. Cuando uses un registro recuperado, identifica su marcador [L#] de forma breve. No inventes citas ni atribuyas contenido textual específico si no hay extracto autorizado. Puedes combinar la evidencia con conocimiento general del modelo, distinguiendo cuando una afirmación no proviene de texto recuperado.',
    message:`${message}${library.context}`,
    history
  });
