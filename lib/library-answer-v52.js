...
function canonicalMetadata(message,libraryEvidence=[],groundingEvidence=[]){
  ...
  const authors=groundedAuthors.length?groundedAuthors:itemAuthors.slice(0,1);
  ...
  const corroboratedYears=[...firstYears,...(groundedYear?[groundedYear]:[])];
  const firstYear=corroboratedYears.length?Math.min(...corroboratedYears):null;
