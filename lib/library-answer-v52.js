...
export const LIBRARY_ANSWER_VERSION='library-answer/v59-factual-grounding';
...
export async function runLibraryAnswer({body={},userKey='anonymous'}={}){
  ...
  const quality=evaluateAnswer({question:message,answer:reply,mode:groundingNeeded?'research':'analysis',sources});
  const response=buildAssistantResponse({content:reply,sources,provider:'universal_core',model:'library-intelligence-v59',latencyMs,memoryCount:0,requestId:crypto.randomUUID(),webUsed:sources.length>0,degraded:false});
  response.metadata={...response.metadata,libraryIntelligence:true,libraryVersion:LIBRARY_ANSWER_VERSION,contextTrustPlane:CONTEXT_TRUST_PLANE_VERSION,factualGrounding:publicGroundingMetadata(grounding),quality};
  return{
    success:true,reply,response,speech_text:response.speechText,components:response.components,actions:response.actions,
    provider:'universal_core',model:'library-intelligence-v59',web_sources:sources,
