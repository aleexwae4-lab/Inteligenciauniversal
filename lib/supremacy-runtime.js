import { evaluationPlaneCapabilities, tournamentCandidateScore } from './evaluation-plane.js';
...
export function shouldRunChallenger({message='',mode='general',quality={},degraded=false,requestedProvider='auto'}={}){
  ...
  // v85: explicit requirement failures always deserve a second candidate when a real provider is available.
  if(quality?.requirementCoverage?.hardFailure===true)return true;

  // Short prompts can still be difficult. Skip the challenger only when a short answer is already reasonably strong.
  if(String(message||'').length<70&&score>=.58)return false;

  // Research now uses the same production pass threshold as the verifier. Previously scores 0.50-0.67
  // could leave the runtime without a challenger even though quality.pass was false.
  if(String(mode)==='research')return score<.68;

  // Sensitive domains remain conservative on extra generation, but materially weak drafts are challenged.
  if(SENSITIVE_RX.test(message))return score<.52;
...
export function selectBestCandidate(candidates=[]){
  ...
  valid.sort((a,b)=>tournamentCandidateScore(b)-tournamentCandidateScore(a));
  return valid[0];
}
...
export function supremacyStats(){
  ...
  return {contract:'universal-core-supremacy/v3',semanticCache:{enabled:true,scope:'user',entries:entries.length,hits:entries.reduce((n,x)=>n+(x.hits||0),0),ttlMs:DEFAULT_TTL_MS,sensitiveCaching:false,sourcedRecoveryCaching:false},selectiveTournament:{enabled:true,maxCandidates:2,qualityTriggered:true,blind:true,passThreshold:.68,researchThresholdAligned:true,hardRequirementRetry:true},reranker:{enabled:true,engine:'blind-requirement-weighted-v35',providerIdentityUsed:false},evaluationPlane:evaluationPlaneCapabilities()};
}
