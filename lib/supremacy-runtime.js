...
export function semanticCacheStore({message,userKey,mode='general',provider='auto',text,model,actualProvider,quality,sources=[],ttlMs=DEFAULT_TTL_MS}={}){
  ...
  cache.set(key,{scope,message:String(message||''),normalized:normalized(message),text:String(text||''),provider:String(actualProvider||'universal_core'),model:String(model||'unknown'),quality:quality||null,sources:Array.isArray(sources)?sources.slice(0,8):[],createdAt,lastHitAt:createdAt,hits:0,expiresAt:createdAt+Math.max(60_000,Number(ttlMs)||DEFAULT_TTL_MS)});
  prune(createdAt);
  return true;
}
...
export function supremacyStats(){
  ...
  return {contract:'universal-core-supremacy/v1',semanticCache:{enabled:true,scope:'user',entries:entries.length,hits:entries.reduce((n,x)=>n+(x.hits||0),0),ttlMs:DEFAULT_TTL_MS,sensitiveCaching:false},selectiveTournament:{enabled:true,maxCandidates:2,qualityTriggered:true},reranker:{enabled:true,engine:'deterministic-quality-v1'}};
}
