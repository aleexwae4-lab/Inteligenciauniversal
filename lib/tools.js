...
export async function runTools({ agent, message, requestedTools=[] }) {
  ...
  for (const id of desired) {
    ...
    try {
      ...
      if (id === 'public_research') { const result=await retrieveResearch(message,{mode:'auto'}); if(!result.ok)throw new Error(result.code||'public_research_unavailable');data=result.results; }
