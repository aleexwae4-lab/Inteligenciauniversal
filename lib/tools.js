...
import {retrieveResearch,researchCapabilities} from './live-research-v119.js';
...
export function toolRegistry() {
  return [
    ...
    { id:'public_research', configured:true, readonly:true, description:'Metadatos académicos o referencias enciclopédicas públicas: no equivalen a noticias en vivo ni a web general.' },
...
export async function runTools({ agent, message, requestedTools=[] }) {
  ...
  for (const id of desired) {
    ...
    try {
      ...
      if (id === 'public_research') { data = await retrieveResearch(message,{mode:'auto'}); if(!data.ok)throw new Error(data.code||'public_research_unavailable'); }
      if (id === 'public_research') {
        const q=String(message||'');
        const mode=/\b(cientific|ciencia|research|estudio|paper|academ|investigacion|laboratorio|medic|epidem|science)\b/i.test(q)?'academic':'encyclopedia';
        const r=await retrieveResearch(q,{mode});
        data=r.results;
      }
