...
import {searchWaeweb,waewebSearchConfigured} from './waeweb-research-v125.js';
...
export function toolRegistry() {
  return [
    { id:'waeweb_search', configured:waewebSearchConfigured(), readonly:true, description:'WAEWEB: Connect/v1 autenticado cuando existe; búsqueda pública de solo lectura si está habilitada.' },
...
export async function runTools({ agent, message, requestedTools=[] }) {
  ...
  const automaticPublicFallback=desired.includes('web_search')&&!registry.web_search.configured&&requestedTools.includes('web_search')&&!requestedTools.includes('public_research');
  if(automaticPublicFallback)desired.push('public_research');
  const results = [];
  ...
  for (const id of desired) {
    ...
    if(waewebComplete&&(id==='web_search'||id==='public_research'&&automaticPublicFallback))continue;
    try {
      let data = null,coverage=null;
      if (id === 'waeweb_search') {
        const response=await searchWaeweb(message);
        ...
        coverage={status:response.status,sources:response.sources,
          ...
          fetchedAt:response.fetchedAt,transport:response.transport||'waeweb-connect/v1',generalIndex:response.generalIndex??null};
