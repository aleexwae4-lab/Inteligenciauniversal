import { planRuntimeTools } from './truth-speed.js';
...
async function executeTool(id,message){
  try {
    let data = null;
    if (id === 'web_search') data = await webSearch(message);
    if (id === 'github_search') data = await githubSearch(message, message);
    return data ? { tool:id, ok:true, data } : null;
  } catch (error) {
    return { tool:id, ok:false, error:String(error.message || error) };
  }
}

export async function runTools({ agent, message, requestedTools=[], provider='auto', attachments=[] }) {
  const registry = Object.fromEntries(toolRegistry().map(t => [t.id,t]));
  const {tools} = planRuntimeTools({agentTools:agent?.tools||[],requestedTools,message,mode:agent?.id||'general',attachments,provider});
  const desired = tools.filter(id=>registry[id]?.configured);
  const results = await Promise.all(desired.map(id=>executeTool(id,message)));
