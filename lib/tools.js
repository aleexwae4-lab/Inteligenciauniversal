...
import { researchWebV2 } from './web/research-v2.js';
import { resolveCurrentFactQuery, rankCurrentFactEvidence } from './current-fact-resolution-v1.js';
...
export function toolRegistry() {
  return Object.entries(LEGACY_TOOL_ALIAS).map(([id,canonicalId])=>{
    ...
    return {
      ...
      description:canonicalId==='web.search'?'Investigación web v2 multi-proveedor con recuperación directa, evidencia estructurada, PDFs, autoridad, frescura y citas.':'Búsqueda de código en repositorios accesibles por GitHub.',
...
export async function executeWebSearch(query) {
  const result = await researchWebV2(query,{limit:10});
...
export async function runTools({ agent, message, requestedTools=[], allowKeylessWeb=false }) {
  ...
  for (const id of desired) {
    ...
    if (!meta) continue;
    // Even without a paid general-search provider, attempt the explicitly limited,
    // keyless DuckDuckGo Instant Answer path. Keep the registry's configured=false
    // contract intact: this fallback is NOT a configured full-web search engine.
    if (!meta.configured && (id !== 'web_search' || allowKeylessWeb!==true)) continue;
    try {
      let data = null;
