...
import { GAME_STUDIO_VERSION, createNativeGameStudioProject, inspectGameStudioProject, supportsNativeGameStudio } from './game-studio-v13.js';
...
export function inspectFoundryProject(files,{profile='digital_product'}={}){
  ...
  if(profile==='game_3d'||profile==='simulation_3d'){
    Object.assign(checks,{
      ...
      webgl:/getContext\(\s*['"]webgl2?['"](?:\s*,[^)]*)?\s*\)/i.test(js),
...
function profileInstructions(profile){
  ...
  if(profile==='game_3d'||profile==='simulation_3d')base.push(
    ...
    'GAME STUDIO V12: incluye scene.json con engine wae-game-studio/v13, world.json, logic.json con schema wae-logic/v12, npc.json, combat.json, story.json con schema wae-story/v12 y savegame.json con schema wae-save/v12. Debe implementar quests, objetivos encadenados, árboles de diálogo, decisiones, variables narrativas, journal y guardado/restauración de partida.'
  );
...
export async function buildDigitalProduct({request,files=[],kind='auto',generate=generateWithFallback}){
  ...
  try{
    ...
    if(nativeStudio&&audit.pass){
      ...
      if(!studioAudit.pass){
        generationFailure=ERROR('game_studio_quality_gate_failed','Game Studio v6 incompleto: '+studioAudit.failed.join(', ')+'.');
      }
    }

    if(!audit.pass&&!generationFailure){
      generationFailure=ERROR('project_quality_gate_failed','No se aplicó el proyecto: fallaron '+audit.failed.join(', ')+'.');
    }
  }catch(error){
  ...
  if(generationFailure){
    if(nativeStudio&&existing.length===0){
      ...
      if(!audit.pass||!studioAudit.pass){
        throw ERROR('native_game_studio_failed','La recuperación nativa Game Studio v6 no superó QA: '+[...audit.failed,...studioAudit.failed].join(', ')+'.');
      }

      result={provider:'wae_native_game_studio',model:GAME_STUDIO_VERSION};
      recovery={
        mode:'native_game_studio',
        reason:String(generationFailure.code||'provider_or_quality_failure').slice(0,80),
  ...
  if(nativeStudio&&!studioAudit.pass){
    throw ERROR('game_studio_quality_gate_failed','Game Studio v6 incompleto: '+studioAudit.failed.join(', ')+'.');
  }

  const old=byName(existing);
  const changes=audit.files
    .filter(file=>old.get(file.name)!==file.content)
    .map(file=>({name:file.name,change:old.has(file.name)?'modified':'created'}));

  return{
    version:PRODUCT_FOUNDRY_VERSION,
    status:'completed',
    kind:profile,
    profile:PRODUCT_PROFILES[profile],
    project:{files:audit.files},
    plan:compact(output.plan).slice(0,1000),
    changes,
    quality:{
      structural:'passed',
      checks:audit.checks,
      repaired,
      browserTests:'not_run',
      nativeBuild:'not_run',
      totalChars:audit.totalChars,
      gameStudio:studioAudit?.checks||null
    },
    artifact:{preview:'index.html',targets:targetsFor(profile),export:'zip-source'},
    studio:nativeStudio?{
      version:GAME_STUDIO_VERSION,
      mode:recovery?'native_fallback':'ai_generated',
      qa:'passed'
    }:null,
    recovery,
