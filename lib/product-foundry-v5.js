...
export const PRODUCT_PROFILES=Object.freeze({
  ...
  game_3d:{label:'Juego 3D',preview:'browser',requirements:['WebGL/WebGL2 real','shaders o pipeline 3D propio','game loop','cámara o matrices 3D','input teclado/táctil','resize','HUD']},
  simulation_3d:{label:'Simulación 3D',preview:'browser',requirements:['WebGL/WebGL2 real','escena 3D','loop','cámara','controles','estado de simulación']},
...
export function inspectFoundryProject(files,{profile='digital_product'}={}){
  ...
  const allJs=cleaned.filter(f=>/\.js$/i.test(f.name));
  ...
  const checks={
    ...
    jsSyntax:allJs.every(f=>jsSyntax(f.content,f.name)),
