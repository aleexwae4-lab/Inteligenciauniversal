...
export async function buildDigitalProduct({request,files=[],kind='auto',generate=generateWithFallback}){
  ...
  if(goal.length<8||goal.length>4200)throw ERROR('invalid_brief','Describe tu producto entre 8 y 4200 caracteres.',400);
  const profile=productKind(goal,kind==='auto'?'':kind);
  const existing=compactExisting(files);
  const nativeStudio=supportsNativeGameStudio(profile);
  let result=null,output=null,audit=null,studioAudit=null,repaired=false,recovery=null,generationFailure=null;
  try{
    result=await generateProject({goal,existing,profile,generate});
    output=parseFoundryOutput(result.text);
    audit=inspectFoundryProject(output.files,{profile});
    if(!audit.pass){
      const repairedResult=await generateProject({goal,existing:output.files,profile,generate,repairFrom:audit});
      output=parseFoundryOutput(repairedResult.text);
      audit=inspectFoundryProject(output.files,{profile});
      repaired=true;result=repairedResult;
    }
    if(nativeStudio&&audit.pass){
      studioAudit=inspectGameStudioProject(audit.files);
      if(!studioAudit.pass)generationFailure=ERROR('game_studio_quality_gate_failed','Game Studio v6 incompleto: '+studioAudit.failed.join(', ')+'.');
    }
    if(!audit.pass&&!generationFailure)generationFailure=ERROR('project_quality_gate_failed','No se aplicó el proyecto: fallaron '+audit.failed.join(', ')+'.');
  }catch(error){generationFailure=error}
  if(generationFailure){
    if(nativeStudio&&existing.length===0){
      output=createNativeGameStudioProject({request:goal,profile});
      audit=inspectFoundryProject(output.files,{profile});
      studioAudit=inspectGameStudioProject(audit.files);
      if(!audit.pass||!studioAudit.pass)throw ERROR('native_game_studio_failed','La recuperación nativa Game Studio v6 no superó QA: '+[...audit.failed,...studioAudit.failed].join(', ')+'.');
      result={provider:'wae_native_game_studio',model:GAME_STUDIO_VERSION};
      recovery={mode:'native_game_studio',reason:String(generationFailure.code||'provider_or_quality_failure').slice(0,80),providerIndependent:true};
    }else throw generationFailure;
  }
  if(nativeStudio&&!studioAudit)studioAudit=inspectGameStudioProject(audit.files);
  if(nativeStudio&&!studioAudit.pass)throw ERROR('game_studio_quality_gate_failed','Game Studio v6 incompleto: '+studioAudit.failed.join(', ')+'.');
  ...
  return{
    version:PRODUCT_FOUNDRY_VERSION,status:'completed',kind:profile,profile:PRODUCT_PROFILES[profile],
    project:{files:audit.files},plan:compact(output.plan).slice(0,1000),changes,
    quality:{structural:'passed',checks:audit.checks,repaired,browserTests:'not_run',nativeBuild:'not_run',totalChars:audit.totalChars,gameStudio:studioAudit?.checks||null},
    artifact:{preview:'index.html',targets:targetsFor(profile),export:'zip-source'},
    studio:nativeStudio?{version:GAME_STUDIO_VERSION,mode:recovery?'native_fallback':'ai_generated',qa:'passed'}:null,
    recovery,
  ...
  const profile=productKind(goal,kind==='auto'?'':kind);
  const existing=compactExisting(files);
  let result=await generateProject({goal,existing,profile,generate});
  let output=parseFoundryOutput(result.text),audit=inspectFoundryProject(output.files,{profile}),repaired=false;
  if(!audit.pass){
    const repairedResult=await generateProject({goal,existing:output.files,profile,generate,repairFrom:audit});
    output=parseFoundryOutput(repairedResult.text);
    audit=inspectFoundryProject(output.files,{profile});
  ...
  if(!audit.pass)throw ERROR('project_quality_gate_failed','No se aplicó el proyecto: fallaron '+audit.failed.join(', ')+'.');
  const old=byName(existing);
  const changes=audit.files.filter(f=>old.get(f.name)!==f.content).map(f=>({name:f.name,change:old.has(f.name)?'modified':'created'}));
  return{
    version:PRODUCT_FOUNDRY_VERSION,status:'completed',kind:profile,profile:PRODUCT_PROFILES[profile],
    project:{files:audit.files},plan:compact(output.plan).slice(0,1000),changes,
    quality:{structural:'passed',checks:audit.checks,repaired,browserTests:'not_run',nativeBuild:'not_run',totalChars:audit.totalChars},
    artifact:{preview:'index.html',targets:targetsFor(profile),export:'zip-source'},
    provider:result.provider,model:result.model
  };
}
