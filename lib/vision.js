...
export async function analyzeVisual(body={},env=process.env,request=fetch){
  ...
  const visible=extractVisibleVisualReply(reply);
  if(!visible)throw Object.assign(new Error('No hubo una respuesta visual final verificable; conserva tu captura.'),{statusCode:502,code:'empty_visual_response'});
  return{reply:visible,grounded:true,pixelTransport:'inline_data_uri',mediaKind:kind,analyzedFrames:frames.length,videoScope:kind==='video'?'sampled_frames_only':'image',model};
}
