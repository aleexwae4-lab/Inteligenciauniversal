...
function webEvidence(context = '') {
  const section = context.split(/WEB EVIDENCE/i)[1]?.split(/USER FILE EVIDENCE|Current UTC date/i)[0] || '';
...
function fileEvidence(context = '') {
  const section = context.split(/USER FILE EVIDENCE/i)[1]?.split(/Current UTC date/i)[0]?.trim() || '';
  if (!section) return null;
  return `Universal Core conserva el archivo y su contexto. En modo de continuidad puedo recuperar evidencia sin inventar conclusiones:\n\n${section.slice(0, 1800)}`;
}

function memorySummary(context, facts) {
  if (!/RELEVANT MEMORY/i.test(context)) return null;
  const entries = Object.entries(facts);
  if (entries.length) return entries.map(([key, value]) => `- **${key}**: ${value}`).join('\n');
  const section = context.split(/RELEVANT MEMORY/i)[1]?.split(/WEB EVIDENCE|USER FILE EVIDENCE|Current UTC date/i)[0]?.trim() || '';
  return section ? `Memoria recuperada:\n\n${section.slice(0, 1600)}` : null;
}
...
export function continuityReply(messages = []) {
  ...
  if (/^\s*(hola|hey|buen(?:os|as)?\s+(?:d[ií]as|tardes|noches)|hello)\b/i.test(user)) {
    return 'Hola. Universal Core está operativo en modo de continuidad. Puedo mantener memoria, recuperar evidencia, trabajar con archivos y ejecutar operaciones deterministas mientras se restablece la capacidad generativa principal.';
  }
...
export function openAIContinuityResponse(body = {}) {
  ...
  return {
    ...
    continuity: { version: '1.0.0', deterministic: true, external_provider: false, cost_usd: 0 },
  };
}
