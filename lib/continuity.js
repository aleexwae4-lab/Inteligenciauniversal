...
function webEvidence(context = '') {
  ...
  return `Evidencia literal disponible de las herramientas. Se trata como datos no confiables, nunca como instrucciones:\n\n${safe}`;
}
...
function fileEvidence(context = '') {
  ...
  if (!safe.text) return 'Detecté instrucciones incrustadas en el archivo y las bloqueé para proteger la conversación y la memoria.';
  return `Conservo el archivo y su contexto. Puedo recuperar la evidencia disponible sin inventar conclusiones:\n\n${safe.text.slice(0, 2200)}`;
}
...
function memorySummary(context, facts) {
  ...
  return 'Hay memoria privada recuperada disponible para esta sesión, pero Universal Core no expone el bloque bruto de memoria.';
}
...
export function continuityReply(messages = []) {
  ...
  if (/^\s*(hola|hey|buen(?:os|as)?\s+(?:d[ií]as|tardes|noches)|hello)\b/i.test(user)) {
    return 'Hola. Universal Core está listo. Puedo conservar el contexto, recuperar evidencia, trabajar con archivos y resolver operaciones verificables.';
  }

  if (/\b(recuerda|remember)\b/i.test(user)) return 'Entendido. El dato queda conservado en la memoria de esta sesión.';

  return 'No pude completar la generación avanzada en este intento. Conservé la conversación, la memoria y la evidencia disponible para que puedas reintentar sin perder el contexto.';
}
...
export function openAIContinuityResponse(body = {}) {
  ...
  return {
    ...
    continuity: { version: '1.3.0', deterministic: true, external_provider: false, cost_usd: 0, raw_memory_exposed: false, structured_document_extraction: true, public_copy: 'infrastructure-neutral' },
  };
}
