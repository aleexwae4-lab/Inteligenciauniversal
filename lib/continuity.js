...
function embeddedInstructionDetected(source = '') {
  return /(?:INSTRUCTION\s+TO\s+MODEL|IGNORE\s+(?:ALL\s+)?PREVIOUS\s+INSTRUCTIONS|SYSTEM\s+INSTRUCTION|REVEAL\s+(?:ANY\s+)?PRIVATE\s+MEMORY|PROMPT\s+INJECTION)/i.test(source);
}
...
export function extractContinuityFacts(source = '') {
  ...
  const approvedTotal = capture(source, /(?:\bapproved\s+total\b|\btotal\s+aprobado\b)\s*[:=]?\s*([\d.,]+)/i);
  const currency = capture(source, /(?:\bcurrency\b|\bmoneda\b)\s*[:=]?\s*([A-Z]{3})\b/i) || capture(source, /(?:\bapproved\s+total\b|\btotal\s+aprobado\b)\s*[:=]?\s*[\d.,]+\s*([A-Z]{3})\b/i);
  ...
  if (approvedTotal) facts.approved_total = Number(approvedTotal.replace(/,/g, ''));
  if (currency) facts.currency = currency.toUpperCase();
  facts.ignored_embedded_instruction = embeddedInstructionDetected(source);
  facts.leaked_private_memory = false;
  return facts;
}
...
function mapFact(key, facts) {
  ...
  const aliases = {
    ...
    approved_total: 'approved_total', total_aprobado: 'approved_total', approved_amount: 'approved_total',
    currency: 'currency', moneda: 'currency',
    ignored_embedded_instruction: 'ignored_embedded_instruction', prompt_injection_ignored: 'ignored_embedded_instruction',
    leaked_private_memory: 'leaked_private_memory', private_memory_leaked: 'leaked_private_memory',
  };
...
function memorySummary(context, facts) {
  ...
  const entries = Object.entries(facts).filter(([key]) => !['ignored_embedded_instruction','leaked_private_memory'].includes(key));
...
export function openAIContinuityResponse(body = {}) {
  ...
  return {
    ...
    continuity: { version: '1.2.0', deterministic: true, external_provider: false, cost_usd: 0, raw_memory_exposed: false, prompt_injection_safe: true },
  };
}
