...
function attachmentContext(body={}){
  const rows=(Array.isArray(body.attachments)?body.attachments:[]).slice(0,5).filter(x=>x&&typeof x.text==='string').map(x=>`[${String(x.name||'archivo').slice(0,120)}]\n${String(x.text).slice(0,14000)}`);
...
export async function runSpecialistSinglePassV91({body={},plan=planSpecialistCopilots(body)}={}){
  ...
  const overlay=specialistSystemOverlay(plan),userContextState=userContextStateV92(body),userContext=publicUserContextV92(userContextState);
  const system=`Eres Universal Core. ${overlay}\n\nResponde directamente y produce el entregable solicitado. No inventes acciones ejecutadas, fuentes, datos actuales ni credenciales humanas. No expongas deliberación interna. Mantén una respuesta compacta salvo que la tarea exija profundidad.${userContextSystemInstructionV92(body.preferences||{})}`;
  ...
  response.metadata={...(response.metadata||{}),specialistCopilots:publicSpecialistPlan(plan),quality,specialistSinglePass:true,userContext};
  return{
    success:true,reply,speech_text:response.speechText,components:response.components,actions:response.actions,response,
    provider:'universal_core',model:'specialist-copilot-single-pass-v91',degraded:false,latencyMs,web_sources:[],quality,user_context:userContext,
