...
import {searchWaewebPublic,waewebPublicConfigured} from './waeweb-public-v126.js';
...
// A working private connection takes precedence. Public GET remains separate,
// with no machine credentials or privileged API exposure.
export const waewebSearchConfigured=(env=process.env)=>waewebConfigured(env)||waewebPublicConfigured(env);
export async function searchWaeweb(query,{env=process.env,privateSearch=searchWaewebEvidence,publicSearch=searchWaewebPublic}={}){
 if(waewebConfigured(env)){
  try{return {...await privateSearch(query,{env}),transport:'waeweb-connect/v1'}}
  catch(error){if(!waewebPublicConfigured(env))throw error}
 }
 if(waewebPublicConfigured(env))return publicSearch(query,{env});
 throw new WaewebConnectError('waeweb_search_not_configured',503);
}
