...
async function build(){
  ...
  busy=true;send.disabled=true;entry.disabled=true;['#wfProjects','#wfNewProject','#wfImportCanvas','#wfAgentEngine'].forEach(q=>{const el=$(q);if(el)el.disabled=true});
  ...
  try{
    const res=await fetch(multiFile?'/api/factory-project':'/api/canvas',{
      ...
      body:JSON.stringify(multiFile
        ...
        :{request:mission,kind:t.kind||undefined,baseHtml:refining?snapshot.html:''}),
    ...
    clearTimeout(timeout);busy=false;send.disabled=false;entry.disabled=false;['#wfProjects','#wfNewProject','#wfImportCanvas','#wfAgentEngine'].forEach(q=>{const el=$(q);if(el)el.disabled=false});entry.focus();
  }
}
...
function init(){
  ...
  $('#wfAgentExport').addEventListener('click',()=>$('#wfExportZIP')?.click());
