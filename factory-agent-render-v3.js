...
async function build(){
  ...
  state(multiFile&&['game_3d','simulation_3d'].includes(selectedKind)?'Game Studio v11 → mundo → NPCs → combate → inventario → checkpoint → respawn → QA…':multiFile?'Arquitectura → archivos → auditoría → guardado…':refining?'Analizando contexto → especialistas → construcción → QA…':'Definiendo producto → especialistas → construcción → QA…');
  ...
  try{
    ...
    save();
    const action=data.recovery?.mode==='native_game_studio'?'La ruta generativa no respondió, pero Game Studio v11 nativo recuperó la construcción y creó un producto 3D funcional.':refining?'Actualicé tu producto.':'Construí la primera versión de tu producto.';
